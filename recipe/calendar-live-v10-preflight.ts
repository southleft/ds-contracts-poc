...
import {
  validateCalendarLiveV10AntecedentIndex,
  verifyCalendarLiveV10Authorization,
  type CalendarLiveV10AntecedentIndex,
  type CalendarLiveV10AuthorizationProof,
} from "./calendar-live-v10-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V10_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v10/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V10_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v10-security-attestation.json";

export interface CalendarLiveV10SecurityAttestation {
  artifactVersion: "calendar-live-v10-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV10PreflightReport {
  artifactVersion: "calendar-live-v10-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV10SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV10SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV10AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v10-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V10_TARGET) ||
...
export function validateCalendarLiveV10ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV10Authorization");
  const preflight = source.indexOf("runCalendarLiveV10Preflight");
  const initialize = source.indexOf("CalendarLiveV10Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV10Preflight(
  root: string,
  proof: CalendarLiveV10AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V10_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV10PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V10_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V10_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV10AntecedentIndex;
  failures.push(...validateCalendarLiveV10AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V10_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V10_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV10SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v10-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV10ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v10 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV10SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v10-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V10_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV10Preflight(
    process.cwd(),
    verifyCalendarLiveV10Authorization(),
    attestationPath,
