...
export const INPUT_LIVE_V70_VERIFIER_VERSION = "input-live-v70-verifier-v1";

export function buildInputLiveV70RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV70RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV70RawNode[];
}

export interface InputLiveV70NormalizedScene {
...
export interface InputLiveV70SceneProof {
...
export function normalizeInputLiveV70Scene(
  root: InputLiveV70RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV70NormalizedScene {
  const canonicalBindings: InputLiveV70NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV70RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV70Scene<Instance>(
  normalized: InputLiveV70NormalizedScene,
...
export function compileInputLiveV70ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV70PreCaptureGates(payload: unknown): void {
