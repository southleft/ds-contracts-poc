...
export const INPUT_LIVE_V60_VERIFIER_VERSION = "input-live-v60-verifier-v1";

export function buildInputLiveV60RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV60RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV60RawNode[];
}

export interface InputLiveV60NormalizedScene {
...
export interface InputLiveV60SceneProof {
...
export function normalizeInputLiveV60Scene(
  root: InputLiveV60RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV60NormalizedScene {
  const canonicalBindings: InputLiveV60NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV60RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV60Scene<Instance>(
  normalized: InputLiveV60NormalizedScene,
...
export function compileInputLiveV60ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV60PreCaptureGates(payload: unknown): void {
