/**
 * Table live v5 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V5_FIXED_VERIFIER_VERSION = "table-live-v5-fixed-point-v1";
export const TABLE_LIVE_V5_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV5FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV5FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV5FixedSourceProbe {
...
export interface TableLiveV5FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV5FixedFixedPoint;
}

export interface TableLiveV5FixedHardGateInput {
  thresholds: TableLiveV5FixedThresholds;
  sourceProbes: TableLiveV5FixedSourceProbe[];
  cells: TableLiveV5FixedCellValidation[];
  sceneProofs: TableLiveV5FixedSceneProof[];
...
export interface TableLiveV5FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V5_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV5FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV5FixedHardGates(
  input: TableLiveV5FixedHardGateInput,
): TableLiveV5FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV5FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  if (input.visualRows.length !== 0)
    failures.push("table live v5 must not invent a visual comparison");
  ...
  return {
    verifierVersion: TABLE_LIVE_V5_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV5FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV5FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
