...
import {
  verifyCalendarLiveV25FixedSceneFixedPoint,
  type CalendarLiveV25FixedFixedPoint,
} from "./calendar-live-v25-fixed-point.js";

export const CALENDAR_LIVE_V25_VERIFIER_VERSION = "calendar-live-v25-verifier-v1";

export function buildCalendarLiveV25RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV25RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV25RawNode[];
}

export interface CalendarLiveV25NormalizedScene {
...
export interface CalendarLiveV25SceneProof {
...
export function normalizeCalendarLiveV25Scene(
  root: CalendarLiveV25RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV25NormalizedScene {
  const canonicalBindings: CalendarLiveV25NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV25RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV25Scene<Instance>(
  normalized: CalendarLiveV25NormalizedScene,
...
): CalendarLiveV25SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV25FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV25ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV25PreCaptureGates(payload: unknown): void {
