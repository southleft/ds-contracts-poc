/**
 * Calendar live v40 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V40_FIXED_VERIFIER_VERSION = "calendar-live-v40-fixed-point-v1";
export const CALENDAR_LIVE_V40_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV40FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV40FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV40FixedSourceProbe {
...
export interface CalendarLiveV40FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV40FixedFixedPoint;
}

export interface CalendarLiveV40FixedHardGateInput {
  thresholds: CalendarLiveV40FixedThresholds;
  sourceProbes: CalendarLiveV40FixedSourceProbe[];
  cells: CalendarLiveV40FixedCellValidation[];
  sceneProofs: CalendarLiveV40FixedSceneProof[];
...
export interface CalendarLiveV40FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V40_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV40FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV40FixedHardGates(
  input: CalendarLiveV40FixedHardGateInput,
): CalendarLiveV40FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV40FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V40_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV40FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV40FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV40FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
