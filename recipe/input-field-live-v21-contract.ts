...
} from "./input-field-live-v3-verifier-v21.js";
import {
  buildInputLiveV21RawPropertyRuntime,
  normalizeInputLiveV21Scene,
  type InputLiveV21RawNode,
} from "./input-field-live-v21-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v21.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v21.js";
...
export const INPUT_LIVE_V21_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V21_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V21_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V21_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V21_VARIANT_COUNT = 256;
export const INPUT_LIVE_V21_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V21_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV21SourceId = (typeof INPUT_LIVE_V21_SOURCE_IDS)[number];

export interface InputLiveV21SourceIdentity {
  source: InputLiveV21SourceId;
...
export interface InputLiveV21WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV21ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV21SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV21RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV21RootProof {
  source: InputLiveV21SourceId;
  adapterIdentity: string;
...
export interface InputLiveV21ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV21SourceId;
    adapterIdentity: string;
...
export interface InputLiveV21CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV21SourceId;
  adapterIdentity: string;
...
export interface InputLiveV21CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV21SourceId;
  strata: InputLiveV21CaptureCell["strata"];
...
export interface InputLiveV21CleanupPayload {
...
export function validateInputLiveV21WriterPayload(
  value: unknown,
): InputLiveV21WriterOwnership {
...
const identityPlan = (source: InputLiveV21SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV21ExtractProgram(
  writer: InputLiveV21WriterOwnership,
  sources: readonly InputLiveV21SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V21_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V21_NAMESPACE)}
${buildInputLiveV21RawPropertyRuntime()}
...
export function validateInputLiveV21ExtractPayload(
  value: unknown,
  writer: InputLiveV21WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V21_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV21ExtractPayload;
}

export function proveInputLiveV21Roots<Instance>(
  extract: InputLiveV21ExtractPayload,
  sources: readonly (InputLiveV21SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV21Scene(
      root.scene,
...
export function assertInputLiveV21RootProofs(
  proofs: readonly InputLiveV21RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV21ProbeProgram(
  writer: InputLiveV21WriterOwnership,
  sources: readonly InputLiveV21SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V21_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV21ProbePayload(
  value: unknown,
  writer: InputLiveV21WriterOwnership,
): InputLiveV21ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV21ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V21_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V21_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV21CaptureManifest(
  cells: readonly InputLiveV21CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V21_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V21_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V21_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V21_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V21_SOURCE_IDS.some(
...
export const inputLiveV21CaptureManifestSha256 = (
  cells: readonly InputLiveV21CaptureCell[],
): string => {
  validateInputLiveV21CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV21CaptureProgram(
  writer: InputLiveV21WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V21_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V21_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V21_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV21CapturePayload(
  value: unknown,
  cell: InputLiveV21CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV21CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V21_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V21_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV21CaptureResponses(
  manifest: readonly InputLiveV21CaptureCell[],
  responses: readonly InputLiveV21CapturePayload[],
): void {
  validateInputLiveV21CaptureManifest(manifest);
  if (
...
export interface InputLiveV21ObjectiveReport {
  artifactVersion: "input-live-v21-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV21Objective(
  manifest: readonly InputLiveV21CaptureCell[],
  responses: readonly InputLiveV21CapturePayload[],
  readReference: (cell: InputLiveV21CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV21CaptureCell) => Buffer,
): InputLiveV21ObjectiveReport {
  assertInputLiveV21CaptureResponses(manifest, responses);
...
export function buildInputLiveV21CleanupProgram(
  writer: InputLiveV21WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V21_NAMESPACE,
...
export function validateInputLiveV21CleanupPayload(
  value: unknown,
  writer: InputLiveV21WriterOwnership,
): InputLiveV21CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV21CleanupPayload;
...
export const INPUT_LIVE_V21_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV21WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
