...
import { buildCalendarLiveV40CleanupRuntime } from "./calendar-live-v40-cleanup.js";
import { type CalendarLiveV40FixedFixedPoint } from "./calendar-live-v40-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV40RestoreProgram,
  validateCalendarLiveV40RestorePayload,
  type CalendarLiveV40RestorePayload,
} from "./calendar-live-v40-restore.js";
import {
  buildCalendarLiveV40RawPropertyRuntime,
  normalizeCalendarLiveV40Scene,
  type CalendarLiveV40RawNode,
} from "./calendar-live-v40-verifier.js";
...
export const CALENDAR_LIVE_V40_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V40_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V40_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V40_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V40_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V40_SET_COUNT = 3;
export const CALENDAR_LIVE_V40_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V40_HOST_PHASES = 3;
export const CALENDAR_LIVE_V40_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V40_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V40_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V40_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV40SourceId = (typeof CALENDAR_LIVE_V40_SOURCE_IDS)[number];
export type CalendarLiveV40Kind = "calendar" | "week" | "day";

export interface CalendarLiveV40SourceIdentity {
  source: CalendarLiveV40SourceId;
...
export interface CalendarLiveV40WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV40ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV40SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV40RawNode;
    weekScene: CalendarLiveV40RawNode;
    dayScene: CalendarLiveV40RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV40RootProof {
  source: CalendarLiveV40SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV40FixedFixedPoint;
}

export interface CalendarLiveV40ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV40SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV40CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV40SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV40CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV40SourceId;
  frameWidth: number;
...
export interface CalendarLiveV40CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v40 ${label} must be an object`);
...
export function validateCalendarLiveV40WriterPayload(
  value: unknown,
): CalendarLiveV40WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v40") ||
    ...
    result.namespace !== CALENDAR_LIVE_V40_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v40 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v40 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v40 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV40SourceIdentity,
  kind: CalendarLiveV40Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV40ExtractProgram(
  writer: CalendarLiveV40WriterOwnership,
  sources: readonly CalendarLiveV40SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v40 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V40_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V40_NAMESPACE)}
${buildCalendarLiveV40RawPropertyRuntime()}
...
export function validateCalendarLiveV40ExtractPayload(
  value: unknown,
  writer: CalendarLiveV40WriterOwnership,
): CalendarLiveV40ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v40 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V40_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v40 extract schema/one-root mismatch");
  return payload as CalendarLiveV40ExtractPayload;
}

export function proveCalendarLiveV40Roots<Instance>(
  extract: CalendarLiveV40ExtractPayload,
  sources: readonly (CalendarLiveV40SourceIdentity & {
...
): CalendarLiveV40RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v40 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v40 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV40Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV40Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV40Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV40FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v40 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v40 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v40 compile lost owned sets");
...
export function assertCalendarLiveV40RootProofs(
  proofs: readonly CalendarLiveV40RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v40 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v40 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV40ProbeProgram(
  writer: CalendarLiveV40WriterOwnership,
  sources: readonly CalendarLiveV40SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V40_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV40ProbePayload(
  value: unknown,
  writer: CalendarLiveV40WriterOwnership,
): CalendarLiveV40ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV40ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V40_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V40_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v40 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV40CaptureManifest(
  cells: readonly CalendarLiveV40CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V40_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V40_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V40_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V40_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v40 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV40CaptureCell[],
): string => {
  validateCalendarLiveV40CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV40CaptureProgram(
  writer: CalendarLiveV40WriterOwnership,
  cell: CalendarLiveV40CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V40_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v40 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v40 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V40_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV40CapturePayload(
  value: unknown,
  cell: CalendarLiveV40CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV40CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV40CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V40_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V40_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v40 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV40CaptureResponses(
  manifest: readonly CalendarLiveV40CaptureCell[],
  responses: readonly CalendarLiveV40CapturePayload[],
): void {
  validateCalendarLiveV40CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v40 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV40ObjectiveReport {
  artifactVersion: "calendar-live-v40-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV40Objective(
  manifest: readonly CalendarLiveV40CaptureCell[],
  responses: readonly CalendarLiveV40CapturePayload[],
): CalendarLiveV40ObjectiveReport {
  assertCalendarLiveV40CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v40-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV40CleanupProgram(
  writer: CalendarLiveV40WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV40CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V40_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v40 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV40CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV40WriterOwnership,
): CalendarLiveV40CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV40CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v40 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V40_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV40WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
