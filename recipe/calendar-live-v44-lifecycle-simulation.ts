...
import { calendarLiveV1Sha256 } from "./calendar-live-v44-broker.js";

export interface CalendarLiveV44LifecycleSimulationReport {
  artifactVersion: "calendar-live-v44-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV44AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV44Lifecycle(): Promise<CalendarLiveV44LifecycleSimulationReport> {
  await buildCalendarLiveV44Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V44_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV44AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v44-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V44_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V44_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV44History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV44History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV44AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V44_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V44_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV44History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV44History(authorized, "pending");
    ...
    await buildCalendarLiveV44Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V44_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV44AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV44Lifecycle(), null, 2)}\n`,
  );
