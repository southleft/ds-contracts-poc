...
import {
  verifyComboboxLiveV26FixedSceneFixedPoint,
  type ComboboxLiveV26FixedFixedPoint,
} from "./combobox-live-v26-fixed-point.js";

export const COMBOBOX_LIVE_V26_VERIFIER_VERSION = "combobox-live-v26-verifier-v1";

export function buildComboboxLiveV26RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV26RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV26RawNode[];
}

export interface ComboboxLiveV26NormalizedScene {
...
export interface ComboboxLiveV26SceneProof {
...
export function normalizeComboboxLiveV26Scene(
  root: ComboboxLiveV26RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV26NormalizedScene {
  const canonicalBindings: ComboboxLiveV26NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV26RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV26Scene<Instance>(
  normalized: ComboboxLiveV26NormalizedScene,
...
): ComboboxLiveV26SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV26FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV26ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV26PreCaptureGates(payload: unknown): void {
