...
import {
  acceptInputLiveV24Response,
  createInputLiveV24Transaction,
  inputLiveV24ReceiptPath,
  inputLiveV24RequestPath,
  inputLiveV24ResponsePath,
  inputLiveV24Sha256,
  issueInputLiveV24Request,
  persistInputLiveV24RawResponse,
  persistInputLiveV24TechnicalGates,
  readInputLiveV24AcceptedRawPayload,
  type InputLiveV24Request,
  type InputLiveV24TransactionAuthorization,
} from "./input-field-live-v24-broker.js";
import {
  INPUT_LIVE_V24_CAPTURE_COUNT,
  assertInputLiveV24CaptureResponses,
  assertInputLiveV24RootProofs,
  buildInputLiveV24CaptureProgram,
  buildInputLiveV24CleanupProgram,
  buildInputLiveV24ExtractProgram,
  buildInputLiveV24ProbeProgram,
  buildInputLiveV24RestoreProgram,
  validateInputLiveV24RestorePayload,
  evaluateInputLiveV24Objective,
  inputLiveV24CaptureManifestSha256,
  proveInputLiveV24Roots,
  validateInputLiveV24CaptureManifest,
  validateInputLiveV24CapturePayload,
  validateInputLiveV24CleanupPayload,
  validateInputLiveV24ExtractPayload,
  validateInputLiveV24ProbePayload,
  validateInputLiveV24WriterPayload,
  type InputLiveV24CaptureCell,
  type InputLiveV24CapturePayload,
  type InputLiveV24RootProof,
  type InputLiveV24SourceIdentity,
  type InputLiveV24WriterOwnership,
} from "./input-field-live-v24-contract.js";
import {
  INPUT_LIVE_V24_CAPTURE_MANIFEST_PATH,
  INPUT_LIVE_V24_PLAN_PATH,
  type InputLiveV24ProofPlan,
...
import type { ExpectedScenePlan } from "./scene-readback-v24.js";

export const INPUT_LIVE_V24_RUNNER_VERSION = "input-live-v24-runner-v1";

interface InputLiveV24JournalEntry {
  index: number;
  event: string;
...
interface InputLiveV24OrchestratorState {
  artifactVersion: "input-live-v24-orchestrator-state-v1";
  ...
  journal: InputLiveV24JournalEntry[];
}

interface LoadedProofInputs {
  plan: InputLiveV24ProofPlan;
  planSha256: string;
  captures: InputLiveV24CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    InputLiveV24SourceIdentity & {
...
export interface InputLiveV24NextAction {
...
const writeState = (
  directory: string,
  state: InputLiveV24OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, INPUT_LIVE_V24_PLAN_PATH);
  const capturePath = path.join(root, INPUT_LIVE_V24_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as InputLiveV24ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as InputLiveV24CaptureCell[];
  validateInputLiveV24CaptureManifest(captures);
  if (
    plan.captureManifest.sha256 !== inputLiveV24Sha256(captureBytes) ||
    captureArtifact.cellsSha256 !== inputLiveV24CaptureManifestSha256(captures)
  )
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (
      inputLiveV24Sha256(expectedBytes) !== source.expectedScenePlan.sha256 ||
      inputLiveV24Sha256(uncompressed) !==
  ...
  return {
    plan,
    planSha256: inputLiveV24Sha256(planBytes),
    captures,
    captureManifestSha256: inputLiveV24Sha256(captureBytes),
    sources,
...
const appendJournal = (
  ...
  const payloadSha256 = inputLiveV24Sha256(canonicalJson(payload));
  const body = {
  ...
  const entry = {
    ...body,
    entrySha256: inputLiveV24Sha256(canonicalJson(body)),
  };
  writeNew(
    path.join(
      directory,
...
const writerOwnership = (directory: string): InputLiveV24WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): InputLiveV24RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): InputLiveV24Request =>
  readJson<InputLiveV24Request>(file);

export class InputLiveV24Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueInputLiveV24Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: InputLiveV24OrchestratorState = {
      artifactVersion: "input-live-v24-orchestrator-state-v1",
      transactionDirectory: options.transactionDirectory,
      activeRequest: inputLiveV24RequestPath(
        options.transactionDirectory,
        "writer",
      ),
      cleanupRequest: null,
    ...
    return new InputLiveV24Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<InputLiveV24OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new InputLiveV24Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): InputLiveV24NextAction {
    const cleanup =
      this.#state.cleanupRequest &&
      !this.#state.cleanupAccepted &&
      !existsSync(inputLiveV24ReceiptPath(this.#directory, "cleanup"))
        ? this.#state.cleanupRequest
        : null;
  ...
  ingestAndAdvance(rawResponse: string): InputLiveV24NextAction {
  ...
  ingestCleanup(rawResponse: string): InputLiveV24NextAction {
    ...
    persistInputLiveV24RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptInputLiveV24Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV24CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      existsSync(inputLiveV24ReceiptPath(this.#directory, "writer")) &&
      !existsSync(path.join(this.#directory, OWNERSHIP_FILE))
    ) {
      const raw = readInputLiveV24AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateInputLiveV24WriterPayload(raw),
      );
    }
    if (existsSync(inputLiveV24RequestPath(this.#directory, "cleanup")))
      this.#state.cleanupRequest = inputLiveV24RequestPath(
        this.#directory,
        "cleanup",
      );
    if (
      existsSync(path.join(this.#directory, OWNERSHIP_FILE)) &&
      !existsSync(inputLiveV24RequestPath(this.#directory, "cleanup"))
    )
      this.#issueCleanupAndRestore(false);
    else if (
      existsSync(inputLiveV24RequestPath(this.#directory, "cleanup")) &&
      !existsSync(inputLiveV24RequestPath(this.#directory, "restore"))
    )
      this.#issueRestore(false);
    if (
      existsSync(inputLiveV24RequestPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV24ReceiptPath(this.#directory, "restore"))
    )
      this.#state.activeRequest = inputLiveV24RequestPath(
        this.#directory,
        "restore",
      );
    if (
      existsSync(inputLiveV24ReceiptPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV24RequestPath(this.#directory, "extract"))
    ) {
      const ownership = writerOwnership(this.#directory);
      const extract = issueInputLiveV24Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildInputLiveV24ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV24RequestPath(
        this.#directory,
        "extract",
    ...
    if (existsSync(inputLiveV24ReceiptPath(this.#directory, "cleanup")))
      this.#state.cleanupAccepted = true;
    this.#persist();
  }

  #acceptActive(request: InputLiveV24Request): void {
    if (request.phase === "writer") {
      const accepted = acceptInputLiveV24Response(this.#directory, "writer", {
        validate: validateInputLiveV24WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptInputLiveV24Response(this.#directory, "restore", {
        validate: (payload) =>
          validateInputLiveV24RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptInputLiveV24Response(this.#directory, "extract", {
        validate: (payload) =>
          validateInputLiveV24ExtractPayload(payload, ownership),
      });
      const proofs = proveInputLiveV24Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseInputFieldRecipe,
        compileInputFieldRecipe,
      );
      assertInputLiveV24RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "input-live-v24-host-root-proofs-v1",
      ...
      appendJournal(
        ...
        request.requestId,
        { proofsSha256: inputLiveV24Sha256(canonicalJson(proofs)) },
      );
      const next = issueInputLiveV24Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildInputLiveV24ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV24RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptInputLiveV24Response(this.#directory, "probe", {
        validate: (payload) =>
          validateInputLiveV24ProbePayload(payload, ownership),
      });
      persistInputLiveV24TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      const accepted = acceptInputLiveV24Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateInputLiveV24CapturePayload(payload, cell, rawBytes),
      });
      const png = Buffer.from(accepted.payload.pngBase64, "base64");
      const capturePath = path.join(
        this.#directory,
        "captures",
        `${String(cell.index).padStart(3, "0")}-${inputLiveV24Sha256(cell.cellKey).slice(0, 20)}.png`,
      );
      ...
      if (next < INPUT_LIVE_V24_CAPTURE_COUNT) this.#issueCapture(next);
      else {
          ...
          pngBase64: Buffer.from(
            readFileSync(
              path.join(
                this.#directory,
                "captures",
                `${String(response.index).padStart(3, "0")}-${inputLiveV24Sha256(response.cellKey).slice(0, 20)}.png`,
              ),
            ),
          ).toString("base64"),
        }));
        assertInputLiveV24CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateInputLiveV24Objective(
        ...
        appendJournal(
          ...
          request.requestId,
          {
            reportSha256: inputLiveV24Sha256(canonicalJson(objective)),
  ...
  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueInputLiveV24Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildInputLiveV24CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = inputLiveV24RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueInputLiveV24Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildInputLiveV24RestoreProgram(ownership),
    );
    this.#state.activeRequest = inputLiveV24RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueInputLiveV24Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildInputLiveV24CaptureProgram(
  ...
  #readCaptureResponseIndex(): InputLiveV24CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: InputLiveV24CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedInputLiveV24Authorization = (
  privateKey: KeyObject,
): InputLiveV24TransactionAuthorization => ({
  mode: "simulated",
  ...
  signingPublicKeySha256: inputLiveV24Sha256(
...
export const generateInputLiveV24SimulationKey = (): KeyObject =>
