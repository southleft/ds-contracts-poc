...
import { CALENDAR_LIVE_V21_INDEX_PATH } from "./build-calendar-live-proof-v21.js";
import {
...
import {
  buildCalendarLiveV21AuthorizationArtifact,
  validateCalendarLiveV21AntecedentIndex,
  validateCalendarLiveV21AuthorizationArtifact,
  validateCalendarLiveV21History,
  validateCalendarLiveV21UnitTestSource,
  type CalendarLiveV21AntecedentIndex,
  type CalendarLiveV21AuthorizationArtifact,
  type CalendarLiveV21AuthorizationProof,
  type CalendarLiveV21HistoryState,
} from "./calendar-live-v21-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV21ControlFlowSource,
  validateCalendarLiveV21SecurityAttestation,
  type CalendarLiveV21SecurityAttestation,
} from "./calendar-live-v21-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V21_INDEX_PATH);
const index = (): CalendarLiveV21AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV21AntecedentIndex;

const artifact = (): CalendarLiveV21AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV21AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV21HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV21HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV21AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v21/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV21AuthorizationArtifact,
): CalendarLiveV21SecurityAttestation => {
  const body: Omit<CalendarLiveV21SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V21_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV21History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV21History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV21History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV21History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV21History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV21UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v21-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV21UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV21AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV21AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV21AuthorizationArtifact(
        planted as CalendarLiveV21AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV21SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV21SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV21SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v21-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV21ControlFlowSource(source), []);
});
