...
import {
  acceptComboboxLiveV41Response,
  createComboboxLiveV41Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV41Request,
  persistComboboxLiveV41RawResponse,
  persistComboboxLiveV41TechnicalGates,
  readComboboxLiveV41AcceptedRawPayload,
  type ComboboxLiveV41Request,
  type ComboboxLiveV41TransactionAuthorization,
} from "./combobox-live-v41-broker.js";
import {
  COMBOBOX_LIVE_V41_CAPTURE_COUNT,
  assertComboboxLiveV41CaptureResponses,
  ...
  evaluateComboboxLiveV41Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV41Roots,
  validateComboboxLiveV41CaptureManifest,
  validateComboboxLiveV41CapturePayload,
  validateComboboxLiveV41CleanupPayload,
  validateComboboxLiveV41ExtractPayload,
  validateComboboxLiveV41ProbePayload,
  validateComboboxLiveV41WriterPayload,
  type ComboboxLiveV41CaptureCell,
  type ComboboxLiveV41CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V41_PLAN_PATH,
  type ComboboxLiveV41ProofPlan,
} from "./build-combobox-live-proof-v41.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v41.js";

export const COMBOBOX_LIVE_V41_RUNNER_VERSION = "combobox-live-v41-runner-v1";

interface ComboboxLiveV41JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV41OrchestratorState {
  artifactVersion: "combobox-live-v41-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV41JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV41ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV41CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV41SourceIdentity & {
...
export interface ComboboxLiveV41NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV41OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v41 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V41_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V41_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV41ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV41CaptureCell[];
  validateComboboxLiveV41CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v41 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v41 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV41WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV41RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV41Request =>
  readJson<ComboboxLiveV41Request>(file);

export class ComboboxLiveV41Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV41Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV41OrchestratorState = {
      artifactVersion: "combobox-live-v41-orchestrator-state-v1",
    ...
    return new ComboboxLiveV41Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV41OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV41Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV41NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV41NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v41 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV41NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v41 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV41RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV41Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV41CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV41AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV41WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV41Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV41ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV41Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV41Response(this.#directory, "writer", {
        validate: validateComboboxLiveV41WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV41Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV41RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV41Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV41ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV41Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV41RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v41-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV41Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV41ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV41Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV41ProbePayload(payload, ownership),
      });
      persistComboboxLiveV41TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v41 capture cell missing");
      const accepted = acceptComboboxLiveV41Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV41CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V41_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV41CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV41Objective(
    ...
    throw new Error(`Combobox live v41 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV41Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV41CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV41Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV41RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV41Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV41CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV41CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV41CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV41Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV41TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV41SimulationKey = (): KeyObject =>
