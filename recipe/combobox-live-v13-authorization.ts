...
export type ComboboxLiveV13HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV13AuthorizationArtifact {
  artifactVersion: "combobox-live-v13-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V13_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV13AntecedentIndex {
...
export interface ComboboxLiveV13HistoryState {
  index?: ComboboxLiveV13AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV13AuthorizationArtifact;
...
export interface ComboboxLiveV13AuthorizationProof extends ComboboxLiveV13TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V13_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V13_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V13_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V13_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v13-authorization") ||
  artifactPath.includes("combobox-live-v13-preflight") ||
  artifactPath.includes("combobox-live-v13-authorized") ||
  artifactPath.includes("create-combobox-live-v13-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v13-status") ||
...
export function validateComboboxLiveV13AntecedentIndex(
  index: ComboboxLiveV13AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v13-antecedent-index-v1" ||
...
export function validateComboboxLiveV13UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV13History\(\s*["']/.test(source) ||
    /readComboboxLiveV13HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV13AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV13AuthorizationArtifact {
  ...
  ) as ComboboxLiveV13AntecedentIndex;
  const indexFailures = validateComboboxLiveV13AntecedentIndex(index);
...
export function validateComboboxLiveV13AuthorizationArtifact(
  artifact: ComboboxLiveV13AuthorizationArtifact | undefined,
  index: ComboboxLiveV13AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v13-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v13" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V13_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V13_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V13_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V13_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V13_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V13_SOURCE_ROOTS ||
  ...
  if (
    ...
    failures.push(
      "v13 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
...
export function validateComboboxLiveV13History(
  state: ComboboxLiveV13HistoryState,
  expected: ComboboxLiveV13HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV13AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV13AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV13HistoryState(
  root = process.cwd(),
): ComboboxLiveV13HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V13_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV13AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V13_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V13_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V13_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV13History(
  expected: ComboboxLiveV13HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV13HistoryState {
  const state = readComboboxLiveV13HistoryState(root);
  const failures = validateComboboxLiveV13History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v13 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV13Authorization(
  root = process.cwd(),
): ComboboxLiveV13AuthorizationProof {
  const state = verifyComboboxLiveV13History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V13_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v13/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v13.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V13_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V13_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V13_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV13HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV13History(expected);
  process.stdout.write(
    `Combobox live v13 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
