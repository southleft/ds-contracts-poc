...
import { verifyTableLiveV2Authorization } from "./table-live-v2-authorization.js";
import { TABLE_LIVE_V2_TARGET } from "./table-live-v2-broker.js";
import {
  TABLE_LIVE_V2_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV2Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v2-session";
...
const attestation: TableLiveV2SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV2SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v2 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V2_SECURITY_ATTESTATION_DEFAULT_PATH,
);
