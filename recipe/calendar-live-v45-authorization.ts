...
export type CalendarLiveV45HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV45AuthorizationArtifact {
  artifactVersion: "calendar-live-v45-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V45_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV45AntecedentIndex {
...
export interface CalendarLiveV45HistoryState {
  index?: CalendarLiveV45AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV45AuthorizationArtifact;
...
export interface CalendarLiveV45AuthorizationProof
  extends CalendarLiveV45TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V45_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V45_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V45_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V45_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v45-authorization") ||
  artifactPath.includes("calendar-live-v45-preflight") ||
  artifactPath.includes("calendar-live-v45-authorized") ||
  artifactPath.includes("create-calendar-live-v45-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v45-status") ||
...
export function validateCalendarLiveV45AntecedentIndex(
  index: CalendarLiveV45AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v45-antecedent-index-v1" ||
...
export function validateCalendarLiveV45UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV45History\(\s*["']/.test(source) ||
    /readCalendarLiveV45HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV45AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV45AuthorizationArtifact {
  ...
  ) as CalendarLiveV45AntecedentIndex;
  const indexFailures = validateCalendarLiveV45AntecedentIndex(index);
...
export function validateCalendarLiveV45AuthorizationArtifact(
  artifact: CalendarLiveV45AuthorizationArtifact | undefined,
  index: CalendarLiveV45AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v45-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v45" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V45_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V45_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V45_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V45_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V45_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V45_SOURCE_ROOTS ||
  ...
  if (
    ...
    artifact?.execution?.comboboxLiveV45AuthorizationReusable !== false ||
...
export function validateCalendarLiveV45History(
  state: CalendarLiveV45HistoryState,
  expected: CalendarLiveV45HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV45AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV45AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV45HistoryState(
  root = process.cwd(),
): CalendarLiveV45HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V45_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV45AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V45_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V45_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V45_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV45History(
  expected: CalendarLiveV45HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV45HistoryState {
  const state = readCalendarLiveV45HistoryState(root);
  const failures = validateCalendarLiveV45History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v45 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV45Authorization(
  root = process.cwd(),
): CalendarLiveV45AuthorizationProof {
  const state = verifyCalendarLiveV45History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V45_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v45/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v45.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V45_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V45_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V45_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV45HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV45History(expected);
  process.stdout.write(
    `Calendar live v45 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
