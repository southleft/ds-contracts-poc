...
import { COMBOBOX_LIVE_V12_INDEX_PATH } from "./build-combobox-live-proof-v12.js";
import {
  COMBOBOX_LIVE_V12_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V12_TARGET,
} from "./combobox-live-v12-broker.js";
import {
  buildComboboxLiveV12AuthorizationArtifact,
  validateComboboxLiveV12AntecedentIndex,
  validateComboboxLiveV12AuthorizationArtifact,
  validateComboboxLiveV12History,
  validateComboboxLiveV12UnitTestSource,
  type ComboboxLiveV12AntecedentIndex,
  type ComboboxLiveV12AuthorizationArtifact,
  type ComboboxLiveV12AuthorizationProof,
  type ComboboxLiveV12HistoryState,
} from "./combobox-live-v12-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV12ControlFlowSource,
  validateComboboxLiveV12SecurityAttestation,
  type ComboboxLiveV12SecurityAttestation,
} from "./combobox-live-v12-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v12-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V12_INDEX_PATH);
const index = (): ComboboxLiveV12AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV12AntecedentIndex;

const artifact = (): ComboboxLiveV12AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV12AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV12HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV12HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV12AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v12/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV12AuthorizationArtifact,
): ComboboxLiveV12SecurityAttestation => {
  const body: Omit<ComboboxLiveV12SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V12_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV12History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV12History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV12History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV12History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV12History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV12UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV12UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV12AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV12AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV12AuthorizationArtifact(
        planted as ComboboxLiveV12AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV12SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV12SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV12SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV12ControlFlowSource(source), []);
});
