...
import { buildTableLiveV12CleanupRuntime } from "./table-live-v12-cleanup.js";
import { type TableLiveV12FixedFixedPoint } from "./table-live-v12-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV12RestoreProgram,
  validateTableLiveV12RestorePayload,
  type TableLiveV12RestorePayload,
} from "./table-live-v12-restore.js";
import {
  buildTableLiveV12RawPropertyRuntime,
  normalizeTableLiveV12Scene,
  type TableLiveV12RawNode,
} from "./table-live-v12-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v12.js";
import {
...
export const TABLE_LIVE_V12_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V12_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V12_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V12_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V12_VARIANT_COUNT = 20;
export const TABLE_LIVE_V12_SET_COUNT = 6;
export const TABLE_LIVE_V12_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V12_HOST_PHASES = 3;
export const TABLE_LIVE_V12_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V12_RESTORE_COUNT = 8;
export const TABLE_LIVE_V12_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V12_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV12SourceId = (typeof TABLE_LIVE_V12_SOURCE_IDS)[number];
export type TableLiveV12Kind = "table" | "row" | "cell";

export interface TableLiveV12SourceIdentity {
  source: TableLiveV12SourceId;
...
export interface TableLiveV12WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV12ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV12SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV12RawNode;
    rowScene: TableLiveV12RawNode;
    cellScene: TableLiveV12RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV12RootProof {
  source: TableLiveV12SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV12FixedFixedPoint;
}

export interface TableLiveV12ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV12SourceId;
    adapterIdentity: string;
...
export interface TableLiveV12CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV12SourceId;
  adapterIdentity: string;
...
export interface TableLiveV12CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV12SourceId;
  frameWidth: number;
...
export interface TableLiveV12CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v12 ${label} must be an object`);
...
export function validateTableLiveV12WriterPayload(
  value: unknown,
): TableLiveV12WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V12_NAMESPACE ||
    ...
    throw new TypeError("Table live v12 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v12 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v12 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV12SourceIdentity,
  kind: TableLiveV12Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV12ExtractProgram(
  writer: TableLiveV12WriterOwnership,
  sources: readonly TableLiveV12SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v12 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V12_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V12_NAMESPACE)}
${buildTableLiveV12RawPropertyRuntime()}
...
export function validateTableLiveV12ExtractPayload(
  value: unknown,
  writer: TableLiveV12WriterOwnership,
): TableLiveV12ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v12 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V12_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v12 extract schema/two-root mismatch");
  return payload as TableLiveV12ExtractPayload;
}

export function proveTableLiveV12Roots<Instance>(
  extract: TableLiveV12ExtractPayload,
  sources: readonly (TableLiveV12SourceIdentity & {
...
): TableLiveV12RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v12 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v12 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV12Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV12Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV12Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV12FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v12 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v12 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v12 compile lost owned sets");
...
export function assertTableLiveV12RootProofs(
  proofs: readonly TableLiveV12RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v12 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v12 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV12ProbeProgram(
  writer: TableLiveV12WriterOwnership,
  sources: readonly TableLiveV12SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V12_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
for(const row of byAdapter.values()){
 ...
 const cell0=propertyKey(rowInstance,"Cell 0"),cell0Before=cell0&&rowInstance.componentProperties[cell0].value;let textPropertiesRestored=false;if(cell0){rowInstance.setProperties({[cell0]:"Table v12 deterministic probe"});const changed=nodes(rowInstance).some(node=>node.type==="TEXT"&&node.characters==="Table v12 deterministic probe");rowInstance.setProperties({[cell0]:cell0Before});textPropertiesRestored=changed&&JSON.stringify(original)===JSON.stringify(plain(instance));}
 rowInstance.remove();
...
export function validateTableLiveV12ProbePayload(
  value: unknown,
  writer: TableLiveV12WriterOwnership,
): TableLiveV12ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV12ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V12_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v12 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV12CaptureManifest(
  cells: readonly TableLiveV12CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V12_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V12_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V12_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V12_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v12 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV12CaptureCell[],
): string => {
  validateTableLiveV12CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV12CaptureProgram(
  writer: TableLiveV12WriterOwnership,
  cell: TableLiveV12CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V12_CAPTURE_COUNT
  )
    throw new TypeError("Table live v12 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v12 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
const frame=figma.createFrame();frame.name="Table v12 ephemeral / "+cell.cellKey;page.appendChild(frame);
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V12_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV12CapturePayload(
  value: unknown,
  cell: TableLiveV12CaptureCell,
  rawResponseBytes: number,
): TableLiveV12CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV12CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V12_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V12_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v12 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV12CaptureResponses(
  manifest: readonly TableLiveV12CaptureCell[],
  responses: readonly TableLiveV12CapturePayload[],
): void {
  validateTableLiveV12CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v12 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV12ObjectiveReport {
  artifactVersion: "table-live-v12-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV12Objective(
  manifest: readonly TableLiveV12CaptureCell[],
  responses: readonly TableLiveV12CapturePayload[],
): TableLiveV12ObjectiveReport {
  assertTableLiveV12CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v12-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV12CleanupProgram(
  writer: TableLiveV12WriterOwnership,
): string {
  const baseProgram = buildTableLiveV12CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V12_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v12 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV12CleanupPayload(
  payload: unknown,
  writer: TableLiveV12WriterOwnership,
): TableLiveV12CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV12CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v12 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V12_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV12WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
