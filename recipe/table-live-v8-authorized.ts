...
import { TABLE_LIVE_V8_AUTHORIZATION_PATH } from "./build-table-live-proof-v8.js";
import { verifyTableLiveV8Authorization } from "./table-live-v8-authorization.js";
import { runTableLiveV8Preflight } from "./table-live-v8-preflight.js";
import { TableLiveV8Orchestrator } from "./run-table-live-v8.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v8 requires exact authorization artifact ${TABLE_LIVE_V8_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v8-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV8Authorization(root);
    runTableLiveV8Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV8Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV8Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
