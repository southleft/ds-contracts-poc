...
import {
  ...
  verifyCalendarLiveV16Authorization,
  type CalendarLiveV16AntecedentIndex,
  type CalendarLiveV16AuthorizationProof,
} from "./calendar-live-v16-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V16_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v16/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V16_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v16-security-attestation.json";

export interface CalendarLiveV16SecurityAttestation {
...
export interface CalendarLiveV16PreflightReport {
  artifactVersion: "calendar-live-v16-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV16SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV16SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV16AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v16-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V16_TARGET) ||
...
export function validateCalendarLiveV16ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV16Authorization");
  const preflight = source.indexOf("runCalendarLiveV16Preflight");
  const initialize = source.indexOf("CalendarLiveV16Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV16Preflight(
  root: string,
  proof: CalendarLiveV16AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V16_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV16PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V16_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V16_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV16AntecedentIndex;
  failures.push(...validateCalendarLiveV16AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V16_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V16_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v16-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV16ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v16 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV16SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v16-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V16_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV16Preflight(
    process.cwd(),
    verifyCalendarLiveV16Authorization(),
    attestationPath,
