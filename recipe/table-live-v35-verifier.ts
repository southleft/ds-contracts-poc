...
import {
  verifyTableLiveV35FixedSceneFixedPoint,
  type TableLiveV35FixedFixedPoint,
} from "./table-live-v35-fixed-point.js";

export const TABLE_LIVE_V35_VERIFIER_VERSION = "table-live-v35-verifier-v1";

export function buildTableLiveV35RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV35RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV35RawNode[];
}

export interface TableLiveV35NormalizedScene {
...
export interface TableLiveV35SceneProof {
...
export function normalizeTableLiveV35Scene(
  root: TableLiveV35RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV35NormalizedScene {
  const canonicalBindings: TableLiveV35NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV35RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV35Scene<Instance>(
  normalized: TableLiveV35NormalizedScene,
...
): TableLiveV35SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV35FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV35ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV35PreCaptureGates(payload: unknown): void {
