...
import {
  verifyComboboxLiveV27FixedSceneFixedPoint,
  type ComboboxLiveV27FixedFixedPoint,
} from "./combobox-live-v27-fixed-point.js";

export const COMBOBOX_LIVE_V27_VERIFIER_VERSION = "combobox-live-v27-verifier-v1";

export function buildComboboxLiveV27RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV27RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV27RawNode[];
}

export interface ComboboxLiveV27NormalizedScene {
...
export interface ComboboxLiveV27SceneProof {
...
export function normalizeComboboxLiveV27Scene(
  root: ComboboxLiveV27RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV27NormalizedScene {
  const canonicalBindings: ComboboxLiveV27NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV27RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV27Scene<Instance>(
  normalized: ComboboxLiveV27NormalizedScene,
...
): ComboboxLiveV27SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV27FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV27ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV27PreCaptureGates(payload: unknown): void {
