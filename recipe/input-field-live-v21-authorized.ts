...
import { INPUT_LIVE_V21_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v21.js";
import { verifyInputLiveV21Authorization } from "./input-field-live-v21-authorization.js";
import { runInputLiveV21Preflight } from "./input-field-live-v21-preflight.js";
import { InputLiveV21Orchestrator } from "./run-input-field-live-v21.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V21_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v21-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV21Authorization(root);
    runInputLiveV21Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV21Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
