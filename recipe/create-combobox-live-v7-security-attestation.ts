...
import { verifyComboboxLiveV7Authorization } from "./combobox-live-v7-authorization.js";
import { COMBOBOX_LIVE_V7_TARGET } from "./combobox-live-v7-broker.js";
import {
  COMBOBOX_LIVE_V7_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV7Authorization();
...
const attestation: ComboboxLiveV7SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV7SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v7 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V7_SECURITY_ATTESTATION_DEFAULT_PATH,
);
