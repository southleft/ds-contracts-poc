...
export interface InputLiveV67LifecycleSimulationReport {
  artifactVersion: "input-live-v67-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV67AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV67Lifecycle(): Promise<InputLiveV67LifecycleSimulationReport> {
  await buildInputLiveV67Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V67_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV67AntecedentIndex;
  const indexSha256 = inputLiveV67Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v67-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V67_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V67_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV67History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV67History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV67AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V67_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V67_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV67History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV67History(authorized, "pending");
    ...
    await buildInputLiveV67Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V67_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV67AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV67Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV67Lifecycle(), null, 2)}\n`,
  );
