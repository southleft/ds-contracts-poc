...
export interface InputLiveV33LifecycleSimulationReport {
  artifactVersion: "input-live-v33-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV33AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV33Lifecycle(): Promise<InputLiveV33LifecycleSimulationReport> {
  await buildInputLiveV33Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V33_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV33AntecedentIndex;
  const indexSha256 = inputLiveV33Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v33-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V33_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V33_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV33History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV33History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV33AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V33_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V33_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV33History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV33History(authorized, "pending");
    ...
    await buildInputLiveV33Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V33_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV33AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV33Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV33Lifecycle(), null, 2)}\n`,
  );
