...
import { COMBOBOX_LIVE_V33_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v33.js";
import { verifyComboboxLiveV33Authorization } from "./combobox-live-v33-authorization.js";
import { runComboboxLiveV33Preflight } from "./combobox-live-v33-preflight.js";
import { ComboboxLiveV33Orchestrator } from "./run-combobox-live-v33.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v33 requires exact authorization artifact ${COMBOBOX_LIVE_V33_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v33-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV33Authorization(root);
    runComboboxLiveV33Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV33Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV33Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
