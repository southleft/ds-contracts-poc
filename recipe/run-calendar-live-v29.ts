...
import {
  acceptCalendarLiveV29Response,
  createCalendarLiveV29Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV29Request,
  persistCalendarLiveV29RawResponse,
  persistCalendarLiveV29TechnicalGates,
  readCalendarLiveV29AcceptedRawPayload,
  type CalendarLiveV29Request,
  type CalendarLiveV29TransactionAuthorization,
} from "./calendar-live-v29-broker.js";
import {
  CALENDAR_LIVE_V29_CAPTURE_COUNT,
  assertCalendarLiveV29CaptureResponses,
  ...
  evaluateCalendarLiveV29Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV29Roots,
  validateCalendarLiveV29CaptureManifest,
  validateCalendarLiveV29CapturePayload,
  validateCalendarLiveV29CleanupPayload,
  validateCalendarLiveV29ExtractPayload,
  validateCalendarLiveV29ProbePayload,
  validateCalendarLiveV29WriterPayload,
  type CalendarLiveV29CaptureCell,
  type CalendarLiveV29CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V29_PLAN_PATH,
  type CalendarLiveV29ProofPlan,
} from "./build-calendar-live-proof-v29.js";
import {
...
export const CALENDAR_LIVE_V29_RUNNER_VERSION = "calendar-live-v29-runner-v1";

interface CalendarLiveV29JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV29OrchestratorState {
  artifactVersion: "calendar-live-v29-orchestrator-state-v1";
  ...
  journal: CalendarLiveV29JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV29ProofPlan;
  planSha256: string;
  captures: CalendarLiveV29CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV29SourceIdentity & {
...
export interface CalendarLiveV29NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV29OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v29 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V29_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V29_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV29ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV29CaptureCell[];
  validateCalendarLiveV29CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v29 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v29 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV29WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV29RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV29Request =>
  readJson<CalendarLiveV29Request>(file);

export class CalendarLiveV29Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV29Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV29OrchestratorState = {
      artifactVersion: "calendar-live-v29-orchestrator-state-v1",
    ...
    return new CalendarLiveV29Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV29OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV29Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV29NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV29NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v29 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV29NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v29 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV29RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV29Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV29CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV29AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV29WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV29Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV29ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV29Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV29Response(this.#directory, "writer", {
        validate: validateCalendarLiveV29WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV29Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV29RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV29Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV29ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV29Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV29RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v29-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV29Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV29ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV29Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV29ProbePayload(payload, ownership),
      });
      persistCalendarLiveV29TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v29 capture cell missing");
      const accepted = acceptCalendarLiveV29Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV29CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V29_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV29CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV29Objective(
    ...
    throw new Error(`Calendar live v29 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV29Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV29CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV29Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV29RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV29Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV29CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV29CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV29CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV29Authorization = (
  privateKey: KeyObject,
): CalendarLiveV29TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV29SimulationKey = (): KeyObject =>
