...
import { INPUT_LIVE_V11_INDEX_PATH } from "./build-input-field-live-proof-v11.js";
import {
  INPUT_LIVE_V11_DYNAMIC_TOOL,
  INPUT_LIVE_V11_TARGET,
} from "./input-field-live-v11-broker.js";
import {
  buildInputLiveV11AuthorizationArtifact,
  validateInputLiveV11AntecedentIndex,
  validateInputLiveV11AuthorizationArtifact,
  validateInputLiveV11History,
  validateInputLiveV11UnitTestSource,
  type InputLiveV11AntecedentIndex,
  type InputLiveV11AuthorizationArtifact,
  type InputLiveV11AuthorizationProof,
  type InputLiveV11HistoryState,
} from "./input-field-live-v11-authorization.js";
import {
  inputLiveV11AttestationSha256,
  validateInputLiveV11ControlFlowSource,
  validateInputLiveV11SecurityAttestation,
  type InputLiveV11SecurityAttestation,
} from "./input-field-live-v11-preflight.js";
import { inputLiveV11Sha256 } from "./input-field-live-v11-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V11_INDEX_PATH);
const index = (): InputLiveV11AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV11AntecedentIndex;

const artifact = (): InputLiveV11AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV11AuthorizationArtifact({
...
const pendingState = (): InputLiveV11HistoryState => ({
  index: index(),
  indexSha256: inputLiveV11Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV11HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV11AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV11Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V11_TARGET,
  expectedDynamicTool: INPUT_LIVE_V11_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v11/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV11AuthorizationArtifact,
): InputLiveV11SecurityAttestation => {
  const body: Omit<InputLiveV11SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v11-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V11_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV11AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV11History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV11History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV11History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV11History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV11History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV11UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV11UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV11AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV11Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV11AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV11AuthorizationArtifact(
        planted as InputLiveV11AuthorizationArtifact,
        index(),
        inputLiveV11Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV11SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV11AttestationSha256(
      body as Omit<InputLiveV11SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV11SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV11ControlFlowSource(source), []);
});
