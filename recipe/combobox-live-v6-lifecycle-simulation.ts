...
export interface ComboboxLiveV6LifecycleSimulationReport {
  artifactVersion: "combobox-live-v6-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV6AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV6Lifecycle(): Promise<ComboboxLiveV6LifecycleSimulationReport> {
  await buildComboboxLiveV6Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V6_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV6AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v6-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V6_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V6_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV6History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV6History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV6AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V6_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V6_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV6History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV6History(authorized, "pending");
    ...
    await buildComboboxLiveV6Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V6_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV6AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV6Lifecycle(), null, 2)}\n`,
  );
