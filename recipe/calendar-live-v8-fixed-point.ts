/**
 * Calendar live v8 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V8_FIXED_VERIFIER_VERSION = "calendar-live-v8-fixed-point-v1";
export const CALENDAR_LIVE_V8_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV8FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV8FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV8FixedSourceProbe {
...
export interface CalendarLiveV8FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV8FixedFixedPoint;
}

export interface CalendarLiveV8FixedHardGateInput {
  thresholds: CalendarLiveV8FixedThresholds;
  sourceProbes: CalendarLiveV8FixedSourceProbe[];
  cells: CalendarLiveV8FixedCellValidation[];
  sceneProofs: CalendarLiveV8FixedSceneProof[];
...
export interface CalendarLiveV8FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V8_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV8FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV8FixedHardGates(
  input: CalendarLiveV8FixedHardGateInput,
): CalendarLiveV8FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV8FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V8_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV8FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV8FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
