...
import { buildCalendarLiveV5CleanupRuntime } from "./calendar-live-v5-cleanup.js";
import { type CalendarLiveV5FixedFixedPoint } from "./calendar-live-v5-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV5RestoreProgram,
  validateCalendarLiveV5RestorePayload,
  type CalendarLiveV5RestorePayload,
} from "./calendar-live-v5-restore.js";
import {
  buildCalendarLiveV5RawPropertyRuntime,
  normalizeCalendarLiveV5Scene,
  type CalendarLiveV5RawNode,
} from "./calendar-live-v5-verifier.js";
...
export const CALENDAR_LIVE_V5_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V5_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V5_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V5_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V5_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V5_SET_COUNT = 3;
export const CALENDAR_LIVE_V5_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V5_HOST_PHASES = 3;
export const CALENDAR_LIVE_V5_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V5_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V5_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V5_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV5SourceId = (typeof CALENDAR_LIVE_V5_SOURCE_IDS)[number];
export type CalendarLiveV5Kind = "calendar" | "week" | "day";

export interface CalendarLiveV5SourceIdentity {
  source: CalendarLiveV5SourceId;
...
export interface CalendarLiveV5WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV5ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV5SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV5RawNode;
    weekScene: CalendarLiveV5RawNode;
    dayScene: CalendarLiveV5RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV5RootProof {
  source: CalendarLiveV5SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV5FixedFixedPoint;
}

export interface CalendarLiveV5ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV5SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV5CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV5SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV5CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV5SourceId;
  frameWidth: number;
...
export interface CalendarLiveV5CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v5 ${label} must be an object`);
...
export function validateCalendarLiveV5WriterPayload(
  value: unknown,
): CalendarLiveV5WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v5") ||
    ...
    result.namespace !== CALENDAR_LIVE_V5_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v5 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v5 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v5 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV5SourceIdentity,
  kind: CalendarLiveV5Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV5ExtractProgram(
  writer: CalendarLiveV5WriterOwnership,
  sources: readonly CalendarLiveV5SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v5 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V5_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V5_NAMESPACE)}
${buildCalendarLiveV5RawPropertyRuntime()}
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("CALENDAR-V5-EXTRACT-PAGE");
...
if(get(page,"pageOwner")!=="recipe/calendar/"+${JSON.stringify(writer.runIdentity)})throw new Error("CALENDAR-V5-EXTRACT-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.has(node.id));
if(sets.length!==3)throw new Error("CALENDAR-V5-EXTRACT-ROOTS:"+sets.length);
...
for(const set of sets){
  ...
  if(!IDENTITIES[adapterIdentity]||(kind!=="calendar"&&kind!=="week"&&kind!=="day"))throw new Error("CALENDAR-V5-EXTRACT-ADAPTER:"+adapterIdentity+":"+kind);
...
if(roots.length!==1||roots.some(root=>!root.calendarScene||!root.weekScene||!root.dayScene))throw new Error("CALENDAR-V5-EXTRACT-TRIPLE");
...
export function validateCalendarLiveV5ExtractPayload(
  value: unknown,
  writer: CalendarLiveV5WriterOwnership,
): CalendarLiveV5ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v5 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V5_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v5 extract schema/one-root mismatch");
  return payload as CalendarLiveV5ExtractPayload;
}

export function proveCalendarLiveV5Roots<Instance>(
  extract: CalendarLiveV5ExtractPayload,
  sources: readonly (CalendarLiveV5SourceIdentity & {
...
): CalendarLiveV5RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v5 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v5 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV5Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV5Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV5Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV5FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v5 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v5 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v5 compile lost owned sets");
...
export function assertCalendarLiveV5RootProofs(
  proofs: readonly CalendarLiveV5RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v5 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v5 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV5ProbeProgram(
  writer: CalendarLiveV5WriterOwnership,
  sources: readonly CalendarLiveV5SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V5_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV5ProbePayload(
  value: unknown,
  writer: CalendarLiveV5WriterOwnership,
): CalendarLiveV5ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV5ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V5_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V5_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v5 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV5CaptureManifest(
  cells: readonly CalendarLiveV5CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V5_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V5_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V5_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V5_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v5 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV5CaptureCell[],
): string => {
  validateCalendarLiveV5CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV5CaptureProgram(
  writer: CalendarLiveV5WriterOwnership,
  cell: CalendarLiveV5CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V5_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v5 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v5 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V5_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV5CapturePayload(
  value: unknown,
  cell: CalendarLiveV5CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV5CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV5CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V5_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V5_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v5 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV5CaptureResponses(
  manifest: readonly CalendarLiveV5CaptureCell[],
  responses: readonly CalendarLiveV5CapturePayload[],
): void {
  validateCalendarLiveV5CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v5 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV5ObjectiveReport {
  artifactVersion: "calendar-live-v5-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV5Objective(
  manifest: readonly CalendarLiveV5CaptureCell[],
  responses: readonly CalendarLiveV5CapturePayload[],
): CalendarLiveV5ObjectiveReport {
  assertCalendarLiveV5CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v5-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV5CleanupProgram(
  writer: CalendarLiveV5WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV5CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V5_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v5 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV5CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV5WriterOwnership,
): CalendarLiveV5CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV5CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v5 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V5_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV5WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
