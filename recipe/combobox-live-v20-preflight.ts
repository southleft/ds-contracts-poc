...
import {
  validateComboboxLiveV20AntecedentIndex,
  verifyComboboxLiveV20Authorization,
  type ComboboxLiveV20AntecedentIndex,
  type ComboboxLiveV20AuthorizationProof,
} from "./combobox-live-v20-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V20_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v20/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V20_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v20-security-attestation.json";

export interface ComboboxLiveV20SecurityAttestation {
  artifactVersion: "combobox-live-v20-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV20PreflightReport {
  artifactVersion: "combobox-live-v20-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV20SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV20SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV20AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v20-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V20_TARGET) ||
...
export function validateComboboxLiveV20ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV20Authorization");
  const preflight = source.indexOf("runComboboxLiveV20Preflight");
  const initialize = source.indexOf("ComboboxLiveV20Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV20Preflight(
  root: string,
  proof: ComboboxLiveV20AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V20_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV20PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V20_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V20_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V20_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV20AntecedentIndex;
  failures.push(...validateComboboxLiveV20AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V20_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V20_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV20SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v20-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV20ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v20 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV20SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v20-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V20_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV20Preflight(
    process.cwd(),
    verifyComboboxLiveV20Authorization(),
    attestationPath,
