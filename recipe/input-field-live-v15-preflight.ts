...
import {
  ...
  verifyInputLiveV15Authorization,
  type InputLiveV15AntecedentIndex,
  type InputLiveV15AuthorizationProof,
} from "./input-field-live-v15-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V15_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v15/operator-security-attestation-template.json";
export const INPUT_LIVE_V15_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v15-security-attestation.json";

export interface InputLiveV15SecurityAttestation {
...
export interface InputLiveV15PreflightReport {
  artifactVersion: "input-live-v15-preflight-v1";
...
export const inputLiveV15AttestationSha256 = (
  value: Omit<InputLiveV15SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV15SecurityAttestation(
  value: unknown,
  proof: InputLiveV15AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v15-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V15_TARGET) ||
...
export function validateInputLiveV15ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV15Authorization");
  const preflight = source.indexOf("runInputLiveV15Preflight");
  const initialize = source.indexOf("InputLiveV15Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV15Preflight(
  root: string,
  proof: InputLiveV15AuthorizationProof,
  attestationPath = INPUT_LIVE_V15_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV15PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V15_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V15_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V15_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV15AntecedentIndex;
  failures.push(...validateInputLiveV15AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V15_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V15_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v15-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV15ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV15SecurityAttestation;
  return {
    artifactVersion: "input-live-v15-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V15_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV15Preflight(
    process.cwd(),
    verifyInputLiveV15Authorization(),
    attestationPath,
