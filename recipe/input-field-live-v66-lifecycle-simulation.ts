...
export interface InputLiveV66LifecycleSimulationReport {
  artifactVersion: "input-live-v66-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV66AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV66Lifecycle(): Promise<InputLiveV66LifecycleSimulationReport> {
  await buildInputLiveV66Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V66_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV66AntecedentIndex;
  const indexSha256 = inputLiveV66Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v66-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V66_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V66_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV66History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV66History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV66AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V66_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V66_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV66History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV66History(authorized, "pending");
    ...
    await buildInputLiveV66Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V66_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV66AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV66Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV66Lifecycle(), null, 2)}\n`,
  );
