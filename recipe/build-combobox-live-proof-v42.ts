...
import {
  ...
  COMBOBOX_LIVE_V42_HOST_PHASES,
  COMBOBOX_LIVE_V42_REMOTE_REQUESTS,
  COMBOBOX_LIVE_V42_SOURCE_ROOTS,
  COMBOBOX_LIVE_V42_VARIANT_COUNT,
  buildComboboxLiveV42CaptureProgram,
  buildComboboxLiveV42CleanupProgram,
  buildComboboxLiveV42ExtractProgram,
  buildComboboxLiveV42ProbeProgram,
  buildComboboxLiveV42RestoreProgram,
  comboboxLiveV5CaptureManifestSha256,
  type ComboboxLiveV42CaptureCell,
  type ComboboxLiveV42SourceIdentity,
  type ComboboxLiveV42WriterOwnership,
} from "./combobox-live-v42-contract.js";
import {
  COMBOBOX_LIVE_V42_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V42_TARGET,
  comboboxLiveV5RequestSequence,
} from "./combobox-live-v42-broker.js";
import {
...
export const COMBOBOX_LIVE_V42_EVIDENCE_ROOT =
  "recipe/evidence/combobox-live-pivot-v42";
export const COMBOBOX_LIVE_V42_PROTOCOL_PATH = `${COMBOBOX_LIVE_V42_EVIDENCE_ROOT}/protocol.json`;
export const COMBOBOX_LIVE_V42_PLAN_PATH = `${COMBOBOX_LIVE_V42_EVIDENCE_ROOT}/proof-plan.json`;
export const COMBOBOX_LIVE_V42_CAPTURE_MANIFEST_PATH = `${COMBOBOX_LIVE_V42_EVIDENCE_ROOT}/capture-manifest.json`;
export const COMBOBOX_LIVE_V42_REQUEST_MANIFEST_PATH = `${COMBOBOX_LIVE_V42_EVIDENCE_ROOT}/request-manifest.json`;
export const COMBOBOX_LIVE_V42_INDEX_PATH = `${COMBOBOX_LIVE_V42_EVIDENCE_ROOT}/antecedent-index.json`;
export const COMBOBOX_LIVE_V42_AUTHORIZATION_TEMPLATE_PATH = `${COMBOBOX_LIVE_V42_EVIDENCE_ROOT}/authorization-template.json`;
export const COMBOBOX_LIVE_V42_AUTHORIZATION_PATH = `${COMBOBOX_LIVE_V42_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface ComboboxLiveV42ProofPlan {
  artifactVersion: "combobox-live-v42-proof-plan-v1";
...
const captureCells = (
...
): ComboboxLiveV42CaptureCell[] => {
  const cells: ComboboxLiveV42CaptureCell[] = [];
  ...
  if (cells.length !== COMBOBOX_LIVE_V42_CAPTURE_COUNT)
    throw new Error(
      `Combobox live v42 capture cells ${cells.length}, expected ${COMBOBOX_LIVE_V42_CAPTURE_COUNT}`,
    );
...
const protocol = (expectedFacts: number) => ({
  artifactVersion: "combobox-live-v42-external-operator-protocol-v1",
  protocolId: "combobox-live-v42",
  status:
  ...
  lifecycle: {
    ...
    comboboxLiveV32AuthorizationReusable: false,
    comboboxLiveV33AuthorizationReusable: false,
    comboboxLiveV34AuthorizationReusable: false,
    comboboxLiveV35AuthorizationReusable: false,
    comboboxLiveV36AuthorizationReusable: false,
    comboboxLiveV37AuthorizationReusable: false,
    comboboxLiveV38AuthorizationReusable: false,
    comboboxLiveV39AuthorizationReusable: false,
    comboboxLiveV40AuthorizationReusable: false,
    comboboxLiveV41AuthorizationReusable: false,
    inputRunIdentityForbidden: true,
    inputPage115295378Forbidden: true,
    signedPage16335981Forbidden: true,
    v41Identity70c24cbdForbidden: true,
  },
  teaching: {
    ...
    hostEmitsCompileCarriedOptionSetLabel: true,
    hostObserveOmitsEmptySlotInstancePayload: true,
    hostObserveOmitsOptionInstanceInstancePayload: true,
    hostRecoversTriggerSlotComponentRefCompileSiblingOrder: true,
    hostRecoversCompileCarriedTriggerCharacters: true,
    hostObserveRecoversCompileCarriedOptionSetName: true,
    hostSceneToNormalizedIrRecoversTriggerSlotComponentRefCompileSiblingOrder: true,
    hostProbeKeepsExactSceneRestorationAfterOpenVariantWalk: true,
    hostProbeExcludesOverlayOptionLabelMessageOverlap: true,
    libraryNamedOverlayChrome: true,
    multiLayerOverlayShadowReceipt: true,
    statusSlotPaddingDeferred: true,
  },
  inheritedFromInput: {
  ...
  denominator: {
    sourceRoots: COMBOBOX_LIVE_V42_SOURCE_ROOTS,
    sets: 4,
    variants: COMBOBOX_LIVE_V42_VARIANT_COUNT,
    captureCells: COMBOBOX_LIVE_V42_CAPTURE_COUNT,
    remoteRequests: COMBOBOX_LIVE_V42_REMOTE_REQUESTS,
    hostPhases: COMBOBOX_LIVE_V42_HOST_PHASES,
...
export async function buildComboboxLiveV42Proof(
  check = process.argv.includes("--check"),
): Promise<ComboboxLiveV42ProofPlan> {
  ...
  if (
    ...
    throw new Error("Combobox live v42 writer reused Input identity");
  const captures = captureCells(sources);
  const writerProgram = Buffer.from(writer.code);
  const writerOwnershipBlueprint: ComboboxLiveV42WriterOwnership = {
  ...
  const identities: ComboboxLiveV42SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildComboboxLiveV42RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildComboboxLiveV42ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildComboboxLiveV42ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildComboboxLiveV42CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildComboboxLiveV42CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  antecedentOutputs.set(
    COMBOBOX_LIVE_V42_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V42_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V42_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V42_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V42_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V42_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V42_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "combobox-live-v42-capture-manifest-v1",
  ...
  antecedentOutputs.set(
    COMBOBOX_LIVE_V42_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: ComboboxLiveV42ProofPlan = {
    artifactVersion: "combobox-live-v42-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: COMBOBOX_LIVE_V42_TARGET,
    namespace: COMBOBOX_FIGMA_NAMESPACE,
    writer: {
      programPath: `${COMBOBOX_LIVE_V42_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: COMBOBOX_LIVE_V42_CAPTURE_MANIFEST_PATH,
      cells: 72,
      requests: 72,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(COMBOBOX_LIVE_V42_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "combobox-live-v42-request-manifest-v1",
    expectedDynamicTool: COMBOBOX_LIVE_V42_DYNAMIC_TOOL,
    target: COMBOBOX_LIVE_V42_TARGET,
  ...
  if (requests.requestCount !== COMBOBOX_LIVE_V42_REMOTE_REQUESTS)
    throw new Error("Combobox live v42 remote request denominator drifted");
  antecedentOutputs.set(
    COMBOBOX_LIVE_V42_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    COMBOBOX_LIVE_V42_AUTHORIZATION_TEMPLATE_PATH,
    COMBOBOX_LIVE_V42_AUTHORIZATION_PATH,
    "recipe/combobox-live-v42-authorization.ts",
    "recipe/combobox-live-v42-authorization.test.ts",
    "recipe/combobox-live-v42-preflight.ts",
    "recipe/combobox-live-v42-authorized.ts",
    "recipe/write-combobox-live-v42-authorization.ts",
    "recipe/create-combobox-live-v42-security-attestation.ts",
    "recipe/evidence/combobox-live-pivot-v42/operator-security-attestation-template.json",
    "recipe/evidence/combobox-live-pivot-v42-status.json",
    "recipe/evidence/status-index.json",
  ];
  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/combobox-live-v42-broker.ts",
    "recipe/combobox-live-v42-contract.ts",
    "recipe/combobox-live-v42-restore.ts",
    "recipe/combobox-live-v42-cleanup.ts",
    "recipe/combobox-live-v42-verifier.ts",
    "recipe/combobox-live-v42-fixed-point.ts",
    "recipe/run-combobox-live-v42.ts",
    "recipe/build-combobox-live-proof-v42.ts",
    "recipe/combobox-live-v42-lifecycle-simulation.ts",
    "recipe/scene-readback-combobox-v42.ts",
    "recipe/scene-readback-runtime-combobox-v42.ts",
    "recipe/combobox-figma-writer.ts",
    "recipe/figma-property-normalizer-v8.ts",
    "recipe/figma-runtime-portability.ts",
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== COMBOBOX_LIVE_V42_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "combobox-live-v42-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
  ...
  antecedentOutputs.set(COMBOBOX_LIVE_V42_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "combobox-live-v42-authorization-template-v1",
    authorizationId: "combobox-live-v42",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: COMBOBOX_LIVE_V42_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
    ...
    denominator: {
      remoteRequests: COMBOBOX_LIVE_V42_REMOTE_REQUESTS,
      hostPhases: COMBOBOX_LIVE_V42_HOST_PHASES,
      captures: COMBOBOX_LIVE_V42_CAPTURE_COUNT,
      sourceRoots: COMBOBOX_LIVE_V42_SOURCE_ROOTS,
      expectedFacts,
    },
    execution: {
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "combobox-live-v42-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/combobox-live-v42-security-attestation.json",
  });
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Combobox live v42 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(COMBOBOX_LIVE_V42_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(COMBOBOX_LIVE_V42_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${COMBOBOX_LIVE_V42_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(COMBOBOX_LIVE_V42_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(COMBOBOX_LIVE_V42_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Combobox live v42 authorization template drift");
  }
  if (
    writer.namespace !== COMBOBOX_FIGMA_NAMESPACE ||
    writer.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    ...
    !writerProgram.toString("utf8").includes("COMBOBOX-V41-IDENTITY-REUSE") ||
    writer.runIdentity === "70c24cbd-d27f2e85-combobox-v1" ||
    protocol(expectedFacts).execution.cleanupOnFailureOnly !== true
  )
    throw new Error("Combobox live v42 identity or cleanup invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildComboboxLiveV42Proof(), null, 2)}\n`,
  );
