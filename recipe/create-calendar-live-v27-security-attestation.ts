...
import { verifyCalendarLiveV27Authorization } from "./calendar-live-v27-authorization.js";
import { CALENDAR_LIVE_V27_TARGET } from "./calendar-live-v27-broker.js";
import {
  CALENDAR_LIVE_V27_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV27Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v27-session";
...
const attestation: CalendarLiveV27SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV27SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v27 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V27_SECURITY_ATTESTATION_DEFAULT_PATH,
);
