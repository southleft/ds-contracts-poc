...
import { INPUT_LIVE_V76_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v76.js";
import { verifyInputLiveV76Authorization } from "./input-field-live-v76-authorization.js";
import { runInputLiveV76Preflight } from "./input-field-live-v76-preflight.js";
import { InputLiveV76Orchestrator } from "./run-input-field-live-v76.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V76_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v76-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV76Authorization(root);
    runInputLiveV76Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV76Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
