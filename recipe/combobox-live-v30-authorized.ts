...
import { COMBOBOX_LIVE_V30_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v30.js";
import { verifyComboboxLiveV30Authorization } from "./combobox-live-v30-authorization.js";
import { runComboboxLiveV30Preflight } from "./combobox-live-v30-preflight.js";
import { ComboboxLiveV30Orchestrator } from "./run-combobox-live-v30.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v30 requires exact authorization artifact ${COMBOBOX_LIVE_V30_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v30-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV30Authorization(root);
    runComboboxLiveV30Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV30Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV30Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
