...
import { verifyTableLiveV7Authorization } from "./table-live-v7-authorization.js";
import { TABLE_LIVE_V7_TARGET } from "./table-live-v7-broker.js";
import {
  TABLE_LIVE_V7_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV7Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v7-session";
...
const attestation: TableLiveV7SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV7SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v7 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V7_SECURITY_ATTESTATION_DEFAULT_PATH,
);
