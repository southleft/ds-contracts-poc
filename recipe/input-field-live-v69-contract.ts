...
} from "./input-field-live-v3-verifier-v69.js";
import {
  buildInputLiveV69RawPropertyRuntime,
  normalizeInputLiveV69Scene,
  type InputLiveV69RawNode,
} from "./input-field-live-v69-verifier.js";
...
export {
  ...
  type InputLiveV69RestorePayload,
} from "./input-field-live-v69-restore.js";

export const INPUT_LIVE_V69_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V69_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V69_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V69_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V69_VARIANT_COUNT = 256;
export const INPUT_LIVE_V69_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V69_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV69SourceId = (typeof INPUT_LIVE_V69_SOURCE_IDS)[number];

export interface InputLiveV69SourceIdentity {
  source: InputLiveV69SourceId;
...
export interface InputLiveV69WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV69ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV69SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV69RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV69RootProof {
  source: InputLiveV69SourceId;
  adapterIdentity: string;
...
export interface InputLiveV69ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV69SourceId;
    adapterIdentity: string;
...
export interface InputLiveV69CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV69SourceId;
  adapterIdentity: string;
...
export interface InputLiveV69CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV69SourceId;
  strata: InputLiveV69CaptureCell["strata"];
...
export interface InputLiveV69CleanupPayload {
...
export function validateInputLiveV69WriterPayload(
  value: unknown,
): InputLiveV69WriterOwnership {
...
const identityPlan = (source: InputLiveV69SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV69ExtractProgram(
  writer: InputLiveV69WriterOwnership,
  sources: readonly InputLiveV69SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V69_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V69_NAMESPACE)}
${buildInputLiveV69RawPropertyRuntime()}
...
export function validateInputLiveV69ExtractPayload(
  value: unknown,
  writer: InputLiveV69WriterOwnership,
): InputLiveV69ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V69_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV69ExtractPayload;
}

export function proveInputLiveV69Roots<Instance>(
  extract: InputLiveV69ExtractPayload,
  sources: readonly (InputLiveV69SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV69Scene(
      root.scene,
...
export function assertInputLiveV69RootProofs(
  proofs: readonly InputLiveV69RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV69ProbeProgram(
  writer: InputLiveV69WriterOwnership,
  sources: readonly InputLiveV69SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V69_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV69ProbePayload(
  value: unknown,
  writer: InputLiveV69WriterOwnership,
): InputLiveV69ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV69ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V69_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V69_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV69CaptureManifest(
  cells: readonly InputLiveV69CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V69_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V69_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V69_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V69_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V69_SOURCE_IDS.some(
...
export const inputLiveV69CaptureManifestSha256 = (
  cells: readonly InputLiveV69CaptureCell[],
): string => {
  validateInputLiveV69CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV69CaptureProgram(
  writer: InputLiveV69WriterOwnership,
  cell: InputLiveV69CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V69_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V69_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V69_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV69CapturePayload(
  value: unknown,
  cell: InputLiveV69CaptureCell,
  rawResponseBytes: number,
): InputLiveV69CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV69CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V69_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V69_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV69CaptureResponses(
  manifest: readonly InputLiveV69CaptureCell[],
  responses: readonly InputLiveV69CapturePayload[],
): void {
  validateInputLiveV69CaptureManifest(manifest);
  if (
...
export interface InputLiveV69ObjectiveReport {
  artifactVersion: "input-live-v69-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV69Objective(
  manifest: readonly InputLiveV69CaptureCell[],
  responses: readonly InputLiveV69CapturePayload[],
  readReference: (cell: InputLiveV69CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV69CaptureCell) => Buffer,
): InputLiveV69ObjectiveReport {
  assertInputLiveV69CaptureResponses(manifest, responses);
...
export function buildInputLiveV69CleanupProgram(
  writer: InputLiveV69WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V69_NAMESPACE,
...
export function validateInputLiveV69CleanupPayload(
  value: unknown,
  writer: InputLiveV69WriterOwnership,
): InputLiveV69CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV69CleanupPayload;
...
export const INPUT_LIVE_V69_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV69WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
