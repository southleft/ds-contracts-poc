...
export interface InputLiveV82LifecycleSimulationReport {
  artifactVersion: "input-live-v82-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV82AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV82Lifecycle(): Promise<InputLiveV82LifecycleSimulationReport> {
  await buildInputLiveV82Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V82_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV82AntecedentIndex;
  const indexSha256 = inputLiveV82Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v82-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V82_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V82_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV82History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV82History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV82AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V82_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V82_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV82History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV82History(authorized, "pending");
    ...
    await buildInputLiveV82Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V82_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV82AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV82Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV82Lifecycle(), null, 2)}\n`,
  );
