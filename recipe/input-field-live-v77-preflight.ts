...
import {
  ...
  verifyInputLiveV77Authorization,
  type InputLiveV77AntecedentIndex,
  type InputLiveV77AuthorizationProof,
} from "./input-field-live-v77-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V77_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v77/operator-security-attestation-template.json";
export const INPUT_LIVE_V77_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v77-security-attestation.json";

export interface InputLiveV77SecurityAttestation {
...
export interface InputLiveV77PreflightReport {
  artifactVersion: "input-live-v77-preflight-v1";
...
export const inputLiveV77AttestationSha256 = (
  value: Omit<InputLiveV77SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV77SecurityAttestation(
  value: unknown,
  proof: InputLiveV77AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v77-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V77_TARGET) ||
...
export function validateInputLiveV77ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV77Authorization");
  const preflight = source.indexOf("runInputLiveV77Preflight");
  const initialize = source.indexOf("InputLiveV77Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV77Preflight(
  root: string,
  proof: InputLiveV77AuthorizationProof,
  attestationPath = INPUT_LIVE_V77_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV77PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V77_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V77_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V77_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV77AntecedentIndex;
  failures.push(...validateInputLiveV77AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V77_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V77_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v77-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV77ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV77SecurityAttestation;
  return {
    artifactVersion: "input-live-v77-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V77_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV77Preflight(
    process.cwd(),
    verifyInputLiveV77Authorization(),
    attestationPath,
