...
import { buildComboboxLiveV41CleanupRuntime } from "./combobox-live-v41-cleanup.js";
import {
  type ComboboxLiveV41FixedFixedPoint,
} from "./combobox-live-v41-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV41RestoreProgram,
  validateComboboxLiveV41RestorePayload,
  type ComboboxLiveV41RestorePayload,
} from "./combobox-live-v41-restore.js";
import {
  buildComboboxLiveV41RawPropertyRuntime,
  normalizeComboboxLiveV41Scene,
  type ComboboxLiveV41RawNode,
} from "./combobox-live-v41-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v41.js";
import {
...
} from "./scene-readback-combobox-v41.js";
...
export const COMBOBOX_LIVE_V41_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V41_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V41_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V41_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V41_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V41_SET_COUNT = 4;
export const COMBOBOX_LIVE_V41_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V41_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V41_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V41_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V41_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV41SourceId = (typeof COMBOBOX_LIVE_V41_SOURCE_IDS)[number];

export interface ComboboxLiveV41SourceIdentity {
  source: ComboboxLiveV41SourceId;
...
export interface ComboboxLiveV41WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV41ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV41SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV41RawNode;
    optionScene: ComboboxLiveV41RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV41RootProof {
  source: ComboboxLiveV41SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV41FixedFixedPoint;
}

export interface ComboboxLiveV41ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV41SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV41CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV41SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV41CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV41SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV41CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v41 ${label} must be an object`);
...
export function validateComboboxLiveV41WriterPayload(
  value: unknown,
): ComboboxLiveV41WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V41_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v41 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v41 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v41 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV41SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV41ExtractProgram(
  writer: ComboboxLiveV41WriterOwnership,
  sources: readonly ComboboxLiveV41SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v41 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V41_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V41_NAMESPACE)}
${buildComboboxLiveV41RawPropertyRuntime()}
...
export function validateComboboxLiveV41ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV41WriterOwnership,
): ComboboxLiveV41ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v41 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V41_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v41 extract schema/two-root mismatch");
  return payload as ComboboxLiveV41ExtractPayload;
}

export function proveComboboxLiveV41Roots<Instance>(
  extract: ComboboxLiveV41ExtractPayload,
  sources: readonly (ComboboxLiveV41SourceIdentity & {
...
): ComboboxLiveV41RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v41 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v41 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV41Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV41Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV41FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v41 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v41 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v41 compile lost owned sets");
...
export function assertComboboxLiveV41RootProofs(
  proofs: readonly ComboboxLiveV41RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v41 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v41 independent root accounting failed: ${failures.join(";")}`,
    );
}

/** v41: measure open-options overlap pairs, then do not score overlay option/label and selected-indicator against in-flow helper/error. */
export function buildComboboxLiveV41ProbeProgram(
  writer: ComboboxLiveV41WriterOwnership,
  sources: readonly ComboboxLiveV41SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V41_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
for(const row of byAdapter.values()){
 ...
 const visitSet=(set,kind)=>{
  for(const component of set.children){
   ...
   const overlayOptionRole=r=>r==="combobox/option/label"||r==="combobox/option/selected-indicator",messageRole=r=>r==="combobox/message/helper"||r==="combobox/message/error";void "COMBOBOX-PROBE-MEASURE-THEN-EXCLUDE-OVERLAY-OPTION-LABEL-MESSAGE-OVERLAP";
   const measuredPairs=[];for(let i=0;i<overlapSemantic.length;i++)for(let j=i+1;j<overlapSemantic.length;j++){const px=overlap(box(overlapSemantic[i]),box(overlapSemantic[j]));if(px>2)measuredPairs.push({a:role(overlapSemantic[i]),b:role(overlapSemantic[j]),aType:overlapSemantic[i].type,bType:overlapSemantic[j].type,aOpacity:overlapSemantic[i].opacity,bOpacity:overlapSemantic[j].opacity,overlapPixels:px});}
   let maximumOverlap=0;for(let i=0;i<overlapSemantic.length;i++)for(let j=i+1;j<overlapSemantic.length;j++){const ra=role(overlapSemantic[i]),rb=role(overlapSemantic[j]);if((overlayOptionRole(ra)&&messageRole(rb))||(overlayOptionRole(rb)&&messageRole(ra)))continue;maximumOverlap=Math.max(maximumOverlap,overlap(box(overlapSemantic[i]),box(overlapSemantic[j])));}
   ...
   cells.push({source,adapterIdentity,cellKey:[source,kind,...Object.values(axis)].join("/"),kind,rolesExact,stateSemanticsExact,noFakeLayout,visibleAreaLoss:Math.max(0,...clipSemantic.map(node=>visibleLoss(box(node),componentBox))),overlapPixels:maximumOverlap,overlapPairs:measuredPairs});
  }
 };
 ...
 const switchingRestored=JSON.stringify(original)===JSON.stringify(plain(instance)),after=snapshot(instance);void "COMBOBOX-PROBE-KEEP-EXACT-SCENE-RESTORATION-AFTER-OPEN-VARIANT-WALK";const exactSceneRestoration=(!!reflowPassed&&switchingRestored&&textPropertiesRestored)||before===after;instance.remove();
 sources.push({source,adapterIdentity,variants:72,visitedVariants:visited.size+optionVisited.size,reflowPassed:!!reflowPassed,contentFillPassed:!!contentFillPassed,bindingCompatibilityPassed,noFakeLayoutPassed:sourceCells.every(cell=>cell.noFakeLayout),stateSemanticsPassed:sourceCells.every(cell=>cell.stateSemanticsExact),switchingRestored,textPropertiesRestored,exactSceneRestoration});
}
...
export function validateComboboxLiveV41ProbePayload(
  value: unknown,
  writer: ComboboxLiveV41WriterOwnership,
): ComboboxLiveV41ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV41ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V41_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V41_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v41 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV41CaptureManifest(
  cells: readonly ComboboxLiveV41CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V41_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V41_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V41_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V41_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v41 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV41CaptureCell[],
): string => {
  validateComboboxLiveV41CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV41CaptureProgram(
  writer: ComboboxLiveV41WriterOwnership,
  cell: ComboboxLiveV41CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V41_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v41 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v41 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V41_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV41CapturePayload(
  value: unknown,
  cell: ComboboxLiveV41CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV41CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV41CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V41_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V41_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v41 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV41CaptureResponses(
  manifest: readonly ComboboxLiveV41CaptureCell[],
  responses: readonly ComboboxLiveV41CapturePayload[],
): void {
  validateComboboxLiveV41CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v41 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV41ObjectiveReport {
  artifactVersion: "combobox-live-v41-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV41Objective(
  manifest: readonly ComboboxLiveV41CaptureCell[],
  responses: readonly ComboboxLiveV41CapturePayload[],
): ComboboxLiveV41ObjectiveReport {
  assertComboboxLiveV41CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v41-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV41CleanupProgram(
  writer: ComboboxLiveV41WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV41CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V41_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v41 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV41CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV41WriterOwnership,
): ComboboxLiveV41CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV41CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v41 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V41_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV41WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
