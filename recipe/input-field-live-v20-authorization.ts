...
import {
  ...
  INPUT_LIVE_V20_INDEX_PATH,
} from "./build-input-field-live-proof-v20.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV20HistoryExpectation = "pending" | "authorized";

export interface InputLiveV20AuthorizationArtifact {
  artifactVersion: "input-live-v20-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V20_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV20AntecedentIndex {
...
export interface InputLiveV20HistoryState {
  index?: InputLiveV20AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV20AuthorizationArtifact;
...
export interface InputLiveV20AuthorizationProof extends InputLiveV20TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V20_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V20_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V20_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V20_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v20-status") ||
...
export function validateInputLiveV20AntecedentIndex(
  index: InputLiveV20AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v20-antecedent-index-v1" ||
...
export function validateInputLiveV20UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV20History\(\s*["']/.test(source) ||
    /readInputLiveV20HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV20AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV20AuthorizationArtifact {
  ...
  ) as InputLiveV20AntecedentIndex;
  const indexFailures = validateInputLiveV20AntecedentIndex(index);
...
export function validateInputLiveV20AuthorizationArtifact(
  artifact: InputLiveV20AuthorizationArtifact | undefined,
  index: InputLiveV20AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v20-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v20" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V20_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V20_TARGET) ||
    !equalJson(
...
export function validateInputLiveV20History(
  state: InputLiveV20HistoryState,
  expected: InputLiveV20HistoryExpectation,
): string[] {
  const failures = validateInputLiveV20AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV20AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV20HistoryState(
  root = process.cwd(),
): InputLiveV20HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V20_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV20AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V20_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V20_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V20_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV20History(
  expected: InputLiveV20HistoryExpectation,
  root = process.cwd(),
): InputLiveV20HistoryState {
  const state = readInputLiveV20HistoryState(root);
  const failures = validateInputLiveV20History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV20Authorization(
  root = process.cwd(),
): InputLiveV20AuthorizationProof {
  const state = verifyInputLiveV20History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V20_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v20/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v20.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V20_TARGET,
    expectedDynamicTool: INPUT_LIVE_V20_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V20_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV20HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV20History(expected);
