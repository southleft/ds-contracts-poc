...
import {
  verifyComboboxLiveV21FixedSceneFixedPoint,
  type ComboboxLiveV21FixedFixedPoint,
} from "./combobox-live-v21-fixed-point.js";

export const COMBOBOX_LIVE_V21_VERIFIER_VERSION = "combobox-live-v21-verifier-v1";

export function buildComboboxLiveV21RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV21RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV21RawNode[];
}

export interface ComboboxLiveV21NormalizedScene {
...
export interface ComboboxLiveV21SceneProof {
...
export function normalizeComboboxLiveV21Scene(
  root: ComboboxLiveV21RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV21NormalizedScene {
  const canonicalBindings: ComboboxLiveV21NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV21RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV21Scene<Instance>(
  normalized: ComboboxLiveV21NormalizedScene,
...
): ComboboxLiveV21SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV21FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV21ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV21PreCaptureGates(payload: unknown): void {
