...
import { verifyTableLiveV37Authorization } from "./table-live-v37-authorization.js";
import { TABLE_LIVE_V37_TARGET } from "./table-live-v37-broker.js";
import {
  TABLE_LIVE_V37_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV37Authorization();
...
const attestation: TableLiveV37SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV37SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v37 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V37_SECURITY_ATTESTATION_DEFAULT_PATH,
);
