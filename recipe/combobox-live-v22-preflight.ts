...
import {
  validateComboboxLiveV22AntecedentIndex,
  verifyComboboxLiveV22Authorization,
  type ComboboxLiveV22AntecedentIndex,
  type ComboboxLiveV22AuthorizationProof,
} from "./combobox-live-v22-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V22_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v22/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V22_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v22-security-attestation.json";

export interface ComboboxLiveV22SecurityAttestation {
...
export interface ComboboxLiveV22PreflightReport {
  artifactVersion: "combobox-live-v22-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV22SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV22SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV22AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v22-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V22_TARGET) ||
...
export function validateComboboxLiveV22ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV22Authorization");
  const preflight = source.indexOf("runComboboxLiveV22Preflight");
  const initialize = source.indexOf("ComboboxLiveV22Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV22Preflight(
  root: string,
  proof: ComboboxLiveV22AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V22_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV22PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V22_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V22_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V22_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV22AntecedentIndex;
  failures.push(...validateComboboxLiveV22AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V22_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V22_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v22-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV22ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v22 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV22SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v22-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V22_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV22Preflight(
    process.cwd(),
    verifyComboboxLiveV22Authorization(),
    attestationPath,
