...
import { buildComboboxLiveV28CleanupRuntime } from "./combobox-live-v28-cleanup.js";
import {
  type ComboboxLiveV28FixedFixedPoint,
} from "./combobox-live-v28-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV28RestoreProgram,
  validateComboboxLiveV28RestorePayload,
  type ComboboxLiveV28RestorePayload,
} from "./combobox-live-v28-restore.js";
import {
  buildComboboxLiveV28RawPropertyRuntime,
  normalizeComboboxLiveV28Scene,
  type ComboboxLiveV28RawNode,
} from "./combobox-live-v28-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v28.js";
import {
...
} from "./scene-readback-combobox-v28.js";
...
export const COMBOBOX_LIVE_V28_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V28_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V28_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V28_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V28_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V28_SET_COUNT = 4;
export const COMBOBOX_LIVE_V28_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V28_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V28_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V28_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V28_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV28SourceId = (typeof COMBOBOX_LIVE_V28_SOURCE_IDS)[number];

export interface ComboboxLiveV28SourceIdentity {
  source: ComboboxLiveV28SourceId;
...
export interface ComboboxLiveV28WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV28ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV28SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV28RawNode;
    optionScene: ComboboxLiveV28RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV28RootProof {
  source: ComboboxLiveV28SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV28FixedFixedPoint;
}

export interface ComboboxLiveV28ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV28SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV28CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV28SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV28CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV28SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV28CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v28 ${label} must be an object`);
...
export function validateComboboxLiveV28WriterPayload(
  value: unknown,
): ComboboxLiveV28WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V28_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v28 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v28 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v28 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV28SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV28ExtractProgram(
  writer: ComboboxLiveV28WriterOwnership,
  sources: readonly ComboboxLiveV28SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v28 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V28_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V28_NAMESPACE)}
${buildComboboxLiveV28RawPropertyRuntime()}
...
export function validateComboboxLiveV28ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV28WriterOwnership,
): ComboboxLiveV28ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v28 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V28_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v28 extract schema/two-root mismatch");
  return payload as ComboboxLiveV28ExtractPayload;
}

export function proveComboboxLiveV28Roots<Instance>(
  extract: ComboboxLiveV28ExtractPayload,
  sources: readonly (ComboboxLiveV28SourceIdentity & {
...
): ComboboxLiveV28RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v28 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v28 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV28Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV28Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV28FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v28 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v28 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v28 compile lost owned sets");
...
export function assertComboboxLiveV28RootProofs(
  proofs: readonly ComboboxLiveV28RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v28 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v28 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV28ProbeProgram(
  writer: ComboboxLiveV28WriterOwnership,
  sources: readonly ComboboxLiveV28SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V28_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV28ProbePayload(
  value: unknown,
  writer: ComboboxLiveV28WriterOwnership,
): ComboboxLiveV28ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV28ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V28_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V28_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v28 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV28CaptureManifest(
  cells: readonly ComboboxLiveV28CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V28_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V28_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V28_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V28_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v28 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV28CaptureCell[],
): string => {
  validateComboboxLiveV28CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV28CaptureProgram(
  writer: ComboboxLiveV28WriterOwnership,
  cell: ComboboxLiveV28CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V28_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v28 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v28 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V28_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV28CapturePayload(
  value: unknown,
  cell: ComboboxLiveV28CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV28CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV28CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V28_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V28_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v28 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV28CaptureResponses(
  manifest: readonly ComboboxLiveV28CaptureCell[],
  responses: readonly ComboboxLiveV28CapturePayload[],
): void {
  validateComboboxLiveV28CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v28 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV28ObjectiveReport {
  artifactVersion: "combobox-live-v28-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV28Objective(
  manifest: readonly ComboboxLiveV28CaptureCell[],
  responses: readonly ComboboxLiveV28CapturePayload[],
): ComboboxLiveV28ObjectiveReport {
  assertComboboxLiveV28CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v28-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV28CleanupProgram(
  writer: ComboboxLiveV28WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV28CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V28_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v28 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV28CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV28WriterOwnership,
): ComboboxLiveV28CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV28CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v28 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V28_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV28WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
