...
import {
  validateComboboxLiveV35AntecedentIndex,
  verifyComboboxLiveV35Authorization,
  type ComboboxLiveV35AntecedentIndex,
  type ComboboxLiveV35AuthorizationProof,
} from "./combobox-live-v35-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V35_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v35/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V35_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v35-security-attestation.json";

export interface ComboboxLiveV35SecurityAttestation {
  artifactVersion: "combobox-live-v35-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV35PreflightReport {
  artifactVersion: "combobox-live-v35-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV35SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV35SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV35AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v35-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V35_TARGET) ||
...
export function validateComboboxLiveV35ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV35Authorization");
  const preflight = source.indexOf("runComboboxLiveV35Preflight");
  const initialize = source.indexOf("ComboboxLiveV35Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV35Preflight(
  root: string,
  proof: ComboboxLiveV35AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V35_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV35PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V35_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V35_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V35_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV35AntecedentIndex;
  failures.push(...validateComboboxLiveV35AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V35_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V35_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV35SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v35-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV35ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v35 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV35SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v35-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V35_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV35Preflight(
    process.cwd(),
    verifyComboboxLiveV35Authorization(),
    attestationPath,
