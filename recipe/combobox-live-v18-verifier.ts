...
import {
  verifyComboboxLiveV18FixedSceneFixedPoint,
  type ComboboxLiveV18FixedFixedPoint,
} from "./combobox-live-v18-fixed-point.js";

export const COMBOBOX_LIVE_V18_VERIFIER_VERSION = "combobox-live-v18-verifier-v1";

export function buildComboboxLiveV18RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV18RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV18RawNode[];
}

export interface ComboboxLiveV18NormalizedScene {
...
export interface ComboboxLiveV18SceneProof {
...
export function normalizeComboboxLiveV18Scene(
  root: ComboboxLiveV18RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV18NormalizedScene {
  const canonicalBindings: ComboboxLiveV18NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV18RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV18Scene<Instance>(
  normalized: ComboboxLiveV18NormalizedScene,
...
): ComboboxLiveV18SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV18FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV18ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV18PreCaptureGates(payload: unknown): void {
