...
import { verifyComboboxLiveV1Authorization } from "./combobox-live-v1-authorization.js";
import { COMBOBOX_LIVE_V1_TARGET } from "./combobox-live-v1-broker.js";
import {
  COMBOBOX_LIVE_V1_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV1Authorization();
...
const attestation: ComboboxLiveV1SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV1AttestationSha256(body),
};
const failures = validateComboboxLiveV1SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v1 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V1_SECURITY_ATTESTATION_DEFAULT_PATH,
);
