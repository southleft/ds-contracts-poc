/**
 * Table live v30 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V30_FIXED_VERIFIER_VERSION = "table-live-v30-fixed-point-v1";
export const TABLE_LIVE_V30_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV30FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV30FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV30FixedSourceProbe {
...
export interface TableLiveV30FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV30FixedFixedPoint;
}

export interface TableLiveV30FixedHardGateInput {
  thresholds: TableLiveV30FixedThresholds;
  sourceProbes: TableLiveV30FixedSourceProbe[];
  cells: TableLiveV30FixedCellValidation[];
  sceneProofs: TableLiveV30FixedSceneProof[];
...
export interface TableLiveV30FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V30_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV30FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV30FixedHardGates(
  input: TableLiveV30FixedHardGateInput,
): TableLiveV30FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV30FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V30_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV30FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV30FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
