...
import {
  validateCalendarLiveV25AntecedentIndex,
  verifyCalendarLiveV25Authorization,
  type CalendarLiveV25AntecedentIndex,
  type CalendarLiveV25AuthorizationProof,
} from "./calendar-live-v25-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V25_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v25/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V25_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v25-security-attestation.json";

export interface CalendarLiveV25SecurityAttestation {
...
export interface CalendarLiveV25PreflightReport {
  artifactVersion: "calendar-live-v25-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV25SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV25SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV25AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v25-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V25_TARGET) ||
...
export function validateCalendarLiveV25ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV25Authorization");
  const preflight = source.indexOf("runCalendarLiveV25Preflight");
  const initialize = source.indexOf("CalendarLiveV25Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV25Preflight(
  root: string,
  proof: CalendarLiveV25AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V25_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV25PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V25_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V25_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV25AntecedentIndex;
  failures.push(...validateCalendarLiveV25AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V25_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V25_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v25-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV25ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v25 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV25SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v25-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V25_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV25Preflight(
    process.cwd(),
    verifyCalendarLiveV25Authorization(),
    attestationPath,
