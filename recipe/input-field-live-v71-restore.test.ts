...
import {
  INPUT_LIVE_V71_MEASURE_VISIBLE_FILL_MARKER,
  INPUT_LIVE_V71_RESTORE_CONTENT_ROLES,
  INPUT_LIVE_V71_RESTORE_PARENT_ROLES,
...
test("v16 restore still measures FILL while visible, then restores visibility", () => {
  const program = buildInputLiveV71RestoreProgram(writer);
  assert.match(program, new RegExp(INPUT_LIVE_V71_MEASURE_VISIBLE_FILL_MARKER));
  ...
  assert.match(program, /INPUT-V71-RESTORE-COUNT/);
  assert.match(program, /INPUT-V71-RESTORE-NOT-FILL/);
  ...
  for (const role of INPUT_LIVE_V71_RESTORE_CONTENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
  for (const role of INPUT_LIVE_V71_RESTORE_PARENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
});

test("v16 restore payload requires 256 filled content texts", () => {
  const accepted = validateInputLiveV71RestorePayload(
    {
      pageId: "1:1",
      setIds: ["2:2", "2:1"],
      restoredCount: 256,
      ...
      marker: INPUT_LIVE_V71_MEASURE_VISIBLE_FILL_MARKER,
    },
    writer,
  );
...
test("v16 writer bytes stay frozen and v17 writer assigns set HUG", () => {
  ...
  const v71Program = readFileSync(
    "recipe/evidence/input-field-live-pivot-v71/programs/writer.txt",
  );
  const v71Payload = readFileSync(
    "recipe/evidence/input-field-live-pivot-v71/programs/writer-payload.js",
  );
  assert.equal(sha256(v16Program), V12_WRITER_PROGRAM_SHA256);
  assert.equal(sha256(v16Payload), V12_WRITER_PAYLOAD_SHA256);
  assert.notEqual(sha256(v71Program), V12_WRITER_PROGRAM_SHA256);
  assert.notEqual(sha256(v71Payload), V12_WRITER_PAYLOAD_SHA256);
  assert.match(v71Payload.toString("utf8"), /set\.layoutSizingHorizontal="HUG"/);
