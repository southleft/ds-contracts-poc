...
import { antdRadioAdapterConfig as antdRadioProposedConfig, antdRadioSource as antdRadioProposedSource } from "./fixtures/generated/radio.antd.js";
import { chakraRadioAdapterConfig as chakraRadioProposedConfig, chakraRadioSource as chakraRadioProposedSource } from "./fixtures/generated/radio.chakra.js";
import { compileRadioRecipe, radioRecipe } from "./recipes/radio.js";

const EVIDENCE = "recipe/evidence/radio-live-pivot-v9";
...
const sources = [
  ...
  {
    // PROPOSED from AntD's own capture by the radio@1 role schema (32 read, 0 invented).
    adapterIdentity: "antd-radio-proposed-v1",
    displayName: "Ant Design (proposed)",
    source: antdRadioProposedSource,
    config: antdRadioProposedConfig,
  },
  {
    // HELD OUT: Chakra's RadioGroup, captured today through ItemIndicator (32 read, 0 invented).
    adapterIdentity: "chakra-radio-proposed-v1",
    displayName: "Chakra (proposed)",
    source: chakraRadioProposedSource,
    config: chakraRadioProposedConfig,
  },
].map((entry) => {
...
const receiptOwned = {
      artifactVersion: "radio-live-pivot-v9-prepare",
      teaching:
...
if (process.argv.includes("--check")) {
  ...
  if (writer.sourcePlans.length !== 5)
    throw new Error("radio prepare requires 5 library sources (reviewed + proposed)");
  console.log(
    JSON.stringify({
      check: "ok",
      runIdentity: writer.runIdentity,
