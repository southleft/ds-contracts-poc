...
import { CALENDAR_LIVE_V33_INDEX_PATH } from "./build-calendar-live-proof-v33.js";
import {
...
import {
  buildCalendarLiveV33AuthorizationArtifact,
  validateCalendarLiveV33AntecedentIndex,
  validateCalendarLiveV33AuthorizationArtifact,
  validateCalendarLiveV33History,
  validateCalendarLiveV33UnitTestSource,
  type CalendarLiveV33AntecedentIndex,
  type CalendarLiveV33AuthorizationArtifact,
  type CalendarLiveV33AuthorizationProof,
  type CalendarLiveV33HistoryState,
} from "./calendar-live-v33-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV33ControlFlowSource,
  validateCalendarLiveV33SecurityAttestation,
  type CalendarLiveV33SecurityAttestation,
} from "./calendar-live-v33-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V33_INDEX_PATH);
const index = (): CalendarLiveV33AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV33AntecedentIndex;

const artifact = (): CalendarLiveV33AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV33AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV33HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV33HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV33AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v33/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV33AuthorizationArtifact,
): CalendarLiveV33SecurityAttestation => {
  const body: Omit<CalendarLiveV33SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V33_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV33History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV33History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV33History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV33History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV33History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV33UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v33-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV33UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV33AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV33AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV33AuthorizationArtifact(
        planted as CalendarLiveV33AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV33SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV33SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV33SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v33-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV33ControlFlowSource(source), []);
});
