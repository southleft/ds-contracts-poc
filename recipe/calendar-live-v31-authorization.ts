...
export type CalendarLiveV31HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV31AuthorizationArtifact {
  artifactVersion: "calendar-live-v31-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V31_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV31AntecedentIndex {
...
export interface CalendarLiveV31HistoryState {
  index?: CalendarLiveV31AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV31AuthorizationArtifact;
...
export interface CalendarLiveV31AuthorizationProof
  extends CalendarLiveV31TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V31_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V31_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V31_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V31_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v31-authorization") ||
  artifactPath.includes("calendar-live-v31-preflight") ||
  artifactPath.includes("calendar-live-v31-authorized") ||
  artifactPath.includes("create-calendar-live-v31-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v31-status") ||
...
export function validateCalendarLiveV31AntecedentIndex(
  index: CalendarLiveV31AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v31-antecedent-index-v1" ||
...
export function validateCalendarLiveV31UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV31History\(\s*["']/.test(source) ||
    /readCalendarLiveV31HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV31AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV31AuthorizationArtifact {
  ...
  ) as CalendarLiveV31AntecedentIndex;
  const indexFailures = validateCalendarLiveV31AntecedentIndex(index);
...
export function validateCalendarLiveV31AuthorizationArtifact(
  artifact: CalendarLiveV31AuthorizationArtifact | undefined,
  index: CalendarLiveV31AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v31-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v31" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V31_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V31_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V31_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V31_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V31_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V31_SOURCE_ROOTS ||
...
export function validateCalendarLiveV31History(
  state: CalendarLiveV31HistoryState,
  expected: CalendarLiveV31HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV31AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV31AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV31HistoryState(
  root = process.cwd(),
): CalendarLiveV31HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V31_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV31AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V31_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V31_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V31_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV31History(
  expected: CalendarLiveV31HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV31HistoryState {
  const state = readCalendarLiveV31HistoryState(root);
  const failures = validateCalendarLiveV31History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v31 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV31Authorization(
  root = process.cwd(),
): CalendarLiveV31AuthorizationProof {
  const state = verifyCalendarLiveV31History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V31_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v31/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v31.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V31_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V31_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V31_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV31HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV31History(expected);
  process.stdout.write(
    `Calendar live v31 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
