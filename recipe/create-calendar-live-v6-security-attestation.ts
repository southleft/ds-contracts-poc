...
import { verifyCalendarLiveV6Authorization } from "./calendar-live-v6-authorization.js";
import { CALENDAR_LIVE_V6_TARGET } from "./calendar-live-v6-broker.js";
import {
  CALENDAR_LIVE_V6_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV6Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v6-session";
...
const attestation: CalendarLiveV6SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV6SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v6 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V6_SECURITY_ATTESTATION_DEFAULT_PATH,
);
