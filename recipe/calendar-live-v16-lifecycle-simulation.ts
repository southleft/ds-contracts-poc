...
import { calendarLiveV1Sha256 } from "./calendar-live-v16-broker.js";

export interface CalendarLiveV16LifecycleSimulationReport {
  artifactVersion: "calendar-live-v16-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV16AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV16Lifecycle(): Promise<CalendarLiveV16LifecycleSimulationReport> {
  await buildCalendarLiveV16Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V16_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV16AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v16-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V16_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V16_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV16History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV16History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV16AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V16_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V16_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV16History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV16History(authorized, "pending");
    ...
    await buildCalendarLiveV16Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V16_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV16AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV16Lifecycle(), null, 2)}\n`,
  );
