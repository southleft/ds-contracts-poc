...
import {
  ...
  INPUT_LIVE_V12_INDEX_PATH,
} from "./build-input-field-live-proof-v12.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV12HistoryExpectation = "pending" | "authorized";

export interface InputLiveV12AuthorizationArtifact {
  artifactVersion: "input-live-v12-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V12_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV12AntecedentIndex {
...
export interface InputLiveV12HistoryState {
  index?: InputLiveV12AntecedentIndex;
  indexSha256: string;
...
export interface InputLiveV12AuthorizationProof extends InputLiveV12TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V12_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V12_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V12_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V12_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v12-status") ||
...
export function validateInputLiveV12AntecedentIndex(
  index: InputLiveV12AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v12-antecedent-index-v1" ||
...
export function validateInputLiveV12UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV12History\(\s*["']/.test(source) ||
    /readInputLiveV12HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV12AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV12AuthorizationArtifact {
  ...
  ) as InputLiveV12AntecedentIndex;
  const indexFailures = validateInputLiveV12AntecedentIndex(index);
...
export function validateInputLiveV12AuthorizationArtifact(
  artifact: InputLiveV12AuthorizationArtifact | undefined,
  index: InputLiveV12AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v12-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v12" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V12_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V12_TARGET) ||
    !equalJson(
...
export function validateInputLiveV12History(
  state: InputLiveV12HistoryState,
  expected: InputLiveV12HistoryExpectation,
): string[] {
  const failures = validateInputLiveV12AntecedentIndex(state.index);
...
export function readInputLiveV12HistoryState(
  root = process.cwd(),
): InputLiveV12HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V12_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV12AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V12_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V12_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V12_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV12History(
  expected: InputLiveV12HistoryExpectation,
  root = process.cwd(),
): InputLiveV12HistoryState {
  const state = readInputLiveV12HistoryState(root);
  const failures = validateInputLiveV12History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV12Authorization(
  root = process.cwd(),
): InputLiveV12AuthorizationProof {
  const state = verifyInputLiveV12History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(path.join(repositoryRoot, INPUT_LIVE_V12_AUTHORIZATION_PATH)),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v12/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v12.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V12_TARGET,
    expectedDynamicTool: INPUT_LIVE_V12_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V12_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV12HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV12History(expected);
