...
import { verifyCalendarLiveV50Authorization } from "./calendar-live-v50-authorization.js";
import { CALENDAR_LIVE_V50_TARGET } from "./calendar-live-v50-broker.js";
import {
  CALENDAR_LIVE_V50_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV50Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v50-session";
...
const attestation: CalendarLiveV50SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV50SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v50 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V50_SECURITY_ATTESTATION_DEFAULT_PATH,
);
