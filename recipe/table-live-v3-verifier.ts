...
import {
  verifyTableLiveV3FixedSceneFixedPoint,
  type TableLiveV3FixedFixedPoint,
} from "./table-live-v3-fixed-point.js";

export const TABLE_LIVE_V3_VERIFIER_VERSION = "table-live-v3-verifier-v1";

export function buildTableLiveV3RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV3RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV3RawNode[];
}

export interface TableLiveV3NormalizedScene {
...
export interface TableLiveV3SceneProof {
...
export function normalizeTableLiveV3Scene(
  root: TableLiveV3RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV3NormalizedScene {
  const canonicalBindings: TableLiveV3NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV3RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV3Scene<Instance>(
  normalized: TableLiveV3NormalizedScene,
...
): TableLiveV3SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV3FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV3ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV3PreCaptureGates(payload: unknown): void {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("table live v3 pre-capture gate payload absent");
