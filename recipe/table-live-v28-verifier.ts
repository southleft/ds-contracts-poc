...
import {
  verifyTableLiveV28FixedSceneFixedPoint,
  type TableLiveV28FixedFixedPoint,
} from "./table-live-v28-fixed-point.js";

export const TABLE_LIVE_V28_VERIFIER_VERSION = "table-live-v28-verifier-v1";

export function buildTableLiveV28RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV28RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV28RawNode[];
}

export interface TableLiveV28NormalizedScene {
...
export interface TableLiveV28SceneProof {
...
export function normalizeTableLiveV28Scene(
  root: TableLiveV28RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV28NormalizedScene {
  const canonicalBindings: TableLiveV28NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV28RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV28Scene<Instance>(
  normalized: TableLiveV28NormalizedScene,
...
): TableLiveV28SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV28FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV28ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV28PreCaptureGates(payload: unknown): void {
