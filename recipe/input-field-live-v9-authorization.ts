...
export type InputLiveV9HistoryExpectation = "pending" | "authorized";

export interface InputLiveV9AuthorizationArtifact {
  artifactVersion: "input-live-v9-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V9_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV9AntecedentIndex {
...
export interface InputLiveV9HistoryState {
  ...
  authorization?: InputLiveV9AuthorizationArtifact;
...
export interface InputLiveV9AuthorizationProof extends InputLiveV9TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V9_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V9_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V9_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V9_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v9-status") ||
...
export function validateInputLiveV9AntecedentIndex(
  index: InputLiveV9AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v9-antecedent-index-v1" ||
...
export function validateInputLiveV9UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV9History\(\s*["']/.test(source) ||
    /readInputLiveV9HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV9AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV9AuthorizationArtifact {
  ...
  const indexFailures = validateInputLiveV9AntecedentIndex(index);
...
export function validateInputLiveV9AuthorizationArtifact(
  artifact: InputLiveV9AuthorizationArtifact | undefined,
  index: InputLiveV9AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v9-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v9" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V9_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V9_TARGET) ||
    !equalJson(
...
export function validateInputLiveV9History(
  state: InputLiveV9HistoryState,
  expected: InputLiveV9HistoryExpectation,
): string[] {
  const failures = validateInputLiveV9AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV9AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV9HistoryState(
  root = process.cwd(),
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V9_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV9AuthorizationArtifact)
    : undefined;
  ...
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V9_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V9_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV9History(
  expected: InputLiveV9HistoryExpectation,
  root = process.cwd(),
): InputLiveV9HistoryState {
  const state = readInputLiveV9HistoryState(root);
  const failures = validateInputLiveV9History(state, expected);
  if (failures.length)
    throw new Error(
      `Input live v9 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyInputLiveV9Authorization(
  root = process.cwd(),
): InputLiveV9AuthorizationProof {
  const state = verifyInputLiveV9History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(path.join(repositoryRoot, INPUT_LIVE_V9_AUTHORIZATION_PATH)),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v9/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v9.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V9_TARGET,
    expectedDynamicTool: INPUT_LIVE_V9_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V9_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV9HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV9History(expected);
  process.stdout.write(
    `Input live v9 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
