...
import { verifyComboboxLiveV4Authorization } from "./combobox-live-v4-authorization.js";
import { COMBOBOX_LIVE_V4_TARGET } from "./combobox-live-v4-broker.js";
import {
  COMBOBOX_LIVE_V4_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV4Authorization();
...
const attestation: ComboboxLiveV4SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV4AttestationSha256(body),
};
const failures = validateComboboxLiveV4SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v4 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V4_SECURITY_ATTESTATION_DEFAULT_PATH,
);
