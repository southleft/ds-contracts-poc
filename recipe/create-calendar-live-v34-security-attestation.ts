...
import { verifyCalendarLiveV34Authorization } from "./calendar-live-v34-authorization.js";
import { CALENDAR_LIVE_V34_TARGET } from "./calendar-live-v34-broker.js";
import {
  CALENDAR_LIVE_V34_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV34Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v34-session";
...
const attestation: CalendarLiveV34SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV34SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v34 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V34_SECURITY_ATTESTATION_DEFAULT_PATH,
);
