/**
 * Calendar live v38 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V38_FIXED_VERIFIER_VERSION = "calendar-live-v38-fixed-point-v1";
export const CALENDAR_LIVE_V38_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV38FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV38FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV38FixedSourceProbe {
...
export interface CalendarLiveV38FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV38FixedFixedPoint;
}

export interface CalendarLiveV38FixedHardGateInput {
  thresholds: CalendarLiveV38FixedThresholds;
  sourceProbes: CalendarLiveV38FixedSourceProbe[];
  cells: CalendarLiveV38FixedCellValidation[];
  sceneProofs: CalendarLiveV38FixedSceneProof[];
...
export interface CalendarLiveV38FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V38_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV38FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV38FixedHardGates(
  input: CalendarLiveV38FixedHardGateInput,
): CalendarLiveV38FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV38FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V38_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV38FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV38FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV38FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
