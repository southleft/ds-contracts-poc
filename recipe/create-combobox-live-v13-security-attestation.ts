...
import { verifyComboboxLiveV13Authorization } from "./combobox-live-v13-authorization.js";
import { COMBOBOX_LIVE_V13_TARGET } from "./combobox-live-v13-broker.js";
import {
  COMBOBOX_LIVE_V13_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV13Authorization();
...
const attestation: ComboboxLiveV13SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV13SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v13 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V13_SECURITY_ATTESTATION_DEFAULT_PATH,
);
