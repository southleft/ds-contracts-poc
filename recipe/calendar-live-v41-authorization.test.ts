...
import { CALENDAR_LIVE_V41_INDEX_PATH } from "./build-calendar-live-proof-v41.js";
import {
...
import {
  ...
  validateCalendarLiveV41UnitTestSource,
  type CalendarLiveV41AntecedentIndex,
  type CalendarLiveV41AuthorizationArtifact,
...
import {
  ...
  validateCalendarLiveV41ControlFlowSource,
  validateCalendarLiveV41SecurityAttestation,
  type CalendarLiveV41SecurityAttestation,
} from "./calendar-live-v41-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V41_INDEX_PATH);
const index = (): CalendarLiveV41AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV41AntecedentIndex;

const artifact = (): CalendarLiveV41AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV41AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV41HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV41HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV41AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v41/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV41AuthorizationArtifact,
): CalendarLiveV41SecurityAttestation => {
  const body: Omit<CalendarLiveV41SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V41_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV41History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV41History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV41History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV41History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV41History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV41UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v41-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV41UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV41AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV41AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV41AuthorizationArtifact(
        planted as CalendarLiveV41AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV41SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV41SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV41SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v41-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV41ControlFlowSource(source), []);
});
