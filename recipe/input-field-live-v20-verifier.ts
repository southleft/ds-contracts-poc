...
export const INPUT_LIVE_V20_VERIFIER_VERSION = "input-live-v20-verifier-v1";

export function buildInputLiveV20RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV20RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV20RawNode[];
}

export interface InputLiveV20NormalizedScene {
...
export interface InputLiveV20SceneProof {
...
export function normalizeInputLiveV20Scene(
  root: InputLiveV20RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV20NormalizedScene {
  const canonicalBindings: InputLiveV20NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV20RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV20Scene<Instance>(
  normalized: InputLiveV20NormalizedScene,
...
export function compileInputLiveV20ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV20PreCaptureGates(payload: unknown): void {
