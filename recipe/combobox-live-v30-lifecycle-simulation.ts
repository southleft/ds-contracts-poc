...
export interface ComboboxLiveV30LifecycleSimulationReport {
  artifactVersion: "combobox-live-v30-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV30AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV30Lifecycle(): Promise<ComboboxLiveV30LifecycleSimulationReport> {
  await buildComboboxLiveV30Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V30_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV30AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v30-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V30_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V30_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV30History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV30History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV30AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V30_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V30_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV30History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV30History(authorized, "pending");
    ...
    await buildComboboxLiveV30Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V30_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV30AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV30Lifecycle(), null, 2)}\n`,
  );
