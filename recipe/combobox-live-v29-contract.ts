...
import { buildComboboxLiveV29CleanupRuntime } from "./combobox-live-v29-cleanup.js";
import {
  type ComboboxLiveV29FixedFixedPoint,
} from "./combobox-live-v29-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV29RestoreProgram,
  validateComboboxLiveV29RestorePayload,
  type ComboboxLiveV29RestorePayload,
} from "./combobox-live-v29-restore.js";
import {
  buildComboboxLiveV29RawPropertyRuntime,
  normalizeComboboxLiveV29Scene,
  type ComboboxLiveV29RawNode,
} from "./combobox-live-v29-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v29.js";
import {
...
export const COMBOBOX_LIVE_V29_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V29_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V29_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V29_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V29_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V29_SET_COUNT = 4;
export const COMBOBOX_LIVE_V29_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V29_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V29_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V29_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V29_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV29SourceId = (typeof COMBOBOX_LIVE_V29_SOURCE_IDS)[number];

export interface ComboboxLiveV29SourceIdentity {
  source: ComboboxLiveV29SourceId;
...
export interface ComboboxLiveV29WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV29ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV29SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV29RawNode;
    optionScene: ComboboxLiveV29RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV29RootProof {
  source: ComboboxLiveV29SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV29ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV29SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV29CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV29SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV29CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV29SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV29CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v29 ${label} must be an object`);
...
export function validateComboboxLiveV29WriterPayload(
  value: unknown,
): ComboboxLiveV29WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V29_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v29 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v29 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v29 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV29SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV29ExtractProgram(
  writer: ComboboxLiveV29WriterOwnership,
  sources: readonly ComboboxLiveV29SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v29 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V29_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V29_NAMESPACE)}
${buildComboboxLiveV29RawPropertyRuntime()}
...
export function validateComboboxLiveV29ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV29WriterOwnership,
): ComboboxLiveV29ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v29 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V29_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v29 extract schema/two-root mismatch");
  return payload as ComboboxLiveV29ExtractPayload;
}

export function proveComboboxLiveV29Roots<Instance>(
  extract: ComboboxLiveV29ExtractPayload,
  sources: readonly (ComboboxLiveV29SourceIdentity & {
...
): ComboboxLiveV29RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v29 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v29 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV29Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV29Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV29FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v29 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v29 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v29 compile lost owned sets");
...
export function assertComboboxLiveV29RootProofs(
  proofs: readonly ComboboxLiveV29RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v29 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v29 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV29ProbeProgram(
  writer: ComboboxLiveV29WriterOwnership,
  sources: readonly ComboboxLiveV29SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V29_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV29ProbePayload(
  value: unknown,
  writer: ComboboxLiveV29WriterOwnership,
): ComboboxLiveV29ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV29ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V29_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V29_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v29 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV29CaptureManifest(
  cells: readonly ComboboxLiveV29CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V29_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V29_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V29_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V29_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v29 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV29CaptureCell[],
): string => {
  validateComboboxLiveV29CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV29CaptureProgram(
  writer: ComboboxLiveV29WriterOwnership,
  cell: ComboboxLiveV29CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V29_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v29 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v29 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V29_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV29CapturePayload(
  value: unknown,
  cell: ComboboxLiveV29CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV29CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV29CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V29_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V29_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v29 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV29CaptureResponses(
  manifest: readonly ComboboxLiveV29CaptureCell[],
  responses: readonly ComboboxLiveV29CapturePayload[],
): void {
  validateComboboxLiveV29CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v29 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV29ObjectiveReport {
  artifactVersion: "combobox-live-v29-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV29Objective(
  manifest: readonly ComboboxLiveV29CaptureCell[],
  responses: readonly ComboboxLiveV29CapturePayload[],
): ComboboxLiveV29ObjectiveReport {
  assertComboboxLiveV29CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v29-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV29CleanupProgram(
  writer: ComboboxLiveV29WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV29CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V29_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v29 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV29CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV29WriterOwnership,
): ComboboxLiveV29CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV29CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v29 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V29_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV29WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
