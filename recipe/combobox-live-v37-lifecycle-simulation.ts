...
export interface ComboboxLiveV37LifecycleSimulationReport {
  artifactVersion: "combobox-live-v37-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV37AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV37Lifecycle(): Promise<ComboboxLiveV37LifecycleSimulationReport> {
  await buildComboboxLiveV37Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V37_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV37AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v37-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V37_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V37_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV37History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV37History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV37AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V37_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V37_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV37History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV37History(authorized, "pending");
    ...
    await buildComboboxLiveV37Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V37_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV37AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV37Lifecycle(), null, 2)}\n`,
  );
