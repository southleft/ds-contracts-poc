...
import { INPUT_LIVE_V66_INDEX_PATH } from "./build-input-field-live-proof-v66.js";
import {
  INPUT_LIVE_V66_DYNAMIC_TOOL,
  INPUT_LIVE_V66_TARGET,
} from "./input-field-live-v66-broker.js";
import {
  buildInputLiveV66AuthorizationArtifact,
  validateInputLiveV66AntecedentIndex,
  validateInputLiveV66AuthorizationArtifact,
  validateInputLiveV66History,
  validateInputLiveV66UnitTestSource,
  type InputLiveV66AntecedentIndex,
  type InputLiveV66AuthorizationArtifact,
  type InputLiveV66AuthorizationProof,
  type InputLiveV66HistoryState,
} from "./input-field-live-v66-authorization.js";
import {
  inputLiveV66AttestationSha256,
  validateInputLiveV66ControlFlowSource,
  validateInputLiveV66SecurityAttestation,
  type InputLiveV66SecurityAttestation,
} from "./input-field-live-v66-preflight.js";
import { inputLiveV66Sha256 } from "./input-field-live-v66-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V66_INDEX_PATH);
const index = (): InputLiveV66AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV66AntecedentIndex;

const artifact = (): InputLiveV66AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV66AuthorizationArtifact({
...
const pendingState = (): InputLiveV66HistoryState => ({
  index: index(),
  indexSha256: inputLiveV66Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV66HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV66AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV66Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V66_TARGET,
  expectedDynamicTool: INPUT_LIVE_V66_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v66/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV66AuthorizationArtifact,
): InputLiveV66SecurityAttestation => {
  const body: Omit<InputLiveV66SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v66-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V66_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV66AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV66History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV66History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV66History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV66History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV66History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV66UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV66UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV66AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV66Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV66AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV66AuthorizationArtifact(
        planted as InputLiveV66AuthorizationArtifact,
        index(),
        inputLiveV66Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV66SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV66AttestationSha256(
      body as Omit<InputLiveV66SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV66SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV66ControlFlowSource(source), []);
});
