...
/**
 * Carried Input live v25 scene-readback. Keeps the v17/v18 slot-fill
 * and slot-color teachings, the v19 uniform per-side stroke-weight
 * teaching, the v20 variant layout-width teaching, the v21 surface
 * layout-height teaching, the v22 no-duplicate alias, and the v23
 * compile-index width alias, then drops extra surface bindings that
 * collapse treats as a structural length edit: duplicate mapped
 * `fills.0` / `strokes.0` color fields, and per-side stroke weights
 * once the uniform `strokes.0.weight` alias is present. Surface role
 * only. Does not invent variables, colors, or treat FIXED as a fill.
 */
...
export const INPUT_LIVE_V25_SLOT_FILL_MARKER = "INPUT-SLOT-FILL-FROM-PAYLOAD";
export const INPUT_LIVE_V25_SLOT_COLOR_BINDING_MARKER =
  "INPUT-SLOT-COLOR-BINDING-FROM-CHILD";
export const INPUT_LIVE_V25_SURFACE_STROKE_WEIGHT_MARKER =
  "INPUT-SURFACE-STROKE-WEIGHT-FROM-PER-SIDE";
export const INPUT_LIVE_V25_VARIANT_LAYOUT_WIDTH_MARKER =
  "INPUT-VARIANT-LAYOUT-WIDTH-FROM-WIDTH";
export const INPUT_LIVE_V25_SURFACE_LAYOUT_HEIGHT_MARKER =
  "INPUT-SURFACE-LAYOUT-HEIGHT-FROM-HEIGHT";
export const INPUT_LIVE_V25_VARIANT_LAYOUT_WIDTH_ALIAS_MARKER =
  "INPUT-VARIANT-LAYOUT-WIDTH-ALIAS-NO-DUPLICATE-WIDTH-VALUE";
export const INPUT_LIVE_V25_VARIANT_LAYOUT_WIDTH_COMPILE_INDEX_MARKER =
  "INPUT-VARIANT-LAYOUT-WIDTH-ALIAS-COMPILE-INDEX";
export const INPUT_LIVE_V25_SURFACE_BINDING_EXTRAS_MARKER =
  "INPUT-SURFACE-BINDING-EXTRAS-DROPPED";
export const INPUT_LIVE_V25_SURFACE_BINDING_COMPILE_ORDER_MARKER =
  "INPUT-SURFACE-BINDING-COMPILE-ORDER";
export const INPUT_LIVE_V25_SURFACE_LAYOUT_HEIGHT_ALIAS_MARKER =
  "INPUT-SURFACE-LAYOUT-HEIGHT-ALIAS-NO-DUPLICATE-HEIGHT-VALUE";
export const INPUT_LIVE_V25_SURFACE_COMPILE_BINDING_FIELDS = [
...
export const INPUT_LIVE_V25_SLOT_ROLES = [
  "input-field/slot/leading",
  "input-field/slot/trailing",
] as const;
export const INPUT_LIVE_V25_SURFACE_ROLES = ["input-field/surface"] as const;
export const INPUT_LIVE_V25_VARIANT_ROLE_PREFIX = "input-field/variant/";
export const INPUT_LIVE_V25_PER_SIDE_STROKE_WEIGHT_FIELDS = [
  "strokeTopWeight",
  "strokeRightWeight",
  "strokeBottomWeight",
  "strokeLeftWeight",
...
const SURFACE_ROLES = new Set<string>(INPUT_LIVE_V25_SURFACE_ROLES);
...
const surfaceVariantLayoutWidthBinding = (
  ...
  if (
    role === undefined ||
    !role.startsWith(INPUT_LIVE_V25_VARIANT_ROLE_PREFIX)
  )
    return own;
...
const orderSurfaceBindingsToCompileFields = (
  ...
  if (role === undefined || !SURFACE_ROLES.has(role)) return own;
  const rank = new Map(
    INPUT_LIVE_V25_SURFACE_COMPILE_BINDING_FIELDS.map((field, index) => [
      field,
      index,
    ]),
  );
...
const SLOT_ROLES = new Set<string>(INPUT_LIVE_V25_SLOT_ROLES);
...
export function sceneToNormalizedIr(scene: SceneNodeSnapshot): IRNode {
  const bindings = orderSurfaceBindingsToCompileFields(
    scene,
