...
export const INPUT_LIVE_V11_VERIFIER_VERSION = "input-live-v11-verifier-v1";

export function buildInputLiveV11RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV11RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV11RawNode[];
}

export interface InputLiveV11NormalizedScene {
...
export interface InputLiveV11SceneProof {
...
export function normalizeInputLiveV11Scene(
  root: InputLiveV11RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV11NormalizedScene {
  const canonicalBindings: InputLiveV11NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: InputLiveV11RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV11Scene<Instance>(
  normalized: InputLiveV11NormalizedScene,
...
export function compileInputLiveV11ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV11PreCaptureGates(payload: unknown): void {
