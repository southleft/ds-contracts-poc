...
import { INPUT_LIVE_V54_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v54.js";
import { verifyInputLiveV54Authorization } from "./input-field-live-v54-authorization.js";
import { runInputLiveV54Preflight } from "./input-field-live-v54-preflight.js";
import { InputLiveV54Orchestrator } from "./run-input-field-live-v54.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V54_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v54-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV54Authorization(root);
    runInputLiveV54Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV54Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
