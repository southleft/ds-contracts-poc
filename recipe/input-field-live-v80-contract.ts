...
} from "./input-field-live-v3-verifier-v80.js";
import {
  buildInputLiveV80RawPropertyRuntime,
  normalizeInputLiveV80Scene,
  type InputLiveV80RawNode,
} from "./input-field-live-v80-verifier.js";
...
export {
  ...
  validateInputLiveV80RestorePayload,
  type InputLiveV80RestorePayload,
} from "./input-field-live-v80-restore.js";

export const INPUT_LIVE_V80_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V80_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V80_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V80_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V80_VARIANT_COUNT = 256;
export const INPUT_LIVE_V80_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V80_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV80SourceId = (typeof INPUT_LIVE_V80_SOURCE_IDS)[number];

export interface InputLiveV80SourceIdentity {
  source: InputLiveV80SourceId;
...
export interface InputLiveV80WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV80ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV80SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV80RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV80RootProof {
  source: InputLiveV80SourceId;
  adapterIdentity: string;
...
export interface InputLiveV80ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV80SourceId;
    adapterIdentity: string;
...
export interface InputLiveV80CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV80SourceId;
  adapterIdentity: string;
...
export interface InputLiveV80CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV80SourceId;
  strata: InputLiveV80CaptureCell["strata"];
...
export interface InputLiveV80CleanupPayload {
...
export function validateInputLiveV80WriterPayload(
  value: unknown,
): InputLiveV80WriterOwnership {
...
const identityPlan = (source: InputLiveV80SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV80ExtractProgram(
  writer: InputLiveV80WriterOwnership,
  sources: readonly InputLiveV80SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V80_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V80_NAMESPACE)}
${buildInputLiveV80RawPropertyRuntime()}
...
export function validateInputLiveV80ExtractPayload(
  value: unknown,
  writer: InputLiveV80WriterOwnership,
): InputLiveV80ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V80_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV80ExtractPayload;
}

export function proveInputLiveV80Roots<Instance>(
  extract: InputLiveV80ExtractPayload,
  sources: readonly (InputLiveV80SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV80Scene(
      root.scene,
...
export function assertInputLiveV80RootProofs(
  proofs: readonly InputLiveV80RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV80ProbeProgram(
  writer: InputLiveV80WriterOwnership,
  sources: readonly InputLiveV80SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V80_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV80ProbePayload(
  value: unknown,
  writer: InputLiveV80WriterOwnership,
): InputLiveV80ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV80ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V80_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V80_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV80CaptureManifest(
  cells: readonly InputLiveV80CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V80_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V80_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V80_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V80_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V80_SOURCE_IDS.some(
...
export const inputLiveV80CaptureManifestSha256 = (
  cells: readonly InputLiveV80CaptureCell[],
): string => {
  validateInputLiveV80CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV80CaptureProgram(
  writer: InputLiveV80WriterOwnership,
  cell: InputLiveV80CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V80_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V80_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V80_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV80CapturePayload(
  value: unknown,
  cell: InputLiveV80CaptureCell,
  rawResponseBytes: number,
): InputLiveV80CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV80CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V80_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V80_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV80CaptureResponses(
  manifest: readonly InputLiveV80CaptureCell[],
  responses: readonly InputLiveV80CapturePayload[],
): void {
  validateInputLiveV80CaptureManifest(manifest);
  if (
...
export interface InputLiveV80ObjectiveReport {
  artifactVersion: "input-live-v80-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV80Objective(
  manifest: readonly InputLiveV80CaptureCell[],
  responses: readonly InputLiveV80CapturePayload[],
  readReference: (cell: InputLiveV80CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV80CaptureCell) => Buffer,
): InputLiveV80ObjectiveReport {
  assertInputLiveV80CaptureResponses(manifest, responses);
...
export function buildInputLiveV80CleanupProgram(
  writer: InputLiveV80WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V80_NAMESPACE,
...
export function validateInputLiveV80CleanupPayload(
  value: unknown,
  writer: InputLiveV80WriterOwnership,
): InputLiveV80CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV80CleanupPayload;
...
export const INPUT_LIVE_V80_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV80WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
