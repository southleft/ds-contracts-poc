...
} from "./input-field-live-v3-verifier-v68.js";
import {
  buildInputLiveV68RawPropertyRuntime,
  normalizeInputLiveV68Scene,
  type InputLiveV68RawNode,
} from "./input-field-live-v68-verifier.js";
...
export {
  ...
  type InputLiveV68RestorePayload,
} from "./input-field-live-v68-restore.js";

export const INPUT_LIVE_V68_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V68_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V68_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V68_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V68_VARIANT_COUNT = 256;
export const INPUT_LIVE_V68_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V68_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV68SourceId = (typeof INPUT_LIVE_V68_SOURCE_IDS)[number];

export interface InputLiveV68SourceIdentity {
  source: InputLiveV68SourceId;
...
export interface InputLiveV68WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV68ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV68SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV68RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV68RootProof {
  source: InputLiveV68SourceId;
  adapterIdentity: string;
...
export interface InputLiveV68ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV68SourceId;
    adapterIdentity: string;
...
export interface InputLiveV68CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV68SourceId;
  adapterIdentity: string;
...
export interface InputLiveV68CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV68SourceId;
  strata: InputLiveV68CaptureCell["strata"];
...
export interface InputLiveV68CleanupPayload {
...
export function validateInputLiveV68WriterPayload(
  value: unknown,
): InputLiveV68WriterOwnership {
...
const identityPlan = (source: InputLiveV68SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV68ExtractProgram(
  writer: InputLiveV68WriterOwnership,
  sources: readonly InputLiveV68SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V68_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V68_NAMESPACE)}
${buildInputLiveV68RawPropertyRuntime()}
...
export function validateInputLiveV68ExtractPayload(
  value: unknown,
  writer: InputLiveV68WriterOwnership,
): InputLiveV68ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V68_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV68ExtractPayload;
}

export function proveInputLiveV68Roots<Instance>(
  extract: InputLiveV68ExtractPayload,
  sources: readonly (InputLiveV68SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV68Scene(
      root.scene,
...
export function assertInputLiveV68RootProofs(
  proofs: readonly InputLiveV68RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV68ProbeProgram(
  writer: InputLiveV68WriterOwnership,
  sources: readonly InputLiveV68SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V68_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV68ProbePayload(
  value: unknown,
  writer: InputLiveV68WriterOwnership,
): InputLiveV68ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV68ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V68_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V68_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV68CaptureManifest(
  cells: readonly InputLiveV68CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V68_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V68_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V68_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V68_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V68_SOURCE_IDS.some(
...
export const inputLiveV68CaptureManifestSha256 = (
  cells: readonly InputLiveV68CaptureCell[],
): string => {
  validateInputLiveV68CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV68CaptureProgram(
  writer: InputLiveV68WriterOwnership,
  cell: InputLiveV68CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V68_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V68_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V68_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV68CapturePayload(
  value: unknown,
  cell: InputLiveV68CaptureCell,
  rawResponseBytes: number,
): InputLiveV68CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV68CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V68_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V68_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV68CaptureResponses(
  manifest: readonly InputLiveV68CaptureCell[],
  responses: readonly InputLiveV68CapturePayload[],
): void {
  validateInputLiveV68CaptureManifest(manifest);
  if (
...
export interface InputLiveV68ObjectiveReport {
  artifactVersion: "input-live-v68-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV68Objective(
  manifest: readonly InputLiveV68CaptureCell[],
  responses: readonly InputLiveV68CapturePayload[],
  readReference: (cell: InputLiveV68CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV68CaptureCell) => Buffer,
): InputLiveV68ObjectiveReport {
  assertInputLiveV68CaptureResponses(manifest, responses);
...
export function buildInputLiveV68CleanupProgram(
  writer: InputLiveV68WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V68_NAMESPACE,
...
export function validateInputLiveV68CleanupPayload(
  value: unknown,
  writer: InputLiveV68WriterOwnership,
): InputLiveV68CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV68CleanupPayload;
...
export const INPUT_LIVE_V68_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV68WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
