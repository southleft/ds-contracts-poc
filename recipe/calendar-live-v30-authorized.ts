...
import { CALENDAR_LIVE_V30_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v30.js";
import { verifyCalendarLiveV30Authorization } from "./calendar-live-v30-authorization.js";
import { runCalendarLiveV30Preflight } from "./calendar-live-v30-preflight.js";
import { CalendarLiveV30Orchestrator } from "./run-calendar-live-v30.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v30 requires exact authorization artifact ${CALENDAR_LIVE_V30_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v30-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV30Authorization(root);
    runCalendarLiveV30Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV30Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV30Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
