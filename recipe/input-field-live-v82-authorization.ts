...
import {
  ...
  INPUT_LIVE_V82_INDEX_PATH,
} from "./build-input-field-live-proof-v82.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV82HistoryExpectation = "pending" | "authorized";

export interface InputLiveV82AuthorizationArtifact {
  artifactVersion: "input-live-v82-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V82_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV82AntecedentIndex {
...
export interface InputLiveV82HistoryState {
  index?: InputLiveV82AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV82AuthorizationArtifact;
...
export interface InputLiveV82AuthorizationProof extends InputLiveV82TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V82_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V82_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V82_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V82_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v82-authorization") ||
  artifactPath.includes("input-field-live-v82-preflight") ||
  artifactPath.includes("input-field-live-v82-authorized") ||
  artifactPath.includes("input-field-live-pivot-v82-status") ||
...
export function validateInputLiveV82AntecedentIndex(
  index: InputLiveV82AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v82-antecedent-index-v1" ||
...
export function validateInputLiveV82UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV82History\(\s*["']/.test(source) ||
    /readInputLiveV82HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV82AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV82AuthorizationArtifact {
  ...
  ) as InputLiveV82AntecedentIndex;
  const indexFailures = validateInputLiveV82AntecedentIndex(index);
...
export function validateInputLiveV82AuthorizationArtifact(
  artifact: InputLiveV82AuthorizationArtifact | undefined,
  index: InputLiveV82AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v82-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v82" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V82_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V82_TARGET) ||
    !equalJson(
...
export function validateInputLiveV82History(
  state: InputLiveV82HistoryState,
  expected: InputLiveV82HistoryExpectation,
): string[] {
  const failures = validateInputLiveV82AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV82AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV82HistoryState(
  root = process.cwd(),
): InputLiveV82HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V82_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV82AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V82_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V82_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V82_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV82History(
  expected: InputLiveV82HistoryExpectation,
  root = process.cwd(),
): InputLiveV82HistoryState {
  const state = readInputLiveV82HistoryState(root);
  const failures = validateInputLiveV82History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV82Authorization(
  root = process.cwd(),
): InputLiveV82AuthorizationProof {
  const state = verifyInputLiveV82History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V82_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v82/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v82.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V82_TARGET,
    expectedDynamicTool: INPUT_LIVE_V82_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V82_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV82HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV82History(expected);
