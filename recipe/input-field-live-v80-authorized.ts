...
import { INPUT_LIVE_V80_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v80.js";
import { verifyInputLiveV80Authorization } from "./input-field-live-v80-authorization.js";
import { runInputLiveV80Preflight } from "./input-field-live-v80-preflight.js";
import { InputLiveV80Orchestrator } from "./run-input-field-live-v80.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V80_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v80-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV80Authorization(root);
    runInputLiveV80Preflight(
      root,
      authorization,
    ...
    const orchestrator = InputLiveV80Orchestrator.initialize({
      root,
  ...
  const orchestrator = InputLiveV80Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
