...
export const INPUT_LIVE_V38_VERIFIER_VERSION = "input-live-v38-verifier-v1";

export function buildInputLiveV38RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV38RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV38RawNode[];
}

export interface InputLiveV38NormalizedScene {
...
export interface InputLiveV38SceneProof {
...
export function normalizeInputLiveV38Scene(
  root: InputLiveV38RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV38NormalizedScene {
  const canonicalBindings: InputLiveV38NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV38RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV38Scene<Instance>(
  normalized: InputLiveV38NormalizedScene,
...
export function compileInputLiveV38ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV38PreCaptureGates(payload: unknown): void {
