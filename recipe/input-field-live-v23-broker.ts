...
import {
  assertInputLiveV23RootProofs,
  INPUT_LIVE_V23_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V23_RESPONSE_CONTRACTS,
  type InputLiveV23RootProof,
} from "./input-field-live-v23-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V23_BROKER_VERSION = "input-live-v23-broker-v2";
export const INPUT_LIVE_V23_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V23_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V23_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V23_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V23_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V23_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV23RemotePhase =
  (typeof INPUT_LIVE_V23_REMOTE_PHASES)[number];
export type InputLiveV23AuthorizationMode = "live" | "simulated";

export interface InputLiveV23CommitPins {
...
export interface InputLiveV23TransactionAuthorization extends InputLiveV23CommitPins {
  mode: InputLiveV23AuthorizationMode;
...
export interface InputLiveV23Manifest {
  artifactVersion: "input-live-v23-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V23_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V23_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V23_TARGET;
  authorization: InputLiveV23TransactionAuthorization;
...
export interface InputLiveV23ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV23RequestBody {
  artifactVersion: "input-live-v23-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V23_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV23RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV23CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V23_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV23ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV23Request extends InputLiveV23RequestBody {
...
export interface InputLiveV23Receipt {
  artifactVersion: "input-live-v23-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V23_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV23RemotePhase;
...
export interface InputLiveV23AcceptedResponse<T = unknown> {
  request: InputLiveV23Request;
  receipt: InputLiveV23Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV23IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV23ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V23_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V23_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V23_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V23_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V23_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V23_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V23_TARGET.editorType;
};
...
export const inputLiveV23RequestSequence = (
  phase: InputLiveV23RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV23RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV23RequestPath = (
  directory: string,
  phase: InputLiveV23RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV23ResponsePath = (
  directory: string,
  phase: InputLiveV23RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV23ReceiptPath = (
  directory: string,
  phase: InputLiveV23RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV23Manifest => {
  const manifest = json<InputLiveV23Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v23-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V23_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V23_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV23RemotePhase,
  captureIndex?: number,
): InputLiveV23Request | undefined => {
  const file = inputLiveV23RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV23Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV23RemotePhase,
  captureIndex?: number,
): InputLiveV23Receipt | undefined => {
  const file = inputLiveV23ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV23Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV23Request | undefined;
  receipt: InputLiveV23Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV23RemotePhase,
): InputLiveV23ResponseContract => ({
  ...INPUT_LIVE_V23_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V23_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V23_DYNAMIC_TOOL,
      target: INPUT_LIVE_V23_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v23-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V23_BROKER_VERSION,
  target: INPUT_LIVE_V23_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V23_PROGRAM_START*/${program}/*INPUT_LIVE_V23_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V23_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V23_PROGRAM_END*/";
...
export function createInputLiveV23Transaction(
  directory: string,
  ...
  const manifest: InputLiveV23Manifest = {
    artifactVersion: "input-live-v23-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V23_BROKER_VERSION,
...
export function validateInputLiveV23Request(
  directory: string,
  request: InputLiveV23Request,
): void {
  ...
  const expectedSequence = inputLiveV23RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v23-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V23_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V23_TARGET.fileKey ||
...
export function issueInputLiveV23Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV23RemotePhase,
  program: string,
  options: InputLiveV23IssueOptions = {},
): InputLiveV23Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV23RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV23Request>(targetPath);
    validateInputLiveV23Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV23ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV23TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV23RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V23_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V23_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V23_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV23RequestBody = {
    artifactVersion: "input-live-v23-broker-request-v2",
    brokerVersion: INPUT_LIVE_V23_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV23Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV23Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV23RawResponse(
  directory: string,
  phase: InputLiveV23RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV23RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV23Request>(requestFile);
  if (
  ...
  const target = inputLiveV23ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV23Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v23-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V23_BROKER_VERSION,
...
export function acceptInputLiveV23Response<T = unknown>(
  directory: string,
  phase: InputLiveV23RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV23AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV23ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV23Request>(
    inputLiveV23RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV23Request(directory, request);
  if (phase === "capture") validateInputLiveV23TechnicalGates(directory);
  const rawFile = inputLiveV23ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v23-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V23_BROKER_VERSION ||
...
export function persistInputLiveV23TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV23RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV23RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V23_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v23-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V23_BROKER_VERSION,
...
export function validateInputLiveV23TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v23-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V23_TECHNICAL_GATES.some(
...
export function validateInputLiveV23Receipt(
  directory: string,
  phase: InputLiveV23RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV23Request(directory, request);
  const receipt = json<InputLiveV23Receipt>(
    inputLiveV23ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV23ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v23-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V23_BROKER_VERSION ||
...
export function readInputLiveV23AcceptedRawPayload(
  directory: string,
  phase: InputLiveV23RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV23Receipt(directory, phase, captureIndex);
...
export function createInputLiveV23FakeDynamicResponse(
  request: InputLiveV23Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V23_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V23_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV23PersistedRequests(
  directory: string,
): InputLiveV23Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV23Request>(file)] : [];
    });
}

export const inputLiveV23Sha256 = sha256;
