/**
 * Table live v7 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V7_FIXED_VERIFIER_VERSION = "table-live-v7-fixed-point-v1";
export const TABLE_LIVE_V7_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV7FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV7FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV7FixedSourceProbe {
...
export interface TableLiveV7FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV7FixedFixedPoint;
}

export interface TableLiveV7FixedHardGateInput {
  thresholds: TableLiveV7FixedThresholds;
  sourceProbes: TableLiveV7FixedSourceProbe[];
  cells: TableLiveV7FixedCellValidation[];
  sceneProofs: TableLiveV7FixedSceneProof[];
...
export interface TableLiveV7FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V7_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV7FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV7FixedHardGates(
  input: TableLiveV7FixedHardGateInput,
): TableLiveV7FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV7FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  if (input.visualRows.length !== 0)
    failures.push("table live v7 must not invent a visual comparison");
  ...
  return {
    verifierVersion: TABLE_LIVE_V7_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV7FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV7FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
