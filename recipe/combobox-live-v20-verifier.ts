...
import {
  verifyComboboxLiveV20FixedSceneFixedPoint,
  type ComboboxLiveV20FixedFixedPoint,
} from "./combobox-live-v20-fixed-point.js";

export const COMBOBOX_LIVE_V20_VERIFIER_VERSION = "combobox-live-v20-verifier-v1";

export function buildComboboxLiveV20RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV20RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV20RawNode[];
}

export interface ComboboxLiveV20NormalizedScene {
...
export interface ComboboxLiveV20SceneProof {
...
export function normalizeComboboxLiveV20Scene(
  root: ComboboxLiveV20RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV20NormalizedScene {
  const canonicalBindings: ComboboxLiveV20NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV20RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV20Scene<Instance>(
  normalized: ComboboxLiveV20NormalizedScene,
...
): ComboboxLiveV20SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV20FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV20ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV20PreCaptureGates(payload: unknown): void {
