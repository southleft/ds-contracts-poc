...
import { verifyCalendarLiveV47Authorization } from "./calendar-live-v47-authorization.js";
import { CALENDAR_LIVE_V47_TARGET } from "./calendar-live-v47-broker.js";
import {
  CALENDAR_LIVE_V47_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV47Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v47-session";
...
const attestation: CalendarLiveV47SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV47SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v47 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V47_SECURITY_ATTESTATION_DEFAULT_PATH,
);
