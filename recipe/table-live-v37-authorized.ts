...
import { TABLE_LIVE_V37_AUTHORIZATION_PATH } from "./build-table-live-proof-v37.js";
import { verifyTableLiveV37Authorization } from "./table-live-v37-authorization.js";
import { runTableLiveV37Preflight } from "./table-live-v37-preflight.js";
import { TableLiveV37Orchestrator } from "./run-table-live-v37.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v37 requires exact authorization artifact ${TABLE_LIVE_V37_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v37-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV37Authorization(root);
    runTableLiveV37Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV37Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV37Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
