...
import { CALENDAR_LIVE_V29_INDEX_PATH } from "./build-calendar-live-proof-v29.js";
import {
...
import {
  buildCalendarLiveV29AuthorizationArtifact,
  validateCalendarLiveV29AntecedentIndex,
  validateCalendarLiveV29AuthorizationArtifact,
  validateCalendarLiveV29History,
  validateCalendarLiveV29UnitTestSource,
  type CalendarLiveV29AntecedentIndex,
  type CalendarLiveV29AuthorizationArtifact,
  type CalendarLiveV29AuthorizationProof,
  type CalendarLiveV29HistoryState,
} from "./calendar-live-v29-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV29ControlFlowSource,
  validateCalendarLiveV29SecurityAttestation,
  type CalendarLiveV29SecurityAttestation,
} from "./calendar-live-v29-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V29_INDEX_PATH);
const index = (): CalendarLiveV29AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV29AntecedentIndex;

const artifact = (): CalendarLiveV29AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV29AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV29HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV29HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV29AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v29/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV29AuthorizationArtifact,
): CalendarLiveV29SecurityAttestation => {
  const body: Omit<CalendarLiveV29SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V29_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV29History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV29History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV29History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV29History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV29History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV29UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v29-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV29UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV29AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV29AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV29AuthorizationArtifact(
        planted as CalendarLiveV29AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV29SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV29SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV29SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v29-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV29ControlFlowSource(source), []);
});
