...
import {
  verifyCalendarLiveV15FixedSceneFixedPoint,
  type CalendarLiveV15FixedFixedPoint,
} from "./calendar-live-v15-fixed-point.js";

export const CALENDAR_LIVE_V15_VERIFIER_VERSION = "calendar-live-v15-verifier-v1";

export function buildCalendarLiveV15RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV15RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV15RawNode[];
}

export interface CalendarLiveV15NormalizedScene {
...
export interface CalendarLiveV15SceneProof {
...
export function normalizeCalendarLiveV15Scene(
  root: CalendarLiveV15RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV15NormalizedScene {
  const canonicalBindings: CalendarLiveV15NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV15RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV15Scene<Instance>(
  normalized: CalendarLiveV15NormalizedScene,
...
): CalendarLiveV15SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV15FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV15ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV15PreCaptureGates(payload: unknown): void {
