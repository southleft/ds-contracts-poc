...
import { buildCalendarLiveV27CleanupRuntime } from "./calendar-live-v27-cleanup.js";
import { type CalendarLiveV27FixedFixedPoint } from "./calendar-live-v27-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV27RestoreProgram,
  validateCalendarLiveV27RestorePayload,
  type CalendarLiveV27RestorePayload,
} from "./calendar-live-v27-restore.js";
import {
  buildCalendarLiveV27RawPropertyRuntime,
  normalizeCalendarLiveV27Scene,
  type CalendarLiveV27RawNode,
} from "./calendar-live-v27-verifier.js";
...
export const CALENDAR_LIVE_V27_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V27_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V27_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V27_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V27_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V27_SET_COUNT = 3;
export const CALENDAR_LIVE_V27_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V27_HOST_PHASES = 3;
export const CALENDAR_LIVE_V27_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V27_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V27_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V27_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV27SourceId = (typeof CALENDAR_LIVE_V27_SOURCE_IDS)[number];
export type CalendarLiveV27Kind = "calendar" | "week" | "day";

export interface CalendarLiveV27SourceIdentity {
  source: CalendarLiveV27SourceId;
...
export interface CalendarLiveV27WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV27ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV27SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV27RawNode;
    weekScene: CalendarLiveV27RawNode;
    dayScene: CalendarLiveV27RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV27RootProof {
  source: CalendarLiveV27SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV27FixedFixedPoint;
}

export interface CalendarLiveV27ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV27SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV27CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV27SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV27CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV27SourceId;
  frameWidth: number;
...
export interface CalendarLiveV27CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v27 ${label} must be an object`);
...
export function validateCalendarLiveV27WriterPayload(
  value: unknown,
): CalendarLiveV27WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v27") ||
    ...
    result.namespace !== CALENDAR_LIVE_V27_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v27 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v27 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v27 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV27SourceIdentity,
  kind: CalendarLiveV27Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV27ExtractProgram(
  writer: CalendarLiveV27WriterOwnership,
  sources: readonly CalendarLiveV27SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v27 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V27_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V27_NAMESPACE)}
${buildCalendarLiveV27RawPropertyRuntime()}
...
export function validateCalendarLiveV27ExtractPayload(
  value: unknown,
  writer: CalendarLiveV27WriterOwnership,
): CalendarLiveV27ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v27 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V27_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v27 extract schema/one-root mismatch");
  return payload as CalendarLiveV27ExtractPayload;
}

export function proveCalendarLiveV27Roots<Instance>(
  extract: CalendarLiveV27ExtractPayload,
  sources: readonly (CalendarLiveV27SourceIdentity & {
...
): CalendarLiveV27RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v27 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v27 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV27Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV27Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV27Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV27FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v27 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v27 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v27 compile lost owned sets");
...
export function assertCalendarLiveV27RootProofs(
  proofs: readonly CalendarLiveV27RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v27 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v27 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV27ProbeProgram(
  writer: CalendarLiveV27WriterOwnership,
  sources: readonly CalendarLiveV27SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V27_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV27ProbePayload(
  value: unknown,
  writer: CalendarLiveV27WriterOwnership,
): CalendarLiveV27ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV27ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V27_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V27_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v27 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV27CaptureManifest(
  cells: readonly CalendarLiveV27CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V27_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V27_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V27_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V27_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v27 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV27CaptureCell[],
): string => {
  validateCalendarLiveV27CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV27CaptureProgram(
  writer: CalendarLiveV27WriterOwnership,
  cell: CalendarLiveV27CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V27_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v27 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v27 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V27_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV27CapturePayload(
  value: unknown,
  cell: CalendarLiveV27CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV27CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV27CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V27_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V27_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v27 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV27CaptureResponses(
  manifest: readonly CalendarLiveV27CaptureCell[],
  responses: readonly CalendarLiveV27CapturePayload[],
): void {
  validateCalendarLiveV27CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v27 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV27ObjectiveReport {
  artifactVersion: "calendar-live-v27-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV27Objective(
  manifest: readonly CalendarLiveV27CaptureCell[],
  responses: readonly CalendarLiveV27CapturePayload[],
): CalendarLiveV27ObjectiveReport {
  assertCalendarLiveV27CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v27-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV27CleanupProgram(
  writer: CalendarLiveV27WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV27CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V27_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v27 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV27CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV27WriterOwnership,
): CalendarLiveV27CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV27CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v27 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V27_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV27WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
