...
export const INPUT_LIVE_V32_VERIFIER_VERSION = "input-live-v32-verifier-v1";

export function buildInputLiveV32RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV32RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV32RawNode[];
}

export interface InputLiveV32NormalizedScene {
...
export interface InputLiveV32SceneProof {
...
export function normalizeInputLiveV32Scene(
  root: InputLiveV32RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV32NormalizedScene {
  const canonicalBindings: InputLiveV32NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV32RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV32Scene<Instance>(
  normalized: InputLiveV32NormalizedScene,
...
export function compileInputLiveV32ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV32PreCaptureGates(payload: unknown): void {
