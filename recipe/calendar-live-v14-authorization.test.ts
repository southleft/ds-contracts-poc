...
import { CALENDAR_LIVE_V14_INDEX_PATH } from "./build-calendar-live-proof-v14.js";
import {
...
import {
  buildCalendarLiveV14AuthorizationArtifact,
  validateCalendarLiveV14AntecedentIndex,
  validateCalendarLiveV14AuthorizationArtifact,
  validateCalendarLiveV14History,
  validateCalendarLiveV14UnitTestSource,
  type CalendarLiveV14AntecedentIndex,
  type CalendarLiveV14AuthorizationArtifact,
  type CalendarLiveV14AuthorizationProof,
  type CalendarLiveV14HistoryState,
} from "./calendar-live-v14-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV14ControlFlowSource,
  validateCalendarLiveV14SecurityAttestation,
  type CalendarLiveV14SecurityAttestation,
} from "./calendar-live-v14-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V14_INDEX_PATH);
const index = (): CalendarLiveV14AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV14AntecedentIndex;

const artifact = (): CalendarLiveV14AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV14AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV14HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV14HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV14AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v14/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV14AuthorizationArtifact,
): CalendarLiveV14SecurityAttestation => {
  const body: Omit<CalendarLiveV14SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V14_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV14History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV14History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV14History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV14History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV14History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV14UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v14-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV14UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV14AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV14AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV14AuthorizationArtifact(
        planted as CalendarLiveV14AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV14SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV14SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV14SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v14-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV14ControlFlowSource(source), []);
});
