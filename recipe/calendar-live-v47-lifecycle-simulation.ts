...
import { calendarLiveV1Sha256 } from "./calendar-live-v47-broker.js";

export interface CalendarLiveV47LifecycleSimulationReport {
  artifactVersion: "calendar-live-v47-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV47AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV47Lifecycle(): Promise<CalendarLiveV47LifecycleSimulationReport> {
  await buildCalendarLiveV47Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V47_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV47AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v47-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V47_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V47_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV47History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV47History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV47AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V47_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V47_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV47History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV47History(authorized, "pending");
    ...
    await buildCalendarLiveV47Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V47_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV47AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV47Lifecycle(), null, 2)}\n`,
  );
