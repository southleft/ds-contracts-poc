...
export interface InputLiveV61LifecycleSimulationReport {
  artifactVersion: "input-live-v61-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV61AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV61Lifecycle(): Promise<InputLiveV61LifecycleSimulationReport> {
  await buildInputLiveV61Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V61_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV61AntecedentIndex;
  const indexSha256 = inputLiveV61Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v61-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V61_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V61_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV61History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV61History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV61AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V61_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V61_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV61History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV61History(authorized, "pending");
    ...
    await buildInputLiveV61Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V61_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV61AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV61Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV61Lifecycle(), null, 2)}\n`,
  );
