...
/**
 * Table live v20 extract runtime. Keeps the v6 skip of untagged
 * `table/row/owned-cell-label` TEXT bind hosts. Extract bytes are
 * unchanged from v19. The v20 teaching lives in host-normalize
 * (`scene-readback-table-v1.ts`): omit `clipsContent` on `table/row`
 * variants because compile row variants omit `clipsContent`. Does not
 * omit cell-variant `clipsContent`. Does not invent `clipsContent`
 * onto compile row variants. Does not omit `table/variant` `strokes`
 * or `cornerRadius`.
 */
