...
import { buildCalendarLiveV37CleanupRuntime } from "./calendar-live-v37-cleanup.js";
import { type CalendarLiveV37FixedFixedPoint } from "./calendar-live-v37-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV37RestoreProgram,
  validateCalendarLiveV37RestorePayload,
  type CalendarLiveV37RestorePayload,
} from "./calendar-live-v37-restore.js";
import {
  buildCalendarLiveV37RawPropertyRuntime,
  normalizeCalendarLiveV37Scene,
  type CalendarLiveV37RawNode,
} from "./calendar-live-v37-verifier.js";
...
export const CALENDAR_LIVE_V37_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V37_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V37_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V37_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V37_SET_COUNT = 3;
export const CALENDAR_LIVE_V37_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V37_HOST_PHASES = 3;
export const CALENDAR_LIVE_V37_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V37_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V37_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V37_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV37SourceId = (typeof CALENDAR_LIVE_V37_SOURCE_IDS)[number];
export type CalendarLiveV37Kind = "calendar" | "week" | "day";

export interface CalendarLiveV37SourceIdentity {
  source: CalendarLiveV37SourceId;
...
export interface CalendarLiveV37WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV37ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV37SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV37RawNode;
    weekScene: CalendarLiveV37RawNode;
    dayScene: CalendarLiveV37RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV37RootProof {
  source: CalendarLiveV37SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV37FixedFixedPoint;
}

export interface CalendarLiveV37ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV37SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV37CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV37SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV37CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV37SourceId;
  frameWidth: number;
...
export interface CalendarLiveV37CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v37 ${label} must be an object`);
...
export function validateCalendarLiveV37WriterPayload(
  value: unknown,
): CalendarLiveV37WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v37") ||
    ...
    result.namespace !== CALENDAR_LIVE_V37_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v37 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v37 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v37 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV37SourceIdentity,
  kind: CalendarLiveV37Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV37ExtractProgram(
  writer: CalendarLiveV37WriterOwnership,
  sources: readonly CalendarLiveV37SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v37 extract requires one source identity",
    );
  if (
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V37_NAMESPACE)}
${buildCalendarLiveV37RawPropertyRuntime()}
...
export function validateCalendarLiveV37ExtractPayload(
  value: unknown,
  writer: CalendarLiveV37WriterOwnership,
): CalendarLiveV37ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v37 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V37_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v37 extract schema/one-root mismatch");
  return payload as CalendarLiveV37ExtractPayload;
}

export function proveCalendarLiveV37Roots<Instance>(
  extract: CalendarLiveV37ExtractPayload,
  sources: readonly (CalendarLiveV37SourceIdentity & {
...
): CalendarLiveV37RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v37 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v37 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV37Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV37Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV37Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV37FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v37 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v37 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v37 compile lost owned sets");
...
export function assertCalendarLiveV37RootProofs(
  proofs: readonly CalendarLiveV37RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v37 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v37 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV37ProbeProgram(
  writer: CalendarLiveV37WriterOwnership,
  sources: readonly CalendarLiveV37SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V37_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV37ProbePayload(
  value: unknown,
  writer: CalendarLiveV37WriterOwnership,
): CalendarLiveV37ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV37ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V37_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V37_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v37 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV37CaptureManifest(
  cells: readonly CalendarLiveV37CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V37_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V37_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V37_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V37_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v37 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV37CaptureCell[],
): string => {
  validateCalendarLiveV37CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV37CaptureProgram(
  writer: CalendarLiveV37WriterOwnership,
  cell: CalendarLiveV37CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V37_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v37 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v37 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V37_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV37CapturePayload(
  value: unknown,
  cell: CalendarLiveV37CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV37CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV37CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V37_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V37_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v37 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV37CaptureResponses(
  manifest: readonly CalendarLiveV37CaptureCell[],
  responses: readonly CalendarLiveV37CapturePayload[],
): void {
  validateCalendarLiveV37CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v37 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV37ObjectiveReport {
  artifactVersion: "calendar-live-v37-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV37Objective(
  manifest: readonly CalendarLiveV37CaptureCell[],
  responses: readonly CalendarLiveV37CapturePayload[],
): CalendarLiveV37ObjectiveReport {
  assertCalendarLiveV37CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v37-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV37CleanupProgram(
  writer: CalendarLiveV37WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV37CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V37_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v37 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV37CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV37WriterOwnership,
): CalendarLiveV37CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV37CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v37 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V37_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV37WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
