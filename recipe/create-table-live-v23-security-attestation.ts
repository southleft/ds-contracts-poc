...
import { verifyTableLiveV23Authorization } from "./table-live-v23-authorization.js";
import { TABLE_LIVE_V23_TARGET } from "./table-live-v23-broker.js";
import {
  TABLE_LIVE_V23_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV23Authorization();
...
const attestation: TableLiveV23SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV23SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v23 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V23_SECURITY_ATTESTATION_DEFAULT_PATH,
);
