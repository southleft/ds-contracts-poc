...
import { tableLiveV2Sha256 } from "./table-live-v31-broker.js";

export interface TableLiveV31LifecycleSimulationReport {
  artifactVersion: "table-live-v31-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV31AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV31Lifecycle(): Promise<TableLiveV31LifecycleSimulationReport> {
  await buildTableLiveV31Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V31_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV31AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v31-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V31_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V31_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV31History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV31History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV31AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V31_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V31_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV31History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV31History(authorized, "pending");
    ...
    await buildTableLiveV31Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V31_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV31AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV31Lifecycle(), null, 2)}\n`,
  );
