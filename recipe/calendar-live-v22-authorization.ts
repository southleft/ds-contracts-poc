...
export type CalendarLiveV22HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV22AuthorizationArtifact {
  artifactVersion: "calendar-live-v22-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V22_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV22AntecedentIndex {
...
export interface CalendarLiveV22HistoryState {
  index?: CalendarLiveV22AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV22AuthorizationArtifact;
...
export interface CalendarLiveV22AuthorizationProof
  extends CalendarLiveV22TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V22_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V22_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V22_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V22_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v22-authorization") ||
  artifactPath.includes("calendar-live-v22-preflight") ||
  artifactPath.includes("calendar-live-v22-authorized") ||
  artifactPath.includes("create-calendar-live-v22-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v22-status") ||
...
export function validateCalendarLiveV22AntecedentIndex(
  index: CalendarLiveV22AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v22-antecedent-index-v1" ||
...
export function validateCalendarLiveV22UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV22History\(\s*["']/.test(source) ||
    /readCalendarLiveV22HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV22AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV22AuthorizationArtifact {
  ...
  ) as CalendarLiveV22AntecedentIndex;
  const indexFailures = validateCalendarLiveV22AntecedentIndex(index);
...
export function validateCalendarLiveV22AuthorizationArtifact(
  artifact: CalendarLiveV22AuthorizationArtifact | undefined,
  index: CalendarLiveV22AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v22-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v22" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V22_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V22_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V22_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V22_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V22_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V22_SOURCE_ROOTS ||
...
export function validateCalendarLiveV22History(
  state: CalendarLiveV22HistoryState,
  expected: CalendarLiveV22HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV22AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV22AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV22HistoryState(
  root = process.cwd(),
): CalendarLiveV22HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V22_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV22AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V22_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V22_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V22_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV22History(
  expected: CalendarLiveV22HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV22HistoryState {
  const state = readCalendarLiveV22HistoryState(root);
  const failures = validateCalendarLiveV22History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v22 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV22Authorization(
  root = process.cwd(),
): CalendarLiveV22AuthorizationProof {
  const state = verifyCalendarLiveV22History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V22_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v22/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v22.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V22_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V22_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V22_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV22HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV22History(expected);
  process.stdout.write(
    `Calendar live v22 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
