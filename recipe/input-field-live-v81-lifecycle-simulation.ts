...
export interface InputLiveV81LifecycleSimulationReport {
  artifactVersion: "input-live-v81-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV81AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV81Lifecycle(): Promise<InputLiveV81LifecycleSimulationReport> {
  await buildInputLiveV81Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V81_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV81AntecedentIndex;
  const indexSha256 = inputLiveV81Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v81-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V81_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V81_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV81History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV81History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV81AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V81_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V81_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV81History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV81History(authorized, "pending");
    ...
    await buildInputLiveV81Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V81_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV81AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV81Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV81Lifecycle(), null, 2)}\n`,
  );
