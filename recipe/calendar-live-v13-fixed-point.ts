/**
 * Calendar live v13 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V13_FIXED_VERIFIER_VERSION = "calendar-live-v13-fixed-point-v1";
export const CALENDAR_LIVE_V13_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV13FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV13FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV13FixedSourceProbe {
...
export interface CalendarLiveV13FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV13FixedFixedPoint;
}

export interface CalendarLiveV13FixedHardGateInput {
  thresholds: CalendarLiveV13FixedThresholds;
  sourceProbes: CalendarLiveV13FixedSourceProbe[];
  cells: CalendarLiveV13FixedCellValidation[];
  sceneProofs: CalendarLiveV13FixedSceneProof[];
...
export interface CalendarLiveV13FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V13_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV13FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV13FixedHardGates(
  input: CalendarLiveV13FixedHardGateInput,
): CalendarLiveV13FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV13FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V13_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV13FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV13FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV13FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
