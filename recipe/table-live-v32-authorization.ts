...
import {
  ...
  type TableLiveV32TransactionAuthorization,
} from "./table-live-v32-broker.js";
import {
  TABLE_LIVE_V32_AUTHORIZATION_PATH,
  TABLE_LIVE_V32_INDEX_PATH,
} from "./build-table-live-proof-v32.js";
import {
...
export type TableLiveV32HistoryExpectation = "pending" | "authorized";

export interface TableLiveV32AuthorizationArtifact {
  artifactVersion: "table-live-v32-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V32_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV32AntecedentIndex {
...
export interface TableLiveV32HistoryState {
  index?: TableLiveV32AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV32AuthorizationArtifact;
...
export interface TableLiveV32AuthorizationProof
  extends TableLiveV32TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V32_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V32_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V32_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V32_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v32-authorization") ||
  artifactPath.includes("table-live-v32-preflight") ||
  artifactPath.includes("table-live-v32-authorized") ||
  artifactPath.includes("create-table-live-v32-security-attestation") ||
  artifactPath.includes("table-live-pivot-v32-status") ||
...
export function validateTableLiveV32AntecedentIndex(
  index: TableLiveV32AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v32-antecedent-index-v1" ||
...
export function validateTableLiveV32UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV32History\(\s*["']/.test(source) ||
    /readTableLiveV32HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV32AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV32AuthorizationArtifact {
  ...
  ) as TableLiveV32AntecedentIndex;
  const indexFailures = validateTableLiveV32AntecedentIndex(index);
...
export function validateTableLiveV32AuthorizationArtifact(
  artifact: TableLiveV32AuthorizationArtifact | undefined,
  index: TableLiveV32AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v32-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v32" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V32_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V32_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V32_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V32_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V32_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V32_SOURCE_ROOTS ||
...
export function validateTableLiveV32History(
  state: TableLiveV32HistoryState,
  expected: TableLiveV32HistoryExpectation,
): string[] {
  const failures = validateTableLiveV32AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV32AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV32HistoryState(
  root = process.cwd(),
): TableLiveV32HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V32_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV32AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V32_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V32_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V32_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV32History(
  expected: TableLiveV32HistoryExpectation,
  root = process.cwd(),
): TableLiveV32HistoryState {
  const state = readTableLiveV32HistoryState(root);
  const failures = validateTableLiveV32History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v32 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV32Authorization(
  root = process.cwd(),
): TableLiveV32AuthorizationProof {
  const state = verifyTableLiveV32History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V32_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v32/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v32.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V32_TARGET,
    expectedDynamicTool: TABLE_LIVE_V32_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V32_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV32HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV32History(expected);
  process.stdout.write(
    `Table live v32 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
