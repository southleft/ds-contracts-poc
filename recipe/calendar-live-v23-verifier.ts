...
import {
  verifyCalendarLiveV23FixedSceneFixedPoint,
  type CalendarLiveV23FixedFixedPoint,
} from "./calendar-live-v23-fixed-point.js";

export const CALENDAR_LIVE_V23_VERIFIER_VERSION = "calendar-live-v23-verifier-v1";

export function buildCalendarLiveV23RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV23RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV23RawNode[];
}

export interface CalendarLiveV23NormalizedScene {
...
export interface CalendarLiveV23SceneProof {
...
export function normalizeCalendarLiveV23Scene(
  root: CalendarLiveV23RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV23NormalizedScene {
  const canonicalBindings: CalendarLiveV23NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV23RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV23Scene<Instance>(
  normalized: CalendarLiveV23NormalizedScene,
...
): CalendarLiveV23SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV23FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV23ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV23PreCaptureGates(payload: unknown): void {
