...
import {
  acceptInputLiveV20Response,
  createInputLiveV20Transaction,
  inputLiveV20ReceiptPath,
  inputLiveV20RequestPath,
  inputLiveV20ResponsePath,
  inputLiveV20Sha256,
  issueInputLiveV20Request,
  persistInputLiveV20RawResponse,
  persistInputLiveV20TechnicalGates,
  readInputLiveV20AcceptedRawPayload,
  type InputLiveV20Request,
  type InputLiveV20TransactionAuthorization,
} from "./input-field-live-v20-broker.js";
import {
  INPUT_LIVE_V20_CAPTURE_COUNT,
  assertInputLiveV20CaptureResponses,
  assertInputLiveV20RootProofs,
  buildInputLiveV20CaptureProgram,
  buildInputLiveV20CleanupProgram,
  ...
  evaluateInputLiveV20Objective,
  inputLiveV20CaptureManifestSha256,
  proveInputLiveV20Roots,
  validateInputLiveV20CaptureManifest,
  validateInputLiveV20CapturePayload,
  validateInputLiveV20CleanupPayload,
  validateInputLiveV20ExtractPayload,
  validateInputLiveV20ProbePayload,
  validateInputLiveV20WriterPayload,
  type InputLiveV20CaptureCell,
  type InputLiveV20CapturePayload,
...
import {
  ...
  INPUT_LIVE_V20_PLAN_PATH,
  type InputLiveV20ProofPlan,
} from "./build-input-field-live-proof-v20.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-v20.js";

export const INPUT_LIVE_V20_RUNNER_VERSION = "input-live-v20-runner-v1";

interface InputLiveV20JournalEntry {
  index: number;
  event: string;
...
interface InputLiveV20OrchestratorState {
  artifactVersion: "input-live-v20-orchestrator-state-v1";
  ...
  journal: InputLiveV20JournalEntry[];
}

interface LoadedProofInputs {
  plan: InputLiveV20ProofPlan;
  planSha256: string;
  captures: InputLiveV20CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    InputLiveV20SourceIdentity & {
...
export interface InputLiveV20NextAction {
...
const writeState = (
  directory: string,
  state: InputLiveV20OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, INPUT_LIVE_V20_PLAN_PATH);
  const capturePath = path.join(root, INPUT_LIVE_V20_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as InputLiveV20ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as InputLiveV20CaptureCell[];
  validateInputLiveV20CaptureManifest(captures);
  if (
    plan.captureManifest.sha256 !== inputLiveV20Sha256(captureBytes) ||
    captureArtifact.cellsSha256 !== inputLiveV20CaptureManifestSha256(captures)
  )
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (
      inputLiveV20Sha256(expectedBytes) !== source.expectedScenePlan.sha256 ||
      inputLiveV20Sha256(uncompressed) !==
  ...
  return {
    plan,
    planSha256: inputLiveV20Sha256(planBytes),
    captures,
    captureManifestSha256: inputLiveV20Sha256(captureBytes),
    sources,
...
const appendJournal = (
  ...
  const payloadSha256 = inputLiveV20Sha256(canonicalJson(payload));
  const body = {
  ...
  const entry = {
    ...body,
    entrySha256: inputLiveV20Sha256(canonicalJson(body)),
  };
  writeNew(
    path.join(
      directory,
...
const writerOwnership = (directory: string): InputLiveV20WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): InputLiveV20RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): InputLiveV20Request =>
  readJson<InputLiveV20Request>(file);

export class InputLiveV20Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueInputLiveV20Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: InputLiveV20OrchestratorState = {
      artifactVersion: "input-live-v20-orchestrator-state-v1",
      transactionDirectory: options.transactionDirectory,
      activeRequest: inputLiveV20RequestPath(
        options.transactionDirectory,
        "writer",
      ),
      cleanupRequest: null,
    ...
    return new InputLiveV20Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<InputLiveV20OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new InputLiveV20Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): InputLiveV20NextAction {
    const cleanup =
      this.#state.cleanupRequest &&
      !this.#state.cleanupAccepted &&
      !existsSync(inputLiveV20ReceiptPath(this.#directory, "cleanup"))
        ? this.#state.cleanupRequest
        : null;
  ...
  ingestAndAdvance(rawResponse: string): InputLiveV20NextAction {
  ...
  ingestCleanup(rawResponse: string): InputLiveV20NextAction {
    ...
    persistInputLiveV20RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptInputLiveV20Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV20CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      existsSync(inputLiveV20ReceiptPath(this.#directory, "writer")) &&
      !existsSync(path.join(this.#directory, OWNERSHIP_FILE))
    ) {
      const raw = readInputLiveV20AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateInputLiveV20WriterPayload(raw),
      );
    }
    if (existsSync(inputLiveV20RequestPath(this.#directory, "cleanup")))
      this.#state.cleanupRequest = inputLiveV20RequestPath(
        this.#directory,
        "cleanup",
      );
    if (
      existsSync(path.join(this.#directory, OWNERSHIP_FILE)) &&
      !existsSync(inputLiveV20RequestPath(this.#directory, "cleanup"))
    )
      this.#issueCleanupAndRestore(false);
    else if (
      existsSync(inputLiveV20RequestPath(this.#directory, "cleanup")) &&
      !existsSync(inputLiveV20RequestPath(this.#directory, "restore"))
    )
      this.#issueRestore(false);
    if (
      existsSync(inputLiveV20RequestPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV20ReceiptPath(this.#directory, "restore"))
    )
      this.#state.activeRequest = inputLiveV20RequestPath(
        this.#directory,
        "restore",
      );
    if (
      existsSync(inputLiveV20ReceiptPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV20RequestPath(this.#directory, "extract"))
    ) {
      const ownership = writerOwnership(this.#directory);
      const extract = issueInputLiveV20Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildInputLiveV20ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV20RequestPath(
        this.#directory,
        "extract",
    ...
    if (existsSync(inputLiveV20ReceiptPath(this.#directory, "cleanup")))
      this.#state.cleanupAccepted = true;
    this.#persist();
  }

  #acceptActive(request: InputLiveV20Request): void {
    if (request.phase === "writer") {
      const accepted = acceptInputLiveV20Response(this.#directory, "writer", {
        validate: validateInputLiveV20WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptInputLiveV20Response(this.#directory, "restore", {
        validate: (payload) =>
          validateInputLiveV20RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptInputLiveV20Response(this.#directory, "extract", {
        validate: (payload) =>
          validateInputLiveV20ExtractPayload(payload, ownership),
      });
      const proofs = proveInputLiveV20Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseInputFieldRecipe,
        compileInputFieldRecipe,
      );
      assertInputLiveV20RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "input-live-v20-host-root-proofs-v1",
      ...
      appendJournal(
        ...
        request.requestId,
        { proofsSha256: inputLiveV20Sha256(canonicalJson(proofs)) },
      );
      const next = issueInputLiveV20Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildInputLiveV20ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV20RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptInputLiveV20Response(this.#directory, "probe", {
        validate: (payload) =>
          validateInputLiveV20ProbePayload(payload, ownership),
      });
      persistInputLiveV20TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      const accepted = acceptInputLiveV20Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateInputLiveV20CapturePayload(payload, cell, rawBytes),
      });
      const png = Buffer.from(accepted.payload.pngBase64, "base64");
      const capturePath = path.join(
        this.#directory,
        "captures",
        `${String(cell.index).padStart(3, "0")}-${inputLiveV20Sha256(cell.cellKey).slice(0, 20)}.png`,
      );
      ...
      if (next < INPUT_LIVE_V20_CAPTURE_COUNT) this.#issueCapture(next);
      else {
          ...
          pngBase64: Buffer.from(
            readFileSync(
              path.join(
                this.#directory,
                "captures",
                `${String(response.index).padStart(3, "0")}-${inputLiveV20Sha256(response.cellKey).slice(0, 20)}.png`,
              ),
            ),
          ).toString("base64"),
        }));
        assertInputLiveV20CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateInputLiveV20Objective(
        ...
        appendJournal(
          ...
          request.requestId,
          {
            reportSha256: inputLiveV20Sha256(canonicalJson(objective)),
  ...
  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueInputLiveV20Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildInputLiveV20CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = inputLiveV20RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueInputLiveV20Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildInputLiveV20RestoreProgram(ownership),
    );
    this.#state.activeRequest = inputLiveV20RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueInputLiveV20Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildInputLiveV20CaptureProgram(
  ...
  #readCaptureResponseIndex(): InputLiveV20CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: InputLiveV20CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedInputLiveV20Authorization = (
  privateKey: KeyObject,
): InputLiveV20TransactionAuthorization => ({
  mode: "simulated",
  ...
  signingPublicKeySha256: inputLiveV20Sha256(
...
export const generateInputLiveV20SimulationKey = (): KeyObject =>
