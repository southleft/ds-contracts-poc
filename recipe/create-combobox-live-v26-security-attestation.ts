...
import { COMBOBOX_LIVE_V26_TARGET } from "./combobox-live-v26-broker.js";
import {
  COMBOBOX_LIVE_V26_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV26Authorization();
...
const attestation: ComboboxLiveV26SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV26SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v26 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V26_SECURITY_ATTESTATION_DEFAULT_PATH,
);
