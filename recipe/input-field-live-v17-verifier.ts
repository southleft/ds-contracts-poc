...
export const INPUT_LIVE_V17_VERIFIER_VERSION = "input-live-v17-verifier-v1";

export function buildInputLiveV17RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV17RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV17RawNode[];
}

export interface InputLiveV17NormalizedScene {
...
export interface InputLiveV17SceneProof {
...
export function normalizeInputLiveV17Scene(
  root: InputLiveV17RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV17NormalizedScene {
  const canonicalBindings: InputLiveV17NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV17RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV17Scene<Instance>(
  normalized: InputLiveV17NormalizedScene,
...
export function compileInputLiveV17ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV17PreCaptureGates(payload: unknown): void {
