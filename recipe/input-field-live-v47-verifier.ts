...
export const INPUT_LIVE_V47_VERIFIER_VERSION = "input-live-v47-verifier-v1";

export function buildInputLiveV47RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV47RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV47RawNode[];
}

export interface InputLiveV47NormalizedScene {
...
export interface InputLiveV47SceneProof {
...
export function normalizeInputLiveV47Scene(
  root: InputLiveV47RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV47NormalizedScene {
  const canonicalBindings: InputLiveV47NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV47RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV47Scene<Instance>(
  normalized: InputLiveV47NormalizedScene,
...
export function compileInputLiveV47ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV47PreCaptureGates(payload: unknown): void {
