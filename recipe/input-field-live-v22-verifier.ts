...
export const INPUT_LIVE_V22_VERIFIER_VERSION = "input-live-v22-verifier-v1";

export function buildInputLiveV22RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV22RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV22RawNode[];
}

export interface InputLiveV22NormalizedScene {
...
export interface InputLiveV22SceneProof {
...
export function normalizeInputLiveV22Scene(
  root: InputLiveV22RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV22NormalizedScene {
  const canonicalBindings: InputLiveV22NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV22RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV22Scene<Instance>(
  normalized: InputLiveV22NormalizedScene,
...
export function compileInputLiveV22ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV22PreCaptureGates(payload: unknown): void {
