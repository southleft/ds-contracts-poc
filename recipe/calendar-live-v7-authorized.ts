...
import { CALENDAR_LIVE_V7_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v7.js";
import { verifyCalendarLiveV7Authorization } from "./calendar-live-v7-authorization.js";
import { runCalendarLiveV7Preflight } from "./calendar-live-v7-preflight.js";
import { CalendarLiveV7Orchestrator } from "./run-calendar-live-v7.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v7 requires exact authorization artifact ${CALENDAR_LIVE_V7_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v7-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV7Authorization(root);
    runCalendarLiveV7Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV7Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV7Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
