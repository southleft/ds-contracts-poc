...
import {
  validateTableLiveV30AntecedentIndex,
  verifyTableLiveV30Authorization,
  type TableLiveV30AntecedentIndex,
  type TableLiveV30AuthorizationProof,
} from "./table-live-v30-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V30_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v30/operator-security-attestation-template.json";
export const TABLE_LIVE_V30_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v30-security-attestation.json";

export interface TableLiveV30SecurityAttestation {
  artifactVersion: "table-live-v30-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV30PreflightReport {
  artifactVersion: "table-live-v30-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV30SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV30SecurityAttestation(
  value: unknown,
  proof: TableLiveV30AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v30-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V30_TARGET) ||
...
export function validateTableLiveV30ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV30Authorization");
  const preflight = source.indexOf("runTableLiveV30Preflight");
  const initialize = source.indexOf("TableLiveV30Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV30Preflight(
  root: string,
  proof: TableLiveV30AuthorizationProof,
  attestationPath = TABLE_LIVE_V30_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV30PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V30_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V30_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV30AntecedentIndex;
  failures.push(...validateTableLiveV30AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V30_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V30_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV30SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v30-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV30ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v30 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV30SecurityAttestation;
  return {
    artifactVersion: "table-live-v30-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V30_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV30Preflight(
    process.cwd(),
    verifyTableLiveV30Authorization(),
    attestationPath,
