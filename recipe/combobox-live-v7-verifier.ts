...
import {
  verifyComboboxLiveV7FixedSceneFixedPoint,
  type ComboboxLiveV7FixedFixedPoint,
} from "./combobox-live-v7-fixed-point.js";

export const COMBOBOX_LIVE_V7_VERIFIER_VERSION = "combobox-live-v7-verifier-v1";

export function buildComboboxLiveV7RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV7RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV7RawNode[];
}

export interface ComboboxLiveV7NormalizedScene {
...
export interface ComboboxLiveV7SceneProof {
...
export function normalizeComboboxLiveV7Scene(
  root: ComboboxLiveV7RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV7NormalizedScene {
  const canonicalBindings: ComboboxLiveV7NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV7RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV7Scene<Instance>(
  normalized: ComboboxLiveV7NormalizedScene,
...
): ComboboxLiveV7SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV7FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV7ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV7PreCaptureGates(payload: unknown): void {
