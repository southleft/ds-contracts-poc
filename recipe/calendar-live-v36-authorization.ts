...
export type CalendarLiveV36HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV36AuthorizationArtifact {
  artifactVersion: "calendar-live-v36-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V36_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV36AntecedentIndex {
...
export interface CalendarLiveV36HistoryState {
  index?: CalendarLiveV36AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV36AuthorizationArtifact;
...
export interface CalendarLiveV36AuthorizationProof
  extends CalendarLiveV36TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V36_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V36_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V36_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V36_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v36-authorization") ||
  artifactPath.includes("calendar-live-v36-preflight") ||
  artifactPath.includes("calendar-live-v36-authorized") ||
  artifactPath.includes("create-calendar-live-v36-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v36-status") ||
...
export function validateCalendarLiveV36AntecedentIndex(
  index: CalendarLiveV36AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v36-antecedent-index-v1" ||
...
export function validateCalendarLiveV36UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV36History\(\s*["']/.test(source) ||
    /readCalendarLiveV36HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV36AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV36AuthorizationArtifact {
  ...
  ) as CalendarLiveV36AntecedentIndex;
  const indexFailures = validateCalendarLiveV36AntecedentIndex(index);
...
export function validateCalendarLiveV36AuthorizationArtifact(
  artifact: CalendarLiveV36AuthorizationArtifact | undefined,
  index: CalendarLiveV36AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v36-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v36" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V36_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V36_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V36_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V36_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V36_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V36_SOURCE_ROOTS ||
...
export function validateCalendarLiveV36History(
  state: CalendarLiveV36HistoryState,
  expected: CalendarLiveV36HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV36AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV36AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV36HistoryState(
  root = process.cwd(),
): CalendarLiveV36HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V36_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV36AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V36_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V36_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V36_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV36History(
  expected: CalendarLiveV36HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV36HistoryState {
  const state = readCalendarLiveV36HistoryState(root);
  const failures = validateCalendarLiveV36History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v36 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV36Authorization(
  root = process.cwd(),
): CalendarLiveV36AuthorizationProof {
  const state = verifyCalendarLiveV36History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V36_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v36/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v36.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V36_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V36_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V36_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV36HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV36History(expected);
  process.stdout.write(
    `Calendar live v36 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
