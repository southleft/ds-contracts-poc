...
import { COMBOBOX_LIVE_V39_INDEX_PATH } from "./build-combobox-live-proof-v39.js";
import {
  COMBOBOX_LIVE_V39_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V39_TARGET,
} from "./combobox-live-v39-broker.js";
import {
  buildComboboxLiveV39AuthorizationArtifact,
  validateComboboxLiveV39AntecedentIndex,
  validateComboboxLiveV39AuthorizationArtifact,
  validateComboboxLiveV39History,
  validateComboboxLiveV39UnitTestSource,
  type ComboboxLiveV39AntecedentIndex,
  type ComboboxLiveV39AuthorizationArtifact,
  type ComboboxLiveV39AuthorizationProof,
  type ComboboxLiveV39HistoryState,
} from "./combobox-live-v39-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV39ControlFlowSource,
  validateComboboxLiveV39SecurityAttestation,
  type ComboboxLiveV39SecurityAttestation,
} from "./combobox-live-v39-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v39-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V39_INDEX_PATH);
const index = (): ComboboxLiveV39AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV39AntecedentIndex;

const artifact = (): ComboboxLiveV39AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV39AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV39HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV39HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV39AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v39/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV39AuthorizationArtifact,
): ComboboxLiveV39SecurityAttestation => {
  const body: Omit<ComboboxLiveV39SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V39_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV39History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV39History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV39History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV39History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV39History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV39UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV39UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV39AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV39AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV39AuthorizationArtifact(
        planted as ComboboxLiveV39AuthorizationArtifact,
        index(),
...
test("two-root denominator, capture gate, cleanup, v6 reuse, and result leakage are rejected", () => {
  ...
  for (const mutate of [
    ...
    (value) => {
      value.execution.v39AuthorizationReusable = true;
    },
    (value) => {
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV39SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV39SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV39SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV39ControlFlowSource(source), []);
});
