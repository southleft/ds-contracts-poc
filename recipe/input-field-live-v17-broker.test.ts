...
import {
  ...
  createInputLiveV17FakeDynamicResponse,
  ...
  listInputLiveV17PersistedRequests,
  persistInputLiveV17RawResponse,
  persistInputLiveV17TechnicalGates,
  validateInputLiveV17Receipt,
  validateInputLiveV17Request,
  type InputLiveV17RemotePhase,
...
import {
  ...
  type InputLiveV17ExtractPayload,
  type InputLiveV17ProbePayload,
...
import {
  ...
  simulatedInputLiveV17Authorization,
} from "./run-input-field-live-v17.js";
...
const captures: InputLiveV17CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV17Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v17-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV17Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV17CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV17FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV17FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V17_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V17_TARGET);
  persistInputLiveV17RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV17Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV17Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV17WriterOwnership,
...
const rootProofs = (): InputLiveV17RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v17-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV17Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV17Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV17RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV17RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV17ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV17ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV17CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV17CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV17CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV17Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV17CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV17PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV17ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV17Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV17Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV17Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV17Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V17_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV17RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V17_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV17Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV17Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV17ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV17Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV17WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV17RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV17Request;
    validateInputLiveV17Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV17Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV17CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v17-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV17Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV17Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV17Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V17_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV17RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V17_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV17Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV17Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV17RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V17_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV17Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV17Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV17Request;
