...
import {
  assertInputLiveV51RootProofs,
  INPUT_LIVE_V51_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V51_RESPONSE_CONTRACTS,
  type InputLiveV51RootProof,
} from "./input-field-live-v51-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V51_BROKER_VERSION = "input-live-v51-broker-v2";
export const INPUT_LIVE_V51_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V51_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V51_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V51_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V51_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V51_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV51RemotePhase =
  (typeof INPUT_LIVE_V51_REMOTE_PHASES)[number];
export type InputLiveV51AuthorizationMode = "live" | "simulated";

export interface InputLiveV51CommitPins {
...
export interface InputLiveV51TransactionAuthorization extends InputLiveV51CommitPins {
  mode: InputLiveV51AuthorizationMode;
...
export interface InputLiveV51Manifest {
  artifactVersion: "input-live-v51-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V51_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V51_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V51_TARGET;
  authorization: InputLiveV51TransactionAuthorization;
...
export interface InputLiveV51ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV51RequestBody {
  artifactVersion: "input-live-v51-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V51_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV51RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV51CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V51_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV51ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV51Request extends InputLiveV51RequestBody {
...
export interface InputLiveV51Receipt {
  artifactVersion: "input-live-v51-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V51_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV51RemotePhase;
...
export interface InputLiveV51AcceptedResponse<T = unknown> {
  request: InputLiveV51Request;
  receipt: InputLiveV51Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV51IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV51ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V51_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V51_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V51_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V51_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V51_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V51_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V51_TARGET.editorType;
};
...
export const inputLiveV51RequestSequence = (
  phase: InputLiveV51RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV51RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV51RequestPath = (
  directory: string,
  phase: InputLiveV51RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV51ResponsePath = (
  directory: string,
  phase: InputLiveV51RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV51ReceiptPath = (
  directory: string,
  phase: InputLiveV51RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV51Manifest => {
  const manifest = json<InputLiveV51Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v51-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V51_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V51_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV51RemotePhase,
  captureIndex?: number,
): InputLiveV51Request | undefined => {
  const file = inputLiveV51RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV51Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV51RemotePhase,
  captureIndex?: number,
): InputLiveV51Receipt | undefined => {
  const file = inputLiveV51ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV51Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV51Request | undefined;
  receipt: InputLiveV51Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV51RemotePhase,
): InputLiveV51ResponseContract => ({
  ...INPUT_LIVE_V51_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V51_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V51_DYNAMIC_TOOL,
      target: INPUT_LIVE_V51_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v51-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V51_BROKER_VERSION,
  target: INPUT_LIVE_V51_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V51_PROGRAM_START*/${program}/*INPUT_LIVE_V51_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V51_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V51_PROGRAM_END*/";
...
export function createInputLiveV51Transaction(
  directory: string,
  ...
  const manifest: InputLiveV51Manifest = {
    artifactVersion: "input-live-v51-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V51_BROKER_VERSION,
...
export function validateInputLiveV51Request(
  directory: string,
  request: InputLiveV51Request,
): void {
  ...
  const expectedSequence = inputLiveV51RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v51-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V51_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V51_TARGET.fileKey ||
...
export function issueInputLiveV51Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV51RemotePhase,
  program: string,
  options: InputLiveV51IssueOptions = {},
): InputLiveV51Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV51RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV51Request>(targetPath);
    validateInputLiveV51Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV51ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV51TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV51RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V51_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V51_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V51_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV51RequestBody = {
    artifactVersion: "input-live-v51-broker-request-v2",
    brokerVersion: INPUT_LIVE_V51_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV51Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV51Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV51RawResponse(
  directory: string,
  phase: InputLiveV51RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV51RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV51Request>(requestFile);
  if (
  ...
  const target = inputLiveV51ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV51Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v51-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V51_BROKER_VERSION,
...
export function acceptInputLiveV51Response<T = unknown>(
  directory: string,
  phase: InputLiveV51RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV51AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV51ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV51Request>(
    inputLiveV51RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV51Request(directory, request);
  if (phase === "capture") validateInputLiveV51TechnicalGates(directory);
  const rawFile = inputLiveV51ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v51-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V51_BROKER_VERSION ||
...
export function persistInputLiveV51TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV51RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV51RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V51_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v51-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V51_BROKER_VERSION,
...
export function validateInputLiveV51TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v51-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V51_TECHNICAL_GATES.some(
...
export function validateInputLiveV51Receipt(
  directory: string,
  phase: InputLiveV51RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV51Request(directory, request);
  const receipt = json<InputLiveV51Receipt>(
    inputLiveV51ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV51ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v51-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V51_BROKER_VERSION ||
...
export function readInputLiveV51AcceptedRawPayload(
  directory: string,
  phase: InputLiveV51RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV51Receipt(directory, phase, captureIndex);
...
export function createInputLiveV51FakeDynamicResponse(
  request: InputLiveV51Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V51_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V51_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV51PersistedRequests(
  directory: string,
): InputLiveV51Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV51Request>(file)] : [];
    });
}

export const inputLiveV51Sha256 = sha256;
