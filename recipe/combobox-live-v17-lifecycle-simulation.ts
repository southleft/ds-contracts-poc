...
export interface ComboboxLiveV17LifecycleSimulationReport {
  artifactVersion: "combobox-live-v17-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV17AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV17Lifecycle(): Promise<ComboboxLiveV17LifecycleSimulationReport> {
  await buildComboboxLiveV17Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V17_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV17AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v17-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V17_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V17_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV17History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV17History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV17AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V17_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V17_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV17History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV17History(authorized, "pending");
    ...
    await buildComboboxLiveV17Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V17_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV17AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV17Lifecycle(), null, 2)}\n`,
  );
