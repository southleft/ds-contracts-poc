...
export interface InputLiveV7LifecycleSimulationReport {
  artifactVersion: "input-live-v7-lifecycle-simulation-v1";
  ...
  fullV7CheckGreenPostAuthorization: true;
...
const gitEnvironment = {
  ...process.env,
  GIT_AUTHOR_NAME: "Input v7 Lifecycle Simulation",
  GIT_AUTHOR_EMAIL: "input-v7-simulation@invalid",
  GIT_COMMITTER_NAME: "Input v7 Lifecycle Simulation",
  GIT_COMMITTER_EMAIL: "input-v7-simulation@invalid",
};
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV7AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV7Lifecycle(): Promise<InputLiveV7LifecycleSimulationReport> {
  await buildInputLiveV7Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V7_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV7AntecedentIndex;
  const indexSha256 = inputLiveV7Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v7-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V7_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V7_INDEX_PATH]);
    git(temporary, ["commit", "-m", "synthetic v7 antecedent"]);
    const antecedentCommit = git(temporary, ["rev-parse", "HEAD"]);
    const pending = historyState(index, indexSha256, antecedentCommit);
    const pendingFailures = validateInputLiveV7History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v7 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV7History(pending, "authorized");
    if (
      !staleAuthorized.some((failure) => failure.includes("stale history mode"))
    )
      throw new Error("v7 lifecycle did not refuse stale authorized mode");

    const { publicKey } = generateKeyPairSync("ed25519");
    const authorization = buildInputLiveV7AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V7_AUTHORIZATION_PATH]);
    git(temporary, ["commit", "-m", "synthetic v7 authorization"]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V7_INDEX_PATH}`,
    ]);
    if (!committedAntecedentBefore.equals(committedAntecedentAfter))
      throw new Error(
        "v7 antecedent index changed across authorization commit",
      );
    ...
    const authorizedFailures = validateInputLiveV7History(
      authorized,
      "authorized",
    );
    if (authorizedFailures.length)
      throw new Error(
        `v7 lifecycle authorized phase failed:\n${authorizedFailures.join("\n")}`,
      );
    const stalePending = validateInputLiveV7History(authorized, "pending");
    if (!stalePending.some((failure) => failure.includes("stale history mode")))
      throw new Error("v7 lifecycle did not refuse stale pending mode");

    await buildInputLiveV7Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V7_INDEX_PATH);
    if (!indexBefore.equals(indexAfter))
      throw new Error(
        "v7 generated check changed after synthetic authorization",
      );
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV7AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
      throw new Error("v7 antecedent hash set changed after authorization");
    return {
      artifactVersion: "input-live-v7-lifecycle-simulation-v1",
      syntheticGitCommits: 2,
      antecedentCheckBeforeAuthorization: true,
      ...
      authorizationSha256: inputLiveV7Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV7Lifecycle(), null, 2)}\n`,
  );
