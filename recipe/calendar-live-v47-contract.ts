...
import { buildCalendarLiveV47CleanupRuntime } from "./calendar-live-v47-cleanup.js";
import { type CalendarLiveV47FixedFixedPoint } from "./calendar-live-v47-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV47RestoreProgram,
  validateCalendarLiveV47RestorePayload,
  type CalendarLiveV47RestorePayload,
} from "./calendar-live-v47-restore.js";
import {
  buildCalendarLiveV47RawPropertyRuntime,
  normalizeCalendarLiveV47Scene,
  type CalendarLiveV47RawNode,
} from "./calendar-live-v47-verifier.js";
...
export const CALENDAR_LIVE_V47_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V47_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V47_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V47_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V47_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V47_SET_COUNT = 3;
export const CALENDAR_LIVE_V47_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V47_HOST_PHASES = 3;
export const CALENDAR_LIVE_V47_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V47_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V47_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V47_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV47SourceId = (typeof CALENDAR_LIVE_V47_SOURCE_IDS)[number];
export type CalendarLiveV47Kind = "calendar" | "week" | "day";

export interface CalendarLiveV47SourceIdentity {
  source: CalendarLiveV47SourceId;
...
export interface CalendarLiveV47WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV47ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV47SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV47RawNode;
    weekScene: CalendarLiveV47RawNode;
    dayScene: CalendarLiveV47RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV47RootProof {
  source: CalendarLiveV47SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV47FixedFixedPoint;
}

export interface CalendarLiveV47ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV47SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV47CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV47SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV47CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV47SourceId;
  frameWidth: number;
...
export interface CalendarLiveV47CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v47 ${label} must be an object`);
...
export function validateCalendarLiveV47WriterPayload(
  value: unknown,
): CalendarLiveV47WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v47") ||
    ...
    result.namespace !== CALENDAR_LIVE_V47_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v47 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v47 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v47 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV47SourceIdentity,
  kind: CalendarLiveV47Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV47ExtractProgram(
  writer: CalendarLiveV47WriterOwnership,
  sources: readonly CalendarLiveV47SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v47 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V47_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V47_NAMESPACE)}
${buildCalendarLiveV47RawPropertyRuntime()}
...
export function validateCalendarLiveV47ExtractPayload(
  value: unknown,
  writer: CalendarLiveV47WriterOwnership,
): CalendarLiveV47ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v47 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V47_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v47 extract schema/one-root mismatch");
  return payload as CalendarLiveV47ExtractPayload;
}

export function proveCalendarLiveV47Roots<Instance>(
  extract: CalendarLiveV47ExtractPayload,
  sources: readonly (CalendarLiveV47SourceIdentity & {
...
): CalendarLiveV47RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v47 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v47 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV47Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV47Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV47Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV47FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v47 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v47 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v47 compile lost owned sets");
...
export function assertCalendarLiveV47RootProofs(
  proofs: readonly CalendarLiveV47RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v47 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v47 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV47ProbeProgram(
  writer: CalendarLiveV47WriterOwnership,
  sources: readonly CalendarLiveV47SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V47_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV47ProbePayload(
  value: unknown,
  writer: CalendarLiveV47WriterOwnership,
): CalendarLiveV47ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV47ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V47_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V47_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v47 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV47CaptureManifest(
  cells: readonly CalendarLiveV47CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V47_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V47_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V47_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V47_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v47 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV47CaptureCell[],
): string => {
  validateCalendarLiveV47CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV47CaptureProgram(
  writer: CalendarLiveV47WriterOwnership,
  cell: CalendarLiveV47CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V47_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v47 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v47 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V47_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV47CapturePayload(
  value: unknown,
  cell: CalendarLiveV47CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV47CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV47CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V47_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V47_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v47 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV47CaptureResponses(
  manifest: readonly CalendarLiveV47CaptureCell[],
  responses: readonly CalendarLiveV47CapturePayload[],
): void {
  validateCalendarLiveV47CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v47 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV47ObjectiveReport {
  artifactVersion: "calendar-live-v47-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV47Objective(
  manifest: readonly CalendarLiveV47CaptureCell[],
  responses: readonly CalendarLiveV47CapturePayload[],
): CalendarLiveV47ObjectiveReport {
  assertCalendarLiveV47CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v47-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV47CleanupProgram(
  writer: CalendarLiveV47WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV47CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V47_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v47 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV47CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV47WriterOwnership,
): CalendarLiveV47CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV47CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v47 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V47_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV47WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
