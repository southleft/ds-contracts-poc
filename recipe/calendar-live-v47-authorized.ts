...
import { CALENDAR_LIVE_V47_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v47.js";
import { verifyCalendarLiveV47Authorization } from "./calendar-live-v47-authorization.js";
import { runCalendarLiveV47Preflight } from "./calendar-live-v47-preflight.js";
import { CalendarLiveV47Orchestrator } from "./run-calendar-live-v47.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v47 requires exact authorization artifact ${CALENDAR_LIVE_V47_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v47-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV47Authorization(root);
    runCalendarLiveV47Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV47Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV47Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
