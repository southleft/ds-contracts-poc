...
} from "./input-field-live-v3-verifier-v14.js";
import {
  buildInputLiveV14RawPropertyRuntime,
  normalizeInputLiveV14Scene,
  type InputLiveV14RawNode,
} from "./input-field-live-v14-verifier.js";
...
export {
  ...
  type InputLiveV14RestorePayload,
} from "./input-field-live-v14-restore.js";

export const INPUT_LIVE_V14_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V14_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V14_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V14_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V14_VARIANT_COUNT = 256;
export const INPUT_LIVE_V14_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V14_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV14SourceId = (typeof INPUT_LIVE_V14_SOURCE_IDS)[number];

export interface InputLiveV14SourceIdentity {
  source: InputLiveV14SourceId;
...
export interface InputLiveV14WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV14ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV14SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV14RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV14RootProof {
  source: InputLiveV14SourceId;
  adapterIdentity: string;
...
export interface InputLiveV14ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV14SourceId;
    adapterIdentity: string;
...
export interface InputLiveV14CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV14SourceId;
  adapterIdentity: string;
...
export interface InputLiveV14CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV14SourceId;
  strata: InputLiveV14CaptureCell["strata"];
...
export interface InputLiveV14CleanupPayload {
...
export function validateInputLiveV14WriterPayload(
  value: unknown,
): InputLiveV14WriterOwnership {
...
const identityPlan = (source: InputLiveV14SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV14ExtractProgram(
  writer: InputLiveV14WriterOwnership,
  sources: readonly InputLiveV14SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V14_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V14_NAMESPACE)}
${buildInputLiveV14RawPropertyRuntime()}
...
export function validateInputLiveV14ExtractPayload(
  value: unknown,
  writer: InputLiveV14WriterOwnership,
): InputLiveV14ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V14_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV14ExtractPayload;
}

export function proveInputLiveV14Roots<Instance>(
  extract: InputLiveV14ExtractPayload,
  sources: readonly (InputLiveV14SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV14Scene(
      root.scene,
...
export function assertInputLiveV14RootProofs(
  proofs: readonly InputLiveV14RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV14ProbeProgram(
  writer: InputLiveV14WriterOwnership,
  sources: readonly InputLiveV14SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V14_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV14ProbePayload(
  value: unknown,
  writer: InputLiveV14WriterOwnership,
): InputLiveV14ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV14ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V14_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V14_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV14CaptureManifest(
  cells: readonly InputLiveV14CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V14_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V14_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V14_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V14_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V14_SOURCE_IDS.some(
...
export const inputLiveV14CaptureManifestSha256 = (
  cells: readonly InputLiveV14CaptureCell[],
): string => {
  validateInputLiveV14CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV14CaptureProgram(
  writer: InputLiveV14WriterOwnership,
  cell: InputLiveV14CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V14_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V14_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V14_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV14CapturePayload(
  value: unknown,
  cell: InputLiveV14CaptureCell,
  rawResponseBytes: number,
): InputLiveV14CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV14CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V14_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V14_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV14CaptureResponses(
  manifest: readonly InputLiveV14CaptureCell[],
  responses: readonly InputLiveV14CapturePayload[],
): void {
  validateInputLiveV14CaptureManifest(manifest);
  if (
...
export interface InputLiveV14ObjectiveReport {
  artifactVersion: "input-live-v14-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV14Objective(
  manifest: readonly InputLiveV14CaptureCell[],
  responses: readonly InputLiveV14CapturePayload[],
  readReference: (cell: InputLiveV14CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV14CaptureCell) => Buffer,
): InputLiveV14ObjectiveReport {
  assertInputLiveV14CaptureResponses(manifest, responses);
...
export function buildInputLiveV14CleanupProgram(
  writer: InputLiveV14WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V14_NAMESPACE,
...
export function validateInputLiveV14CleanupPayload(
  value: unknown,
  writer: InputLiveV14WriterOwnership,
): InputLiveV14CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV14CleanupPayload;
...
export const INPUT_LIVE_V14_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV14WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
