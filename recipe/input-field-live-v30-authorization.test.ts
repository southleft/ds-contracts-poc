...
import { INPUT_LIVE_V30_INDEX_PATH } from "./build-input-field-live-proof-v30.js";
import {
  INPUT_LIVE_V30_DYNAMIC_TOOL,
  INPUT_LIVE_V30_TARGET,
} from "./input-field-live-v30-broker.js";
import {
  buildInputLiveV30AuthorizationArtifact,
  validateInputLiveV30AntecedentIndex,
  validateInputLiveV30AuthorizationArtifact,
  validateInputLiveV30History,
  validateInputLiveV30UnitTestSource,
  type InputLiveV30AntecedentIndex,
  type InputLiveV30AuthorizationArtifact,
  type InputLiveV30AuthorizationProof,
  type InputLiveV30HistoryState,
} from "./input-field-live-v30-authorization.js";
import {
  inputLiveV30AttestationSha256,
  validateInputLiveV30ControlFlowSource,
  validateInputLiveV30SecurityAttestation,
  type InputLiveV30SecurityAttestation,
} from "./input-field-live-v30-preflight.js";
import { inputLiveV30Sha256 } from "./input-field-live-v30-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V30_INDEX_PATH);
const index = (): InputLiveV30AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV30AntecedentIndex;

const artifact = (): InputLiveV30AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV30AuthorizationArtifact({
...
const pendingState = (): InputLiveV30HistoryState => ({
  index: index(),
  indexSha256: inputLiveV30Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV30HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV30AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV30Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V30_TARGET,
  expectedDynamicTool: INPUT_LIVE_V30_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v30/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV30AuthorizationArtifact,
): InputLiveV30SecurityAttestation => {
  const body: Omit<InputLiveV30SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v30-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V30_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV30AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV30History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV30History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV30History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV30History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV30History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV30UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV30UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV30AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV30Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV30AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV30AuthorizationArtifact(
        planted as InputLiveV30AuthorizationArtifact,
        index(),
        inputLiveV30Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV30SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV30AttestationSha256(
      body as Omit<InputLiveV30SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV30SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV30ControlFlowSource(source), []);
});
