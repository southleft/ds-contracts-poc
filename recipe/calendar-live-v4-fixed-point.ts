/**
 * Calendar live v4 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V4_FIXED_VERIFIER_VERSION = "calendar-live-v4-fixed-point-v1";
export const CALENDAR_LIVE_V4_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV4FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV4FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV4FixedSourceProbe {
...
export interface CalendarLiveV4FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV4FixedFixedPoint;
}

export interface CalendarLiveV4FixedHardGateInput {
  thresholds: CalendarLiveV4FixedThresholds;
  sourceProbes: CalendarLiveV4FixedSourceProbe[];
  cells: CalendarLiveV4FixedCellValidation[];
  sceneProofs: CalendarLiveV4FixedSceneProof[];
...
export interface CalendarLiveV4FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V4_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV4FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV4FixedHardGates(
  input: CalendarLiveV4FixedHardGateInput,
): CalendarLiveV4FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV4FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V4_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV4FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV4FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
