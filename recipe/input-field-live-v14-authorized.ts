...
import { INPUT_LIVE_V14_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v14.js";
import { verifyInputLiveV14Authorization } from "./input-field-live-v14-authorization.js";
import { runInputLiveV14Preflight } from "./input-field-live-v14-preflight.js";
import { InputLiveV14Orchestrator } from "./run-input-field-live-v14.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V14_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v14-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV14Authorization(root);
    runInputLiveV14Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV14Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
