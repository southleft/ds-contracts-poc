...
} from "./input-field-live-v3-verifier-v30.js";
import {
  buildInputLiveV30RawPropertyRuntime,
  normalizeInputLiveV30Scene,
  type InputLiveV30RawNode,
} from "./input-field-live-v30-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v30.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v30.js";

export {
  buildInputLiveV30RestoreProgram,
  validateInputLiveV30RestorePayload,
  type InputLiveV30RestorePayload,
} from "./input-field-live-v30-restore.js";

export const INPUT_LIVE_V30_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V30_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V30_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V30_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V30_VARIANT_COUNT = 256;
export const INPUT_LIVE_V30_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V30_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV30SourceId = (typeof INPUT_LIVE_V30_SOURCE_IDS)[number];

export interface InputLiveV30SourceIdentity {
  source: InputLiveV30SourceId;
...
export interface InputLiveV30WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV30ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV30SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV30RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV30RootProof {
  source: InputLiveV30SourceId;
  adapterIdentity: string;
...
export interface InputLiveV30ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV30SourceId;
    adapterIdentity: string;
...
export interface InputLiveV30CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV30SourceId;
  adapterIdentity: string;
...
export interface InputLiveV30CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV30SourceId;
  strata: InputLiveV30CaptureCell["strata"];
...
export interface InputLiveV30CleanupPayload {
...
export function validateInputLiveV30WriterPayload(
  value: unknown,
): InputLiveV30WriterOwnership {
...
const identityPlan = (source: InputLiveV30SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV30ExtractProgram(
  writer: InputLiveV30WriterOwnership,
  sources: readonly InputLiveV30SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V30_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V30_NAMESPACE)}
${buildInputLiveV30RawPropertyRuntime()}
...
export function validateInputLiveV30ExtractPayload(
  value: unknown,
  writer: InputLiveV30WriterOwnership,
): InputLiveV30ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V30_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV30ExtractPayload;
}

export function proveInputLiveV30Roots<Instance>(
  extract: InputLiveV30ExtractPayload,
  sources: readonly (InputLiveV30SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV30Scene(
      root.scene,
...
export function assertInputLiveV30RootProofs(
  proofs: readonly InputLiveV30RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV30ProbeProgram(
  writer: InputLiveV30WriterOwnership,
  sources: readonly InputLiveV30SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V30_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV30ProbePayload(
  value: unknown,
  writer: InputLiveV30WriterOwnership,
): InputLiveV30ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV30ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V30_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V30_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV30CaptureManifest(
  cells: readonly InputLiveV30CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V30_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V30_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V30_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V30_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V30_SOURCE_IDS.some(
...
export const inputLiveV30CaptureManifestSha256 = (
  cells: readonly InputLiveV30CaptureCell[],
): string => {
  validateInputLiveV30CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV30CaptureProgram(
  writer: InputLiveV30WriterOwnership,
  cell: InputLiveV30CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V30_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V30_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V30_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV30CapturePayload(
  value: unknown,
  cell: InputLiveV30CaptureCell,
  rawResponseBytes: number,
): InputLiveV30CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV30CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V30_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V30_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV30CaptureResponses(
  manifest: readonly InputLiveV30CaptureCell[],
  responses: readonly InputLiveV30CapturePayload[],
): void {
  validateInputLiveV30CaptureManifest(manifest);
  if (
...
export interface InputLiveV30ObjectiveReport {
  artifactVersion: "input-live-v30-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV30Objective(
  manifest: readonly InputLiveV30CaptureCell[],
  responses: readonly InputLiveV30CapturePayload[],
  readReference: (cell: InputLiveV30CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV30CaptureCell) => Buffer,
): InputLiveV30ObjectiveReport {
  assertInputLiveV30CaptureResponses(manifest, responses);
...
export function buildInputLiveV30CleanupProgram(
  writer: InputLiveV30WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V30_NAMESPACE,
...
export function validateInputLiveV30CleanupPayload(
  value: unknown,
  writer: InputLiveV30WriterOwnership,
): InputLiveV30CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV30CleanupPayload;
...
export const INPUT_LIVE_V30_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV30WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
