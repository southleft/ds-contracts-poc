...
import {
  verifyComboboxLiveV34FixedSceneFixedPoint,
  type ComboboxLiveV34FixedFixedPoint,
} from "./combobox-live-v34-fixed-point.js";

export const COMBOBOX_LIVE_V34_VERIFIER_VERSION = "combobox-live-v34-verifier-v1";

export function buildComboboxLiveV34RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV34RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV34RawNode[];
}

export interface ComboboxLiveV34NormalizedScene {
...
export interface ComboboxLiveV34SceneProof {
...
export function normalizeComboboxLiveV34Scene(
  root: ComboboxLiveV34RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV34NormalizedScene {
  const canonicalBindings: ComboboxLiveV34NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV34RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV34Scene<Instance>(
  normalized: ComboboxLiveV34NormalizedScene,
...
): ComboboxLiveV34SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV34FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV34ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV34PreCaptureGates(payload: unknown): void {
