...
import { verifyTableLiveV29Authorization } from "./table-live-v29-authorization.js";
import { TABLE_LIVE_V29_TARGET } from "./table-live-v29-broker.js";
import {
  TABLE_LIVE_V29_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV29Authorization();
...
const attestation: TableLiveV29SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV29SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v29 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V29_SECURITY_ATTESTATION_DEFAULT_PATH,
);
