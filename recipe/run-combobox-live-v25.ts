...
import {
  acceptComboboxLiveV25Response,
  createComboboxLiveV25Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV25Request,
  persistComboboxLiveV25RawResponse,
  persistComboboxLiveV25TechnicalGates,
  readComboboxLiveV25AcceptedRawPayload,
  type ComboboxLiveV25Request,
  type ComboboxLiveV25TransactionAuthorization,
} from "./combobox-live-v25-broker.js";
import {
  COMBOBOX_LIVE_V25_CAPTURE_COUNT,
  assertComboboxLiveV25CaptureResponses,
  ...
  evaluateComboboxLiveV25Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV25Roots,
  validateComboboxLiveV25CaptureManifest,
  validateComboboxLiveV25CapturePayload,
  validateComboboxLiveV25CleanupPayload,
  validateComboboxLiveV25ExtractPayload,
  validateComboboxLiveV25ProbePayload,
  validateComboboxLiveV25WriterPayload,
  type ComboboxLiveV25CaptureCell,
  type ComboboxLiveV25CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V25_PLAN_PATH,
  type ComboboxLiveV25ProofPlan,
} from "./build-combobox-live-proof-v25.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v25.js";

export const COMBOBOX_LIVE_V25_RUNNER_VERSION = "combobox-live-v25-runner-v1";

interface ComboboxLiveV25JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV25OrchestratorState {
  artifactVersion: "combobox-live-v25-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV25JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV25ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV25CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV25SourceIdentity & {
...
export interface ComboboxLiveV25NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV25OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v25 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V25_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V25_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV25ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV25CaptureCell[];
  validateComboboxLiveV25CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v25 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v25 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV25WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV25RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV25Request =>
  readJson<ComboboxLiveV25Request>(file);

export class ComboboxLiveV25Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV25Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV25OrchestratorState = {
      artifactVersion: "combobox-live-v25-orchestrator-state-v1",
    ...
    return new ComboboxLiveV25Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV25OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV25Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV25NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV25NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v25 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV25NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v25 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV25RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV25Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV25CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV25AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV25WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV25Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV25ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV25Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV25Response(this.#directory, "writer", {
        validate: validateComboboxLiveV25WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV25Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV25RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV25Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV25ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV25Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV25RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v25-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV25Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV25ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV25Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV25ProbePayload(payload, ownership),
      });
      persistComboboxLiveV25TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v25 capture cell missing");
      const accepted = acceptComboboxLiveV25Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV25CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V25_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV25CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV25Objective(
    ...
    throw new Error(`Combobox live v25 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV25Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV25CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV25Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV25RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV25Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV25CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV25CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV25CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV25Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV25TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV25SimulationKey = (): KeyObject =>
