...
export type CalendarLiveV38HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV38AuthorizationArtifact {
  artifactVersion: "calendar-live-v38-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V38_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV38AntecedentIndex {
...
export interface CalendarLiveV38HistoryState {
  index?: CalendarLiveV38AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV38AuthorizationArtifact;
...
export interface CalendarLiveV38AuthorizationProof
  extends CalendarLiveV38TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V38_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V38_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V38_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V38_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v38-authorization") ||
  artifactPath.includes("calendar-live-v38-preflight") ||
  artifactPath.includes("calendar-live-v38-authorized") ||
  artifactPath.includes("create-calendar-live-v38-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v38-status") ||
...
export function validateCalendarLiveV38AntecedentIndex(
  index: CalendarLiveV38AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v38-antecedent-index-v1" ||
...
export function validateCalendarLiveV38UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV38History\(\s*["']/.test(source) ||
    /readCalendarLiveV38HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV38AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV38AuthorizationArtifact {
  ...
  ) as CalendarLiveV38AntecedentIndex;
  const indexFailures = validateCalendarLiveV38AntecedentIndex(index);
...
export function validateCalendarLiveV38AuthorizationArtifact(
  artifact: CalendarLiveV38AuthorizationArtifact | undefined,
  index: CalendarLiveV38AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v38-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v38" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V38_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V38_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V38_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V38_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V38_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V38_SOURCE_ROOTS ||
...
export function validateCalendarLiveV38History(
  state: CalendarLiveV38HistoryState,
  expected: CalendarLiveV38HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV38AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV38AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV38HistoryState(
  root = process.cwd(),
): CalendarLiveV38HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V38_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV38AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V38_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V38_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V38_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV38History(
  expected: CalendarLiveV38HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV38HistoryState {
  const state = readCalendarLiveV38HistoryState(root);
  const failures = validateCalendarLiveV38History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v38 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV38Authorization(
  root = process.cwd(),
): CalendarLiveV38AuthorizationProof {
  const state = verifyCalendarLiveV38History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V38_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v38/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v38.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V38_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V38_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V38_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV38HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV38History(expected);
  process.stdout.write(
    `Calendar live v38 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
