...
import {
  ...
  INPUT_LIVE_V66_INDEX_PATH,
} from "./build-input-field-live-proof-v66.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV66HistoryExpectation = "pending" | "authorized";

export interface InputLiveV66AuthorizationArtifact {
  artifactVersion: "input-live-v66-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V66_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV66AntecedentIndex {
...
export interface InputLiveV66HistoryState {
  index?: InputLiveV66AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV66AuthorizationArtifact;
...
export interface InputLiveV66AuthorizationProof extends InputLiveV66TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V66_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V66_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V66_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V66_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v66-authorization") ||
  artifactPath.includes("input-field-live-v66-preflight") ||
  artifactPath.includes("input-field-live-v66-authorized") ||
  artifactPath.includes("input-field-live-pivot-v66-status") ||
...
export function validateInputLiveV66AntecedentIndex(
  index: InputLiveV66AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v66-antecedent-index-v1" ||
...
export function validateInputLiveV66UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV66History\(\s*["']/.test(source) ||
    /readInputLiveV66HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV66AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV66AuthorizationArtifact {
  ...
  ) as InputLiveV66AntecedentIndex;
  const indexFailures = validateInputLiveV66AntecedentIndex(index);
...
export function validateInputLiveV66AuthorizationArtifact(
  artifact: InputLiveV66AuthorizationArtifact | undefined,
  index: InputLiveV66AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v66-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v66" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V66_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V66_TARGET) ||
    !equalJson(
...
export function validateInputLiveV66History(
  state: InputLiveV66HistoryState,
  expected: InputLiveV66HistoryExpectation,
): string[] {
  const failures = validateInputLiveV66AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV66AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV66HistoryState(
  root = process.cwd(),
): InputLiveV66HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V66_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV66AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V66_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V66_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V66_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV66History(
  expected: InputLiveV66HistoryExpectation,
  root = process.cwd(),
): InputLiveV66HistoryState {
  const state = readInputLiveV66HistoryState(root);
  const failures = validateInputLiveV66History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV66Authorization(
  root = process.cwd(),
): InputLiveV66AuthorizationProof {
  const state = verifyInputLiveV66History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V66_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v66/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v66.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V66_TARGET,
    expectedDynamicTool: INPUT_LIVE_V66_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V66_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV66HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV66History(expected);
