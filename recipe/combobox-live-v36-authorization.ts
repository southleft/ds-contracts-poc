...
export type ComboboxLiveV36HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV36AuthorizationArtifact {
  artifactVersion: "combobox-live-v36-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V36_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV36AntecedentIndex {
...
export interface ComboboxLiveV36HistoryState {
  index?: ComboboxLiveV36AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV36AuthorizationArtifact;
...
export interface ComboboxLiveV36AuthorizationProof extends ComboboxLiveV36TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V36_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V36_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V36_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V36_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v36-authorization") ||
  artifactPath.includes("combobox-live-v36-preflight") ||
  artifactPath.includes("combobox-live-v36-authorized") ||
  artifactPath.includes("create-combobox-live-v36-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v36-status") ||
...
export function validateComboboxLiveV36AntecedentIndex(
  index: ComboboxLiveV36AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v36-antecedent-index-v1" ||
...
export function validateComboboxLiveV36UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV36History\(\s*["']/.test(source) ||
    /readComboboxLiveV36HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV36AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV36AuthorizationArtifact {
  ...
  ) as ComboboxLiveV36AntecedentIndex;
  const indexFailures = validateComboboxLiveV36AntecedentIndex(index);
...
export function validateComboboxLiveV36AuthorizationArtifact(
  artifact: ComboboxLiveV36AuthorizationArtifact | undefined,
  index: ComboboxLiveV36AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v36-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v36" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V36_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V36_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V36_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V36_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V36_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V36_SOURCE_ROOTS ||
...
export function validateComboboxLiveV36History(
  state: ComboboxLiveV36HistoryState,
  expected: ComboboxLiveV36HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV36AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV36AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV36HistoryState(
  root = process.cwd(),
): ComboboxLiveV36HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V36_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV36AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V36_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V36_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V36_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV36History(
  expected: ComboboxLiveV36HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV36HistoryState {
  const state = readComboboxLiveV36HistoryState(root);
  const failures = validateComboboxLiveV36History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v36 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV36Authorization(
  root = process.cwd(),
): ComboboxLiveV36AuthorizationProof {
  const state = verifyComboboxLiveV36History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V36_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v36/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v36.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V36_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V36_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V36_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV36HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV36History(expected);
  process.stdout.write(
    `Combobox live v36 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
