...
import {
  acceptTableLiveV10Response,
  createTableLiveV10Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV10Request,
  persistTableLiveV10RawResponse,
  persistTableLiveV10TechnicalGates,
  readTableLiveV10AcceptedRawPayload,
  type TableLiveV10Request,
  type TableLiveV10TransactionAuthorization,
} from "./table-live-v10-broker.js";
import {
  TABLE_LIVE_V10_CAPTURE_COUNT,
  assertTableLiveV10CaptureResponses,
  ...
  evaluateTableLiveV10Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV10Roots,
  validateTableLiveV10CaptureManifest,
  validateTableLiveV10CapturePayload,
  validateTableLiveV10CleanupPayload,
  validateTableLiveV10ExtractPayload,
  validateTableLiveV10ProbePayload,
  validateTableLiveV10WriterPayload,
  type TableLiveV10CaptureCell,
  type TableLiveV10CapturePayload,
...
import {
  ...
  TABLE_LIVE_V10_PLAN_PATH,
  type TableLiveV10ProofPlan,
...
export const TABLE_LIVE_V10_RUNNER_VERSION = "table-live-v10-runner-v1";

interface TableLiveV10JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV10OrchestratorState {
  artifactVersion: "table-live-v10-orchestrator-state-v1";
  ...
  journal: TableLiveV10JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV10ProofPlan;
  planSha256: string;
  captures: TableLiveV10CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV10SourceIdentity & {
...
export interface TableLiveV10NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV10OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v10 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V10_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V10_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV10ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV10CaptureCell[];
  validateTableLiveV10CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v10 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v10 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV10WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV10RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV10Request =>
  readJson<TableLiveV10Request>(file);

export class TableLiveV10Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV10Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV10OrchestratorState = {
      artifactVersion: "table-live-v10-orchestrator-state-v1",
    ...
    return new TableLiveV10Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV10OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV10Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV10NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV10NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v10 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV10NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v10 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV10RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV10Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV10CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV10AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV10WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV10Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV10ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV10Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV10Response(this.#directory, "writer", {
        validate: validateTableLiveV10WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV10Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV10RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV10Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV10ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV10Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV10RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v10-host-root-proofs-v1",
      ...
      const next = issueTableLiveV10Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV10ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV10Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV10ProbePayload(payload, ownership),
      });
      persistTableLiveV10TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v10 capture cell missing");
      const accepted = acceptTableLiveV10Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV10CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V10_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV10CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV10Objective(
    ...
    throw new Error(`Table live v10 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV10Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV10CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV10Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV10RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV10Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV10CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV10CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV10CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV10Authorization = (
  privateKey: KeyObject,
): TableLiveV10TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV10SimulationKey = (): KeyObject =>
