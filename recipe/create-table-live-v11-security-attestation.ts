...
import { verifyTableLiveV11Authorization } from "./table-live-v11-authorization.js";
import { TABLE_LIVE_V11_TARGET } from "./table-live-v11-broker.js";
import {
  TABLE_LIVE_V11_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV11Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v11-session";
...
const attestation: TableLiveV11SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV11SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v11 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V11_SECURITY_ATTESTATION_DEFAULT_PATH,
);
