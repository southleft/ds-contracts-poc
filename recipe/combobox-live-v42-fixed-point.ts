/**
 * Carried Combobox live v42 host verifier. Uses scene-readback-combobox-v42 so live
...
export const COMBOBOX_LIVE_V42_FIXED_VERIFIER_VERSION = "combobox-live-v42-fixed-point-v1";
export const COMBOBOX_LIVE_V42_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV42FixedThresholds {
...
export interface ComboboxLiveV42FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV42FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV42FixedSourceProbe {
...
export interface ComboboxLiveV42FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV42FixedFixedPoint;
}

export interface ComboboxLiveV42FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV42FixedHardGateInput {
  thresholds: ComboboxLiveV42FixedThresholds;
  materialRegression: ComboboxLiveV42FixedMaterialRegression;
  sourceProbes: ComboboxLiveV42FixedSourceProbe[];
  cells: ComboboxLiveV42FixedCellValidation[];
  sceneProofs: ComboboxLiveV42FixedSceneProof[];
  visualRows: ComboboxLiveV42FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV42FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V42_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV42FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV42FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV42FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV42FixedHardGates(
  input: ComboboxLiveV42FixedHardGateInput,
): ComboboxLiveV42FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV42FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV42FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V42_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV42FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV42FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV42FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
