...
import {
  verifyCalendarLiveV4FixedSceneFixedPoint,
  type CalendarLiveV4FixedFixedPoint,
} from "./calendar-live-v4-fixed-point.js";

export const CALENDAR_LIVE_V4_VERIFIER_VERSION = "calendar-live-v4-verifier-v1";

export function buildCalendarLiveV4RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV4RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV4RawNode[];
}

export interface CalendarLiveV4NormalizedScene {
...
export interface CalendarLiveV4SceneProof {
...
export function normalizeCalendarLiveV4Scene(
  root: CalendarLiveV4RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV4NormalizedScene {
  const canonicalBindings: CalendarLiveV4NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV4RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV4Scene<Instance>(
  normalized: CalendarLiveV4NormalizedScene,
...
): CalendarLiveV4SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV4FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV4ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV4PreCaptureGates(payload: unknown): void {
