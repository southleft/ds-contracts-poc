...
import {
  validateComboboxLiveV12AntecedentIndex,
  verifyComboboxLiveV12Authorization,
  type ComboboxLiveV12AntecedentIndex,
  type ComboboxLiveV12AuthorizationProof,
} from "./combobox-live-v12-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V12_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v12/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V12_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v12-security-attestation.json";

export interface ComboboxLiveV12SecurityAttestation {
  artifactVersion: "combobox-live-v12-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV12PreflightReport {
  artifactVersion: "combobox-live-v12-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV12SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV12SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV12AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v12-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V12_TARGET) ||
...
export function validateComboboxLiveV12ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV12Authorization");
  const preflight = source.indexOf("runComboboxLiveV12Preflight");
  const initialize = source.indexOf("ComboboxLiveV12Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV12Preflight(
  root: string,
  proof: ComboboxLiveV12AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V12_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV12PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V12_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V12_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V12_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV12AntecedentIndex;
  failures.push(...validateComboboxLiveV12AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V12_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V12_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV12SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v12-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV12ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v12 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV12SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v12-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V12_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV12Preflight(
    process.cwd(),
    verifyComboboxLiveV12Authorization(),
    attestationPath,
