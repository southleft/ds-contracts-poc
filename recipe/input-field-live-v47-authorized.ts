...
import { INPUT_LIVE_V47_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v47.js";
import { verifyInputLiveV47Authorization } from "./input-field-live-v47-authorization.js";
import { runInputLiveV47Preflight } from "./input-field-live-v47-preflight.js";
import { InputLiveV47Orchestrator } from "./run-input-field-live-v47.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V47_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v47-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV47Authorization(root);
    runInputLiveV47Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV47Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
