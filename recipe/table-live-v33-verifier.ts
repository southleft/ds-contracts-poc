...
import {
  verifyTableLiveV33FixedSceneFixedPoint,
  type TableLiveV33FixedFixedPoint,
} from "./table-live-v33-fixed-point.js";

export const TABLE_LIVE_V33_VERIFIER_VERSION = "table-live-v33-verifier-v1";

export function buildTableLiveV33RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV33RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV33RawNode[];
}

export interface TableLiveV33NormalizedScene {
...
export interface TableLiveV33SceneProof {
...
export function normalizeTableLiveV33Scene(
  root: TableLiveV33RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV33NormalizedScene {
  const canonicalBindings: TableLiveV33NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV33RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV33Scene<Instance>(
  normalized: TableLiveV33NormalizedScene,
...
): TableLiveV33SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV33FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV33ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV33PreCaptureGates(payload: unknown): void {
