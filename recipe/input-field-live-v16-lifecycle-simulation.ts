...
export interface InputLiveV16LifecycleSimulationReport {
  artifactVersion: "input-live-v16-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV16AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV16Lifecycle(): Promise<InputLiveV16LifecycleSimulationReport> {
  await buildInputLiveV16Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V16_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV16AntecedentIndex;
  const indexSha256 = inputLiveV16Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v16-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V16_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V16_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV16History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV16History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV16AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V16_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V16_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV16History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV16History(authorized, "pending");
    ...
    await buildInputLiveV16Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V16_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV16AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV16Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV16Lifecycle(), null, 2)}\n`,
  );
