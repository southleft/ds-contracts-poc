...
import {
  ...
  verifyInputLiveV61Authorization,
  type InputLiveV61AntecedentIndex,
  type InputLiveV61AuthorizationProof,
} from "./input-field-live-v61-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V61_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v61/operator-security-attestation-template.json";
export const INPUT_LIVE_V61_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v61-security-attestation.json";

export interface InputLiveV61SecurityAttestation {
...
export interface InputLiveV61PreflightReport {
  artifactVersion: "input-live-v61-preflight-v1";
...
export const inputLiveV61AttestationSha256 = (
  value: Omit<InputLiveV61SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV61SecurityAttestation(
  value: unknown,
  proof: InputLiveV61AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v61-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V61_TARGET) ||
...
export function validateInputLiveV61ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV61Authorization");
  const preflight = source.indexOf("runInputLiveV61Preflight");
  const initialize = source.indexOf("InputLiveV61Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV61Preflight(
  root: string,
  proof: InputLiveV61AuthorizationProof,
  attestationPath = INPUT_LIVE_V61_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV61PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V61_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V61_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V61_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV61AntecedentIndex;
  failures.push(...validateInputLiveV61AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V61_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V61_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v61-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV61ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV61SecurityAttestation;
  return {
    artifactVersion: "input-live-v61-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V61_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV61Preflight(
    process.cwd(),
    verifyInputLiveV61Authorization(),
    attestationPath,
