...
import { CALENDAR_LIVE_V29_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v29.js";
import { verifyCalendarLiveV29Authorization } from "./calendar-live-v29-authorization.js";
import { runCalendarLiveV29Preflight } from "./calendar-live-v29-preflight.js";
import { CalendarLiveV29Orchestrator } from "./run-calendar-live-v29.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v29 requires exact authorization artifact ${CALENDAR_LIVE_V29_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v29-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV29Authorization(root);
    runCalendarLiveV29Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV29Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV29Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
