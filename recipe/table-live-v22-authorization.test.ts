...
import { TABLE_LIVE_V22_INDEX_PATH } from "./build-table-live-proof-v22.js";
import {
...
import {
  buildTableLiveV22AuthorizationArtifact,
  validateTableLiveV22AntecedentIndex,
  validateTableLiveV22AuthorizationArtifact,
  validateTableLiveV22History,
  validateTableLiveV22UnitTestSource,
  type TableLiveV22AntecedentIndex,
  type TableLiveV22AuthorizationArtifact,
  type TableLiveV22AuthorizationProof,
  type TableLiveV22HistoryState,
} from "./table-live-v22-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV22ControlFlowSource,
  validateTableLiveV22SecurityAttestation,
  type TableLiveV22SecurityAttestation,
} from "./table-live-v22-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V22_INDEX_PATH);
const index = (): TableLiveV22AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV22AntecedentIndex;

const artifact = (): TableLiveV22AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV22AuthorizationArtifact({
...
const pendingState = (): TableLiveV22HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV22HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV22AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v22/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV22AuthorizationArtifact,
): TableLiveV22SecurityAttestation => {
  const body: Omit<TableLiveV22SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V22_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV22History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV22History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV22History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV22History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV22History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV22UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v22-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV22UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV22AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV22AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV22AuthorizationArtifact(
        planted as TableLiveV22AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV22SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV22SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV22SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v22-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV22ControlFlowSource(source), []);
});
