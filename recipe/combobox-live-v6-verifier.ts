...
import {
  verifyComboboxLiveV6FixedSceneFixedPoint,
  type ComboboxLiveV6FixedFixedPoint,
} from "./combobox-live-v6-fixed-point.js";

export const COMBOBOX_LIVE_V6_VERIFIER_VERSION = "combobox-live-v6-verifier-v1";

export function buildComboboxLiveV6RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV6RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV6RawNode[];
}

export interface ComboboxLiveV6NormalizedScene {
...
export interface ComboboxLiveV6SceneProof {
...
export function normalizeComboboxLiveV6Scene(
  root: ComboboxLiveV6RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV6NormalizedScene {
  const canonicalBindings: ComboboxLiveV6NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV6RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV6Scene<Instance>(
  normalized: ComboboxLiveV6NormalizedScene,
...
): ComboboxLiveV6SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV6FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV6ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV6PreCaptureGates(payload: unknown): void {
