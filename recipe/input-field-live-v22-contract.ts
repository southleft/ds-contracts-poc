...
} from "./input-field-live-v3-verifier-v22.js";
import {
  buildInputLiveV22RawPropertyRuntime,
  normalizeInputLiveV22Scene,
  type InputLiveV22RawNode,
} from "./input-field-live-v22-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v22.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v22.js";
...
export const INPUT_LIVE_V22_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V22_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V22_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V22_VARIANT_COUNT = 256;
export const INPUT_LIVE_V22_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V22_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV22SourceId = (typeof INPUT_LIVE_V22_SOURCE_IDS)[number];

export interface InputLiveV22SourceIdentity {
  source: InputLiveV22SourceId;
...
export interface InputLiveV22WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV22ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV22SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV22RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV22RootProof {
  source: InputLiveV22SourceId;
  adapterIdentity: string;
...
export interface InputLiveV22ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV22SourceId;
    adapterIdentity: string;
...
export interface InputLiveV22CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV22SourceId;
  adapterIdentity: string;
...
export interface InputLiveV22CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV22SourceId;
  strata: InputLiveV22CaptureCell["strata"];
...
export interface InputLiveV22CleanupPayload {
...
export function validateInputLiveV22WriterPayload(
  value: unknown,
): InputLiveV22WriterOwnership {
...
const identityPlan = (source: InputLiveV22SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV22ExtractProgram(
  writer: InputLiveV22WriterOwnership,
  sources: readonly InputLiveV22SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V22_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V22_NAMESPACE)}
${buildInputLiveV22RawPropertyRuntime()}
...
export function validateInputLiveV22ExtractPayload(
  value: unknown,
  writer: InputLiveV22WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V22_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV22ExtractPayload;
}

export function proveInputLiveV22Roots<Instance>(
  extract: InputLiveV22ExtractPayload,
  sources: readonly (InputLiveV22SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV22Scene(
      root.scene,
...
export function assertInputLiveV22RootProofs(
  proofs: readonly InputLiveV22RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV22ProbeProgram(
  writer: InputLiveV22WriterOwnership,
  sources: readonly InputLiveV22SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V22_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV22ProbePayload(
  value: unknown,
  writer: InputLiveV22WriterOwnership,
): InputLiveV22ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV22ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V22_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V22_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV22CaptureManifest(
  cells: readonly InputLiveV22CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V22_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V22_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V22_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V22_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V22_SOURCE_IDS.some(
...
export const inputLiveV22CaptureManifestSha256 = (
  cells: readonly InputLiveV22CaptureCell[],
): string => {
  validateInputLiveV22CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV22CaptureProgram(
  writer: InputLiveV22WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V22_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V22_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V22_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV22CapturePayload(
  value: unknown,
  cell: InputLiveV22CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV22CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V22_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V22_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV22CaptureResponses(
  manifest: readonly InputLiveV22CaptureCell[],
  responses: readonly InputLiveV22CapturePayload[],
): void {
  validateInputLiveV22CaptureManifest(manifest);
  if (
...
export interface InputLiveV22ObjectiveReport {
  artifactVersion: "input-live-v22-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV22Objective(
  manifest: readonly InputLiveV22CaptureCell[],
  responses: readonly InputLiveV22CapturePayload[],
  readReference: (cell: InputLiveV22CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV22CaptureCell) => Buffer,
): InputLiveV22ObjectiveReport {
  assertInputLiveV22CaptureResponses(manifest, responses);
...
export function buildInputLiveV22CleanupProgram(
  writer: InputLiveV22WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V22_NAMESPACE,
...
export function validateInputLiveV22CleanupPayload(
  value: unknown,
  writer: InputLiveV22WriterOwnership,
): InputLiveV22CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV22CleanupPayload;
...
export const INPUT_LIVE_V22_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV22WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
