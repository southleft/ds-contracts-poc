...
} from "./input-field-live-v3-verifier-v46.js";
import {
  buildInputLiveV46RawPropertyRuntime,
  normalizeInputLiveV46Scene,
  type InputLiveV46RawNode,
} from "./input-field-live-v46-verifier.js";
...
export {
  ...
  type InputLiveV46RestorePayload,
} from "./input-field-live-v46-restore.js";

export const INPUT_LIVE_V46_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V46_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V46_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V46_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V46_VARIANT_COUNT = 256;
export const INPUT_LIVE_V46_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V46_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV46SourceId = (typeof INPUT_LIVE_V46_SOURCE_IDS)[number];

export interface InputLiveV46SourceIdentity {
  source: InputLiveV46SourceId;
...
export interface InputLiveV46WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV46ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV46SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV46RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV46RootProof {
  source: InputLiveV46SourceId;
  adapterIdentity: string;
...
export interface InputLiveV46ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV46SourceId;
    adapterIdentity: string;
...
export interface InputLiveV46CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV46SourceId;
  adapterIdentity: string;
...
export interface InputLiveV46CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV46SourceId;
  strata: InputLiveV46CaptureCell["strata"];
...
export interface InputLiveV46CleanupPayload {
...
export function validateInputLiveV46WriterPayload(
  value: unknown,
): InputLiveV46WriterOwnership {
...
const identityPlan = (source: InputLiveV46SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV46ExtractProgram(
  writer: InputLiveV46WriterOwnership,
  sources: readonly InputLiveV46SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V46_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V46_NAMESPACE)}
${buildInputLiveV46RawPropertyRuntime()}
...
export function validateInputLiveV46ExtractPayload(
  value: unknown,
  writer: InputLiveV46WriterOwnership,
): InputLiveV46ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V46_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV46ExtractPayload;
}

export function proveInputLiveV46Roots<Instance>(
  extract: InputLiveV46ExtractPayload,
  sources: readonly (InputLiveV46SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV46Scene(
      root.scene,
...
export function assertInputLiveV46RootProofs(
  proofs: readonly InputLiveV46RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV46ProbeProgram(
  writer: InputLiveV46WriterOwnership,
  sources: readonly InputLiveV46SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V46_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV46ProbePayload(
  value: unknown,
  writer: InputLiveV46WriterOwnership,
): InputLiveV46ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV46ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V46_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V46_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV46CaptureManifest(
  cells: readonly InputLiveV46CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V46_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V46_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V46_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V46_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V46_SOURCE_IDS.some(
...
export const inputLiveV46CaptureManifestSha256 = (
  cells: readonly InputLiveV46CaptureCell[],
): string => {
  validateInputLiveV46CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV46CaptureProgram(
  writer: InputLiveV46WriterOwnership,
  cell: InputLiveV46CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V46_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V46_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V46_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV46CapturePayload(
  value: unknown,
  cell: InputLiveV46CaptureCell,
  rawResponseBytes: number,
): InputLiveV46CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV46CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V46_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V46_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV46CaptureResponses(
  manifest: readonly InputLiveV46CaptureCell[],
  responses: readonly InputLiveV46CapturePayload[],
): void {
  validateInputLiveV46CaptureManifest(manifest);
  if (
...
export interface InputLiveV46ObjectiveReport {
  artifactVersion: "input-live-v46-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV46Objective(
  manifest: readonly InputLiveV46CaptureCell[],
  responses: readonly InputLiveV46CapturePayload[],
  readReference: (cell: InputLiveV46CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV46CaptureCell) => Buffer,
): InputLiveV46ObjectiveReport {
  assertInputLiveV46CaptureResponses(manifest, responses);
...
export function buildInputLiveV46CleanupProgram(
  writer: InputLiveV46WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V46_NAMESPACE,
...
export function validateInputLiveV46CleanupPayload(
  value: unknown,
  writer: InputLiveV46WriterOwnership,
): InputLiveV46CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV46CleanupPayload;
...
export const INPUT_LIVE_V46_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV46WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
