...
import { CALENDAR_LIVE_V5_INDEX_PATH } from "./build-calendar-live-proof-v5.js";
import {
...
import {
  buildCalendarLiveV5AuthorizationArtifact,
  validateCalendarLiveV5AntecedentIndex,
  validateCalendarLiveV5AuthorizationArtifact,
  validateCalendarLiveV5History,
  validateCalendarLiveV5UnitTestSource,
  type CalendarLiveV5AntecedentIndex,
  type CalendarLiveV5AuthorizationArtifact,
  type CalendarLiveV5AuthorizationProof,
  type CalendarLiveV5HistoryState,
} from "./calendar-live-v5-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV5ControlFlowSource,
  validateCalendarLiveV5SecurityAttestation,
  type CalendarLiveV5SecurityAttestation,
} from "./calendar-live-v5-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V5_INDEX_PATH);
const index = (): CalendarLiveV5AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV5AntecedentIndex;

const artifact = (): CalendarLiveV5AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV5AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV5HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV5HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV5AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v5/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV5AuthorizationArtifact,
): CalendarLiveV5SecurityAttestation => {
  const body: Omit<CalendarLiveV5SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V5_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV5History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV5History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV5History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV5History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV5History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV5UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v5-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV5UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV5AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV5AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV5AuthorizationArtifact(
        planted as CalendarLiveV5AuthorizationArtifact,
        index(),
...
test("one-root denominator, capture gate, cleanup, and prior-lineage reuse are rejected", () => {
  ...
  for (const mutate of [
    ...
    (value: Record<string, any>) => {
      value.execution.calendarLiveV4AuthorizationReusable = true;
    },
    (value: Record<string, any>) => {
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV5SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV5SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV5SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v5-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV5ControlFlowSource(source), []);
});
