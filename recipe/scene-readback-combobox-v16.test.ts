...
import {
  ...
  COMBOBOX_LIVE_V16_LEADING_SLOT_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V16_LEADING_SLOT_COMPILE_CARRY_VISIBLE_MARKER,
  COMBOBOX_LIVE_V16_OMIT_EMPTY_INSTANCE_PAYLOAD_MARKER,
  COMBOBOX_LIVE_V16_OVERLAY_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V16_PROJECT_LIVE_ROOT_OWNERSHIP_KEY_MARKER,
  COMBOBOX_LIVE_V16_RECOVER_COMPONENT_PROPERTY_NAME_MARKER,
  COMBOBOX_LIVE_V16_RECOVER_RECIPE_COMPONENT_REF_MARKER,
  COMBOBOX_LIVE_V16_TRAILING_SLOT_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V16_TRAILING_SLOT_COMPILE_CARRY_VISIBLE_MARKER,
  COMBOBOX_LIVE_V16_TRIGGER_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V16_TRIGGER_EMPTY_EFFECTS_OMITTED_MARKER,
...
test("host orders combobox/overlay bindings to compile field order, not live extract order", () => {
  assert.equal(
    COMBOBOX_LIVE_V16_OVERLAY_BINDING_COMPILE_ORDER_MARKER,
  ...
  assert.deepEqual(
    ...
    "v16 compile-order list must rank layout.width.value then fills, strokes, effects, then cornerRadius corners",
  );
  const instance = adaptReviewedCombobox(
    muiComboboxSource,
    muiComboboxAdapterConfig,
  ...
  const hashed = sceneToNormalizedIrV15(live);
  assert.equal(hashed.kind, "frame");
  assert.deepEqual(
    (hashed.bindings ?? []).map((binding) => binding.field),
    [
  ...
  assert.deepEqual(
    ...
    "v16 host must keep compile field order; width.value stays unknown/last and is not aliased",
  );
  assert.equal(
    observed.effects && observed.effects.length,
    1,
    "v16 must keep overlay compile-carried drop-shadow",
  );
});
