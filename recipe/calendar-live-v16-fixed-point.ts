/**
 * Calendar live v16 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V16_FIXED_VERIFIER_VERSION = "calendar-live-v16-fixed-point-v1";
export const CALENDAR_LIVE_V16_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV16FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV16FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV16FixedSourceProbe {
...
export interface CalendarLiveV16FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV16FixedFixedPoint;
}

export interface CalendarLiveV16FixedHardGateInput {
  thresholds: CalendarLiveV16FixedThresholds;
  sourceProbes: CalendarLiveV16FixedSourceProbe[];
  cells: CalendarLiveV16FixedCellValidation[];
  sceneProofs: CalendarLiveV16FixedSceneProof[];
...
export interface CalendarLiveV16FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V16_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV16FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV16FixedHardGates(
  input: CalendarLiveV16FixedHardGateInput,
): CalendarLiveV16FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV16FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V16_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV16FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV16FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV16FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
