...
import { buildCalendarLiveV34CleanupRuntime } from "./calendar-live-v34-cleanup.js";
import { type CalendarLiveV34FixedFixedPoint } from "./calendar-live-v34-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV34RestoreProgram,
  validateCalendarLiveV34RestorePayload,
  type CalendarLiveV34RestorePayload,
} from "./calendar-live-v34-restore.js";
import {
  buildCalendarLiveV34RawPropertyRuntime,
  normalizeCalendarLiveV34Scene,
  type CalendarLiveV34RawNode,
} from "./calendar-live-v34-verifier.js";
...
export const CALENDAR_LIVE_V34_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V34_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V34_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V34_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V34_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V34_SET_COUNT = 3;
export const CALENDAR_LIVE_V34_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V34_HOST_PHASES = 3;
export const CALENDAR_LIVE_V34_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V34_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V34_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V34_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV34SourceId = (typeof CALENDAR_LIVE_V34_SOURCE_IDS)[number];
export type CalendarLiveV34Kind = "calendar" | "week" | "day";

export interface CalendarLiveV34SourceIdentity {
  source: CalendarLiveV34SourceId;
...
export interface CalendarLiveV34WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV34ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV34SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV34RawNode;
    weekScene: CalendarLiveV34RawNode;
    dayScene: CalendarLiveV34RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV34RootProof {
  source: CalendarLiveV34SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV34FixedFixedPoint;
}

export interface CalendarLiveV34ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV34SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV34CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV34SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV34CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV34SourceId;
  frameWidth: number;
...
export interface CalendarLiveV34CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v34 ${label} must be an object`);
...
export function validateCalendarLiveV34WriterPayload(
  value: unknown,
): CalendarLiveV34WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v34") ||
    ...
    result.namespace !== CALENDAR_LIVE_V34_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v34 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v34 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v34 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV34SourceIdentity,
  kind: CalendarLiveV34Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV34ExtractProgram(
  writer: CalendarLiveV34WriterOwnership,
  sources: readonly CalendarLiveV34SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v34 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V34_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V34_NAMESPACE)}
${buildCalendarLiveV34RawPropertyRuntime()}
...
export function validateCalendarLiveV34ExtractPayload(
  value: unknown,
  writer: CalendarLiveV34WriterOwnership,
): CalendarLiveV34ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v34 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V34_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v34 extract schema/one-root mismatch");
  return payload as CalendarLiveV34ExtractPayload;
}

export function proveCalendarLiveV34Roots<Instance>(
  extract: CalendarLiveV34ExtractPayload,
  sources: readonly (CalendarLiveV34SourceIdentity & {
...
): CalendarLiveV34RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v34 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v34 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV34Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV34Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV34Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV34FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v34 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v34 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v34 compile lost owned sets");
...
export function assertCalendarLiveV34RootProofs(
  proofs: readonly CalendarLiveV34RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v34 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v34 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV34ProbeProgram(
  writer: CalendarLiveV34WriterOwnership,
  sources: readonly CalendarLiveV34SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V34_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV34ProbePayload(
  value: unknown,
  writer: CalendarLiveV34WriterOwnership,
): CalendarLiveV34ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV34ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V34_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V34_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v34 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV34CaptureManifest(
  cells: readonly CalendarLiveV34CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V34_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V34_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V34_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V34_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v34 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV34CaptureCell[],
): string => {
  validateCalendarLiveV34CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV34CaptureProgram(
  writer: CalendarLiveV34WriterOwnership,
  cell: CalendarLiveV34CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V34_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v34 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v34 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V34_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV34CapturePayload(
  value: unknown,
  cell: CalendarLiveV34CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV34CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV34CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V34_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V34_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v34 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV34CaptureResponses(
  manifest: readonly CalendarLiveV34CaptureCell[],
  responses: readonly CalendarLiveV34CapturePayload[],
): void {
  validateCalendarLiveV34CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v34 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV34ObjectiveReport {
  artifactVersion: "calendar-live-v34-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV34Objective(
  manifest: readonly CalendarLiveV34CaptureCell[],
  responses: readonly CalendarLiveV34CapturePayload[],
): CalendarLiveV34ObjectiveReport {
  assertCalendarLiveV34CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v34-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV34CleanupProgram(
  writer: CalendarLiveV34WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV34CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V34_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v34 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV34CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV34WriterOwnership,
): CalendarLiveV34CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV34CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v34 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V34_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV34WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
