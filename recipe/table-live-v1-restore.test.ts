...
import {
  ...
  TABLE_LIVE_V1_MEASURE_HUG_MARKER,
  TABLE_LIVE_V1_RESTORE_CELL_SET_ROLE,
  TABLE_LIVE_V1_RESTORE_CONTENT_ROLES,
  TABLE_LIVE_V1_RESTORE_COUNT,
...
test("Table restore owns HUG labels from cell-set only, never 144 FILL texts", () => {
  ...
  assert.equal(TABLE_LIVE_V1_RESTORE_COUNT, 8);
  assert.equal(TABLE_LIVE_V1_RESTORE_CELL_SET_ROLE, "table/cell-set");
  assert.deepEqual([...TABLE_LIVE_V1_RESTORE_CONTENT_ROLES], [
    "table/cell/label",
  ]);
  assert.throws(() =>
    buildTableLiveV1RestoreProgram({
      pageId: FORBIDDEN_INPUT_PAGE_ID,
      runIdentity: "x-table-v1",
      setIds: ["a", "b", "c", "d", "e", "f"],
    }),
  );
  assert.throws(() =>
  ...
  const program = buildTableLiveV1RestoreProgram({
    pageId: "1:2",
    runIdentity: "x-table-v1",
    setIds: ["a", "b", "c", "d", "e", "f"],
  });
  ...
  assert.match(program, /TABLE-V1-RESTORE-COUNT/);
  assert.match(program, /TABLE-V1-RESTORE-INVENTED-FILL/);
  assert.match(program, new RegExp(TABLE_LIVE_V1_MEASURE_HUG_MARKER));
