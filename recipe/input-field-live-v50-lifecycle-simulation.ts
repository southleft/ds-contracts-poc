...
export interface InputLiveV50LifecycleSimulationReport {
  artifactVersion: "input-live-v50-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV50AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV50Lifecycle(): Promise<InputLiveV50LifecycleSimulationReport> {
  await buildInputLiveV50Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V50_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV50AntecedentIndex;
  const indexSha256 = inputLiveV50Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v50-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V50_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V50_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV50History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV50History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV50AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V50_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V50_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV50History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV50History(authorized, "pending");
    ...
    await buildInputLiveV50Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V50_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV50AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV50Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV50Lifecycle(), null, 2)}\n`,
  );
