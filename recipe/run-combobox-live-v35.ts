...
import {
  acceptComboboxLiveV35Response,
  createComboboxLiveV35Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV35Request,
  persistComboboxLiveV35RawResponse,
  persistComboboxLiveV35TechnicalGates,
  readComboboxLiveV35AcceptedRawPayload,
  type ComboboxLiveV35Request,
  type ComboboxLiveV35TransactionAuthorization,
} from "./combobox-live-v35-broker.js";
import {
  COMBOBOX_LIVE_V35_CAPTURE_COUNT,
  assertComboboxLiveV35CaptureResponses,
  ...
  evaluateComboboxLiveV35Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV35Roots,
  validateComboboxLiveV35CaptureManifest,
  validateComboboxLiveV35CapturePayload,
  validateComboboxLiveV35CleanupPayload,
  validateComboboxLiveV35ExtractPayload,
  validateComboboxLiveV35ProbePayload,
  validateComboboxLiveV35WriterPayload,
  type ComboboxLiveV35CaptureCell,
  type ComboboxLiveV35CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V35_PLAN_PATH,
  type ComboboxLiveV35ProofPlan,
} from "./build-combobox-live-proof-v35.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v35.js";

export const COMBOBOX_LIVE_V35_RUNNER_VERSION = "combobox-live-v35-runner-v1";

interface ComboboxLiveV35JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV35OrchestratorState {
  artifactVersion: "combobox-live-v35-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV35JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV35ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV35CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV35SourceIdentity & {
...
export interface ComboboxLiveV35NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV35OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v35 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V35_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V35_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV35ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV35CaptureCell[];
  validateComboboxLiveV35CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v35 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v35 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV35WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV35RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV35Request =>
  readJson<ComboboxLiveV35Request>(file);

export class ComboboxLiveV35Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV35Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV35OrchestratorState = {
      artifactVersion: "combobox-live-v35-orchestrator-state-v1",
    ...
    return new ComboboxLiveV35Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV35OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV35Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV35NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV35NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v35 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV35NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v35 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV35RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV35Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV35CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV35AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV35WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV35Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV35ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV35Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV35Response(this.#directory, "writer", {
        validate: validateComboboxLiveV35WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV35Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV35RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV35Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV35ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV35Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV35RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v35-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV35Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV35ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV35Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV35ProbePayload(payload, ownership),
      });
      persistComboboxLiveV35TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v35 capture cell missing");
      const accepted = acceptComboboxLiveV35Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV35CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V35_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV35CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV35Objective(
    ...
    throw new Error(`Combobox live v35 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV35Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV35CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV35Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV35RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV35Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV35CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV35CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV35CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV35Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV35TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV35SimulationKey = (): KeyObject =>
