/**
 * Table live v17 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V17_FIXED_VERIFIER_VERSION = "table-live-v17-fixed-point-v1";
export const TABLE_LIVE_V17_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV17FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV17FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV17FixedSourceProbe {
...
export interface TableLiveV17FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV17FixedFixedPoint;
}

export interface TableLiveV17FixedHardGateInput {
  thresholds: TableLiveV17FixedThresholds;
  sourceProbes: TableLiveV17FixedSourceProbe[];
  cells: TableLiveV17FixedCellValidation[];
  sceneProofs: TableLiveV17FixedSceneProof[];
...
export interface TableLiveV17FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V17_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV17FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV17FixedHardGates(
  input: TableLiveV17FixedHardGateInput,
): TableLiveV17FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV17FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V17_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV17FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV17FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
