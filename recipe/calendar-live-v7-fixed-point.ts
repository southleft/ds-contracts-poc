/**
 * Calendar live v7 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V7_FIXED_VERIFIER_VERSION = "calendar-live-v7-fixed-point-v1";
export const CALENDAR_LIVE_V7_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV7FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV7FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV7FixedSourceProbe {
...
export interface CalendarLiveV7FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV7FixedFixedPoint;
}

export interface CalendarLiveV7FixedHardGateInput {
  thresholds: CalendarLiveV7FixedThresholds;
  sourceProbes: CalendarLiveV7FixedSourceProbe[];
  cells: CalendarLiveV7FixedCellValidation[];
  sceneProofs: CalendarLiveV7FixedSceneProof[];
...
export interface CalendarLiveV7FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V7_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV7FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV7FixedHardGates(
  input: CalendarLiveV7FixedHardGateInput,
): CalendarLiveV7FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV7FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V7_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV7FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV7FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
