...
export const INPUT_LIVE_V71_VERIFIER_VERSION = "input-live-v71-verifier-v1";

export function buildInputLiveV71RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV71RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV71RawNode[];
}

export interface InputLiveV71NormalizedScene {
...
export interface InputLiveV71SceneProof {
...
export function normalizeInputLiveV71Scene(
  root: InputLiveV71RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV71NormalizedScene {
  const canonicalBindings: InputLiveV71NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV71RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV71Scene<Instance>(
  normalized: InputLiveV71NormalizedScene,
...
export function compileInputLiveV71ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV71PreCaptureGates(payload: unknown): void {
