...
} from "./input-field-live-v3-verifier-v11.js";
import {
  buildInputLiveV11RawPropertyRuntime,
  normalizeInputLiveV11Scene,
  type InputLiveV11RawNode,
} from "./input-field-live-v11-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v11.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v11.js";

export const INPUT_LIVE_V11_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V11_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V11_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V11_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V11_VARIANT_COUNT = 256;
export const INPUT_LIVE_V11_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V11_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV11SourceId = (typeof INPUT_LIVE_V11_SOURCE_IDS)[number];

export interface InputLiveV11SourceIdentity {
  source: InputLiveV11SourceId;
...
export interface InputLiveV11WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV11ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV11SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV11RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV11RootProof {
  source: InputLiveV11SourceId;
  adapterIdentity: string;
...
export interface InputLiveV11ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV11SourceId;
    adapterIdentity: string;
...
export interface InputLiveV11CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV11SourceId;
  adapterIdentity: string;
...
export interface InputLiveV11CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV11SourceId;
  strata: InputLiveV11CaptureCell["strata"];
...
export interface InputLiveV11CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Input live v11 ${label} must be an object`);
...
const exactUniqueStrings = (
  ...
  if (
    ...
    throw new TypeError(`Input live v11 ${label} cardinality/identity mismatch`);
  return value;
};
const finiteNonnegative = (value: unknown): value is number =>
  typeof value === "number" && Number.isFinite(value) && value >= 0;

export function validateInputLiveV11WriterPayload(
  value: unknown,
): InputLiveV11WriterOwnership {
  ...
  if (
    ...
    throw new TypeError("Input live v11 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Input live v11 writer created-node denominator invalid",
    );
  return {
...
const identityPlan = (source: InputLiveV11SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV11ExtractProgram(
  writer: InputLiveV11WriterOwnership,
  sources: readonly InputLiveV11SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Input live v11 extract requires two source identities");
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V11_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V11_NAMESPACE)}
${buildInputLiveV11RawPropertyRuntime()}
...
export function validateInputLiveV11ExtractPayload(
  value: unknown,
  writer: InputLiveV11WriterOwnership,
): InputLiveV11ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Input live v11 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V11_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Input live v11 extract schema/two-root mismatch");
  return payload as InputLiveV11ExtractPayload;
}

export function proveInputLiveV11Roots<Instance>(
  extract: InputLiveV11ExtractPayload,
  sources: readonly (InputLiveV11SourceIdentity & {
...
): InputLiveV11RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Input live v11 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Input live v11 omitted ${source.source} root`);
    const normalized = normalizeInputLiveV11Scene(
      root.scene,
...
export function assertInputLiveV11RootProofs(
  proofs: readonly InputLiveV11RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Input live v11 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Input live v11 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildInputLiveV11ProbeProgram(
  writer: InputLiveV11WriterOwnership,
  sources: readonly InputLiveV11SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V11_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV11ProbePayload(
  value: unknown,
  writer: InputLiveV11WriterOwnership,
): InputLiveV11ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV11ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V11_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V11_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Input live v11 probe/usability/restoration failed");
  return payload;
}

export function validateInputLiveV11CaptureManifest(
  cells: readonly InputLiveV11CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V11_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V11_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V11_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V11_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V11_SOURCE_IDS.some(
      (source) => cells.filter((cell) => cell.source === source).length !== 64,
    )
  )
    throw new TypeError(
      "Input live v11 capture manifest must be exact 128 cells",
    );
}

export const inputLiveV11CaptureManifestSha256 = (
  cells: readonly InputLiveV11CaptureCell[],
): string => {
  validateInputLiveV11CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV11CaptureProgram(
  writer: InputLiveV11WriterOwnership,
  cell: InputLiveV11CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V11_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V11_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
    ...
    throw new TypeError("Input live v11 capture cell is malformed");
  const expectedSet = writer.sources.find(
    (source) => source.adapterIdentity === cell.adapterIdentity,
  )?.setId;
  if (!expectedSet)
    throw new TypeError(
      `Input live v11 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V11_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV11CapturePayload(
  value: unknown,
  cell: InputLiveV11CaptureCell,
  rawResponseBytes: number,
): InputLiveV11CapturePayload {
  const payload = record(value, "capture payload") as InputLiveV11CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V11_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V11_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Input live v11 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertInputLiveV11CaptureResponses(
  manifest: readonly InputLiveV11CaptureCell[],
  responses: readonly InputLiveV11CapturePayload[],
): void {
  validateInputLiveV11CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Input live v11 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface InputLiveV11ObjectiveReport {
  artifactVersion: "input-live-v11-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV11Objective(
  manifest: readonly InputLiveV11CaptureCell[],
  responses: readonly InputLiveV11CapturePayload[],
  readReference: (cell: InputLiveV11CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV11CaptureCell) => Buffer,
): InputLiveV11ObjectiveReport {
  assertInputLiveV11CaptureResponses(manifest, responses);
  const rows = manifest.map((cell, index) => {
    ...
    if (
      ...
      throw new TypeError(
        `Input live v11 objective byte drift: ${cell.cellKey}`,
      );
    ...
    if (
      ...
      throw new TypeError(
        `Input live v11 objective measurement refused: ${cell.cellKey}`,
      );
    return {
      index: cell.index,
...
export function buildInputLiveV11CleanupProgram(
  writer: InputLiveV11WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V11_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Input live v11 cleanup runtime result hook absent");
  return String.raw`
...
export function validateInputLiveV11CleanupPayload(
  value: unknown,
  writer: InputLiveV11WriterOwnership,
): InputLiveV11CleanupPayload {
  const payload = record(value, "cleanup payload") as InputLiveV11CleanupPayload;
  ...
  if (
    ...
    throw new TypeError("Input live v11 cleanup is incomplete or overbroad");
  return payload;
}

export const INPUT_LIVE_V11_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV11WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
