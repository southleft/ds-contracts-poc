...
import {
  ...
  CALENDAR_LIVE_V44_HOST_PHASES,
  CALENDAR_LIVE_V44_REMOTE_REQUESTS,
  CALENDAR_LIVE_V44_SOURCE_ROOTS,
  CALENDAR_LIVE_V44_VARIANT_COUNT,
  buildCalendarLiveV44CaptureProgram,
  buildCalendarLiveV44CleanupProgram,
  buildCalendarLiveV44ExtractProgram,
  buildCalendarLiveV44ProbeProgram,
  buildCalendarLiveV44RestoreProgram,
  calendarLiveV1CaptureManifestSha256,
  type CalendarLiveV44CaptureCell,
  type CalendarLiveV44SourceIdentity,
  type CalendarLiveV44WriterOwnership,
} from "./calendar-live-v44-contract.js";
import {
  CALENDAR_LIVE_V44_DYNAMIC_TOOL,
  CALENDAR_LIVE_V44_TARGET,
  calendarLiveV1RequestSequence,
} from "./calendar-live-v44-broker.js";
import {
...
export const CALENDAR_LIVE_V44_EVIDENCE_ROOT =
  "recipe/evidence/calendar-live-pivot-v44";
export const CALENDAR_LIVE_V44_PROTOCOL_PATH = `${CALENDAR_LIVE_V44_EVIDENCE_ROOT}/protocol.json`;
export const CALENDAR_LIVE_V44_PLAN_PATH = `${CALENDAR_LIVE_V44_EVIDENCE_ROOT}/proof-plan.json`;
export const CALENDAR_LIVE_V44_CAPTURE_MANIFEST_PATH = `${CALENDAR_LIVE_V44_EVIDENCE_ROOT}/capture-manifest.json`;
export const CALENDAR_LIVE_V44_REQUEST_MANIFEST_PATH = `${CALENDAR_LIVE_V44_EVIDENCE_ROOT}/request-manifest.json`;
export const CALENDAR_LIVE_V44_INDEX_PATH = `${CALENDAR_LIVE_V44_EVIDENCE_ROOT}/antecedent-index.json`;
export const CALENDAR_LIVE_V44_AUTHORIZATION_TEMPLATE_PATH = `${CALENDAR_LIVE_V44_EVIDENCE_ROOT}/authorization-template.json`;
export const CALENDAR_LIVE_V44_AUTHORIZATION_PATH = `${CALENDAR_LIVE_V44_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface CalendarLiveV44ProofPlan {
  artifactVersion: "calendar-live-v44-proof-plan-v1";
...
const captureCells = (
...
): CalendarLiveV44CaptureCell[] => {
  const cells: CalendarLiveV44CaptureCell[] = [];
  ...
  if (cells.length !== CALENDAR_LIVE_V44_CAPTURE_COUNT)
    throw new Error(
      `Calendar live v44 capture cells ${cells.length}, expected ${CALENDAR_LIVE_V44_CAPTURE_COUNT}`,
    );
...
const protocol = (expectedFacts: number) => ({
  artifactVersion: "calendar-live-v44-external-operator-protocol-v1",
  protocolId: "calendar-live-v44",
  status:
  ...
  lifecycle: {
    ...
    comboboxLiveV44AuthorizationReusable: false,
  ...
  denominator: {
    sourceRoots: CALENDAR_LIVE_V44_SOURCE_ROOTS,
    sets: 3,
    variants: CALENDAR_LIVE_V44_VARIANT_COUNT,
    captureCells: CALENDAR_LIVE_V44_CAPTURE_COUNT,
    remoteRequests: CALENDAR_LIVE_V44_REMOTE_REQUESTS,
    hostPhases: CALENDAR_LIVE_V44_HOST_PHASES,
...
export async function buildCalendarLiveV44Proof(
  check = process.argv.includes("--check"),
): Promise<CalendarLiveV44ProofPlan> {
  ...
  if (
    ...
    !writer.runIdentity.endsWith("-calendar-v44") ||
    ...
    throw new Error(
      "Calendar live v44 writer reused Input, Combobox, or Table v1–v24 identity",
    );
  const captures = captureCells(sources);
  const writerProgram = Buffer.from(writer.code);
  const writerOwnershipBlueprint: CalendarLiveV44WriterOwnership = {
  ...
  const identities: CalendarLiveV44SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildCalendarLiveV44RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildCalendarLiveV44ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildCalendarLiveV44ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildCalendarLiveV44CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildCalendarLiveV44CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  const sourceMetadata = sources.map((source) => {
    ...
    const tablePath = `${CALENDAR_LIVE_V44_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-calendar.json.gz`;
    const rowPath = `${CALENDAR_LIVE_V44_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-week.json.gz`;
    const cellPath = `${CALENDAR_LIVE_V44_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-day.json.gz`;
  ...
  antecedentOutputs.set(
    CALENDAR_LIVE_V44_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V44_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V44_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V44_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V44_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V44_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V44_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "calendar-live-v44-capture-manifest-v1",
  ...
  antecedentOutputs.set(
    CALENDAR_LIVE_V44_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: CalendarLiveV44ProofPlan = {
    artifactVersion: "calendar-live-v44-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: CALENDAR_LIVE_V44_TARGET,
    namespace: CALENDAR_FIGMA_NAMESPACE,
    writer: {
      programPath: `${CALENDAR_LIVE_V44_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: CALENDAR_LIVE_V44_CAPTURE_MANIFEST_PATH,
      cells: 8,
      requests: 8,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(CALENDAR_LIVE_V44_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "calendar-live-v44-request-manifest-v1",
    expectedDynamicTool: CALENDAR_LIVE_V44_DYNAMIC_TOOL,
    target: CALENDAR_LIVE_V44_TARGET,
    signedAtRuntime: true,
    requestCount: CALENDAR_LIVE_V44_REMOTE_REQUESTS,
    requests: [
      {
        requestId: "writer",
  ...
  if (requests.requestCount !== CALENDAR_LIVE_V44_REMOTE_REQUESTS)
    throw new Error("Calendar live v44 remote request denominator drifted");
  antecedentOutputs.set(
    CALENDAR_LIVE_V44_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    CALENDAR_LIVE_V44_AUTHORIZATION_TEMPLATE_PATH,
    CALENDAR_LIVE_V44_AUTHORIZATION_PATH,
    "recipe/calendar-live-v44-authorization.ts",
    "recipe/calendar-live-v44-authorization.test.ts",
    "recipe/calendar-live-v44-preflight.ts",
    "recipe/calendar-live-v44-authorized.ts",
    "recipe/write-calendar-live-v44-authorization.ts",
    "recipe/create-calendar-live-v44-security-attestation.ts",
    "recipe/evidence/calendar-live-pivot-v44/operator-security-attestation-template.json",
    "recipe/evidence/calendar-live-pivot-v44-status.json",
    "recipe/evidence/status-index.json",
  ];
  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/calendar-live-v44-broker.ts",
    "recipe/calendar-live-v44-contract.ts",
    "recipe/calendar-live-v44-restore.ts",
    "recipe/calendar-live-v44-cleanup.ts",
    "recipe/calendar-live-v44-verifier.ts",
    "recipe/calendar-live-v44-fixed-point.ts",
    "recipe/run-calendar-live-v44.ts",
    "recipe/build-calendar-live-proof-v44.ts",
    "recipe/calendar-live-v44-lifecycle-simulation.ts",
    ...
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== CALENDAR_LIVE_V44_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "calendar-live-v44-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
  ...
  antecedentOutputs.set(CALENDAR_LIVE_V44_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "calendar-live-v44-authorization-template-v1",
    authorizationId: "calendar-live-v44",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: CALENDAR_LIVE_V44_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
    ...
    denominator: {
      remoteRequests: CALENDAR_LIVE_V44_REMOTE_REQUESTS,
      hostPhases: CALENDAR_LIVE_V44_HOST_PHASES,
      captures: CALENDAR_LIVE_V44_CAPTURE_COUNT,
      sourceRoots: CALENDAR_LIVE_V44_SOURCE_ROOTS,
      expectedFacts,
    },
    execution: {
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "calendar-live-v44-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/calendar-live-v44-security-attestation.json",
  });
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Calendar live v44 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(CALENDAR_LIVE_V44_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(CALENDAR_LIVE_V44_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${CALENDAR_LIVE_V44_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(CALENDAR_LIVE_V44_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(CALENDAR_LIVE_V44_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Calendar live v44 authorization template drift");
  }
  if (
    writer.namespace !== CALENDAR_FIGMA_NAMESPACE ||
    writer.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    ...
    !readFileSync("recipe/calendar-live-v44-contract.ts", "utf8").includes(
    ...
    throw new Error("Calendar live v44 identity or cleanup invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildCalendarLiveV44Proof(), null, 2)}\n`,
  );
