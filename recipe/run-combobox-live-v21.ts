...
import {
  acceptComboboxLiveV21Response,
  createComboboxLiveV21Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV21Request,
  persistComboboxLiveV21RawResponse,
  persistComboboxLiveV21TechnicalGates,
  readComboboxLiveV21AcceptedRawPayload,
  type ComboboxLiveV21Request,
  type ComboboxLiveV21TransactionAuthorization,
} from "./combobox-live-v21-broker.js";
import {
  COMBOBOX_LIVE_V21_CAPTURE_COUNT,
  assertComboboxLiveV21CaptureResponses,
  ...
  evaluateComboboxLiveV21Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV21Roots,
  validateComboboxLiveV21CaptureManifest,
  validateComboboxLiveV21CapturePayload,
  validateComboboxLiveV21CleanupPayload,
  validateComboboxLiveV21ExtractPayload,
  validateComboboxLiveV21ProbePayload,
  validateComboboxLiveV21WriterPayload,
  type ComboboxLiveV21CaptureCell,
  type ComboboxLiveV21CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V21_PLAN_PATH,
  type ComboboxLiveV21ProofPlan,
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v21.js";

export const COMBOBOX_LIVE_V21_RUNNER_VERSION = "combobox-live-v21-runner-v1";

interface ComboboxLiveV21JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV21OrchestratorState {
  artifactVersion: "combobox-live-v21-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV21JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV21ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV21CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV21SourceIdentity & {
...
export interface ComboboxLiveV21NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV21OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v21 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V21_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V21_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV21ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV21CaptureCell[];
  validateComboboxLiveV21CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v21 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v21 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV21WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV21RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV21Request =>
  readJson<ComboboxLiveV21Request>(file);

export class ComboboxLiveV21Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV21Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV21OrchestratorState = {
      artifactVersion: "combobox-live-v21-orchestrator-state-v1",
    ...
    return new ComboboxLiveV21Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV21OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV21Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV21NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV21NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v21 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV21NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v21 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV21RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV21Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV21CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV21AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV21WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV21Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV21ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV21Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV21Response(this.#directory, "writer", {
        validate: validateComboboxLiveV21WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV21Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV21RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV21Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV21ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV21Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV21RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v21-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV21Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV21ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV21Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV21ProbePayload(payload, ownership),
      });
      persistComboboxLiveV21TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v21 capture cell missing");
      const accepted = acceptComboboxLiveV21Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV21CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V21_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV21CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV21Objective(
    ...
    throw new Error(`Combobox live v21 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV21Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV21CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV21Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV21RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV21Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV21CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV21CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV21CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV21Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV21TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV21SimulationKey = (): KeyObject =>
