...
import { COMBOBOX_LIVE_V31_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v31.js";
import { verifyComboboxLiveV31Authorization } from "./combobox-live-v31-authorization.js";
import { runComboboxLiveV31Preflight } from "./combobox-live-v31-preflight.js";
import { ComboboxLiveV31Orchestrator } from "./run-combobox-live-v31.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v31 requires exact authorization artifact ${COMBOBOX_LIVE_V31_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v31-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV31Authorization(root);
    runComboboxLiveV31Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV31Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV31Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
