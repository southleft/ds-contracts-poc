...
import { INPUT_LIVE_V12_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v12.js";
import { verifyInputLiveV12Authorization } from "./input-field-live-v12-authorization.js";
import { runInputLiveV12Preflight } from "./input-field-live-v12-preflight.js";
import { InputLiveV12Orchestrator } from "./run-input-field-live-v12.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V12_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v12-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV12Authorization(root);
    runInputLiveV12Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV12Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
