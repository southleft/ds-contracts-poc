...
} from "./input-field-live-v3-verifier-v42.js";
import {
  buildInputLiveV42RawPropertyRuntime,
  normalizeInputLiveV42Scene,
  type InputLiveV42RawNode,
} from "./input-field-live-v42-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v42.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v42.js";
...
export const INPUT_LIVE_V42_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V42_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V42_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V42_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V42_VARIANT_COUNT = 256;
export const INPUT_LIVE_V42_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V42_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV42SourceId = (typeof INPUT_LIVE_V42_SOURCE_IDS)[number];

export interface InputLiveV42SourceIdentity {
  source: InputLiveV42SourceId;
...
export interface InputLiveV42WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV42ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV42SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV42RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV42RootProof {
  source: InputLiveV42SourceId;
  adapterIdentity: string;
...
export interface InputLiveV42ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV42SourceId;
    adapterIdentity: string;
...
export interface InputLiveV42CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV42SourceId;
  adapterIdentity: string;
...
export interface InputLiveV42CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV42SourceId;
  strata: InputLiveV42CaptureCell["strata"];
...
export interface InputLiveV42CleanupPayload {
...
export function validateInputLiveV42WriterPayload(
  value: unknown,
): InputLiveV42WriterOwnership {
...
const identityPlan = (source: InputLiveV42SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV42ExtractProgram(
  writer: InputLiveV42WriterOwnership,
  sources: readonly InputLiveV42SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V42_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V42_NAMESPACE)}
${buildInputLiveV42RawPropertyRuntime()}
...
export function validateInputLiveV42ExtractPayload(
  value: unknown,
  writer: InputLiveV42WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V42_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV42ExtractPayload;
}

export function proveInputLiveV42Roots<Instance>(
  extract: InputLiveV42ExtractPayload,
  sources: readonly (InputLiveV42SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV42Scene(
      root.scene,
...
export function assertInputLiveV42RootProofs(
  proofs: readonly InputLiveV42RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV42ProbeProgram(
  writer: InputLiveV42WriterOwnership,
  sources: readonly InputLiveV42SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V42_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV42ProbePayload(
  value: unknown,
  writer: InputLiveV42WriterOwnership,
): InputLiveV42ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV42ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V42_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V42_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV42CaptureManifest(
  cells: readonly InputLiveV42CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V42_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V42_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V42_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V42_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V42_SOURCE_IDS.some(
...
export const inputLiveV42CaptureManifestSha256 = (
  cells: readonly InputLiveV42CaptureCell[],
): string => {
  validateInputLiveV42CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV42CaptureProgram(
  writer: InputLiveV42WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V42_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V42_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V42_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV42CapturePayload(
  value: unknown,
  cell: InputLiveV42CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV42CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V42_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V42_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV42CaptureResponses(
  manifest: readonly InputLiveV42CaptureCell[],
  responses: readonly InputLiveV42CapturePayload[],
): void {
  validateInputLiveV42CaptureManifest(manifest);
  if (
...
export interface InputLiveV42ObjectiveReport {
  artifactVersion: "input-live-v42-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV42Objective(
  manifest: readonly InputLiveV42CaptureCell[],
  responses: readonly InputLiveV42CapturePayload[],
  readReference: (cell: InputLiveV42CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV42CaptureCell) => Buffer,
): InputLiveV42ObjectiveReport {
  assertInputLiveV42CaptureResponses(manifest, responses);
...
export function buildInputLiveV42CleanupProgram(
  writer: InputLiveV42WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V42_NAMESPACE,
...
export function validateInputLiveV42CleanupPayload(
  value: unknown,
  writer: InputLiveV42WriterOwnership,
): InputLiveV42CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV42CleanupPayload;
...
export const INPUT_LIVE_V42_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV42WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
