...
import {
  ...
  verifyInputLiveV78Authorization,
  type InputLiveV78AntecedentIndex,
  type InputLiveV78AuthorizationProof,
} from "./input-field-live-v78-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V78_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v78/operator-security-attestation-template.json";
export const INPUT_LIVE_V78_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v78-security-attestation.json";

export interface InputLiveV78SecurityAttestation {
...
export interface InputLiveV78PreflightReport {
  artifactVersion: "input-live-v78-preflight-v1";
...
export const inputLiveV78AttestationSha256 = (
  value: Omit<InputLiveV78SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV78SecurityAttestation(
  value: unknown,
  proof: InputLiveV78AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v78-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V78_TARGET) ||
...
export function validateInputLiveV78ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV78Authorization");
  const preflight = source.indexOf("runInputLiveV78Preflight");
  const initialize = source.indexOf("InputLiveV78Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV78Preflight(
  root: string,
  proof: InputLiveV78AuthorizationProof,
  attestationPath = INPUT_LIVE_V78_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV78PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V78_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V78_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V78_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV78AntecedentIndex;
  failures.push(...validateInputLiveV78AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V78_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V78_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v78-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV78ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV78SecurityAttestation;
  return {
    artifactVersion: "input-live-v78-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V78_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV78Preflight(
    process.cwd(),
    verifyInputLiveV78Authorization(),
    attestationPath,
