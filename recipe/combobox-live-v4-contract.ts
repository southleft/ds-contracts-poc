...
import { buildComboboxLiveV4CleanupRuntime } from "./combobox-live-v4-cleanup.js";
import {
  type ComboboxLiveV4FixedFixedPoint,
} from "./combobox-live-v4-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV4RestoreProgram,
  validateComboboxLiveV4RestorePayload,
  type ComboboxLiveV4RestorePayload,
} from "./combobox-live-v4-restore.js";
import {
  buildComboboxLiveV4RawPropertyRuntime,
  normalizeComboboxLiveV4Scene,
  type ComboboxLiveV4RawNode,
} from "./combobox-live-v4-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v4.js";
import {
...
export const COMBOBOX_LIVE_V4_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V4_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V4_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V4_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V4_SET_COUNT = 4;
export const COMBOBOX_LIVE_V4_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V4_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V4_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V4_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V4_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV4SourceId = (typeof COMBOBOX_LIVE_V4_SOURCE_IDS)[number];

export interface ComboboxLiveV4SourceIdentity {
  source: ComboboxLiveV4SourceId;
...
export interface ComboboxLiveV4WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV4ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV4SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV4RawNode;
    optionScene: ComboboxLiveV4RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV4RootProof {
  source: ComboboxLiveV4SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV4FixedFixedPoint;
}

export interface ComboboxLiveV4ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV4SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV4CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV4SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV4CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV4SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV4CleanupPayload {
...
export function validateComboboxLiveV4WriterPayload(
  value: unknown,
): ComboboxLiveV4WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V4_NAMESPACE ||
...
const identityPlan = (
  source: ComboboxLiveV4SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV4ExtractProgram(
  writer: ComboboxLiveV4WriterOwnership,
  sources: readonly ComboboxLiveV4SourceIdentity[],
): string {
  if (sources.length !== 2)
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V4_NAMESPACE)}
${buildComboboxLiveV4RawPropertyRuntime()}
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("COMBOBOX-V4-EXTRACT-PAGE");
...
if(get(page,"pageOwner")!=="recipe/combobox/"+${JSON.stringify(writer.runIdentity)})throw new Error("COMBOBOX-V4-EXTRACT-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.has(node.id));
if(sets.length!==4)throw new Error("COMBOBOX-V4-EXTRACT-ROOTS:"+sets.length);
...
for(const set of sets){
  ...
  if(!IDENTITIES[adapterIdentity]||(kind!=="combobox"&&kind!=="option"))throw new Error("COMBOBOX-V4-EXTRACT-ADAPTER:"+adapterIdentity+":"+kind);
...
if(roots.length!==2||roots.some(root=>!root.comboboxScene||!root.optionScene))throw new Error("COMBOBOX-V4-EXTRACT-PAIR");
...
export function validateComboboxLiveV4ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV4WriterOwnership,
): ComboboxLiveV4ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V4_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as ComboboxLiveV4ExtractPayload;
}

export function proveComboboxLiveV4Roots<Instance>(
  extract: ComboboxLiveV4ExtractPayload,
  sources: readonly (ComboboxLiveV4SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const comboboxNormalized = normalizeComboboxLiveV4Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV4Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV4FixedFixedPoint => {
      const cycle = () => {
...
export function assertComboboxLiveV4RootProofs(
  proofs: readonly ComboboxLiveV4RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildComboboxLiveV4ProbeProgram(
  writer: ComboboxLiveV4WriterOwnership,
  sources: readonly ComboboxLiveV4SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V4_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE"||page.id==="115:295378")throw new Error("COMBOBOX-V4-PROBE-PAGE");
...
if(sets.length!==4)throw new Error("COMBOBOX-V4-PROBE-ROOTS:"+sets.length);
...
for(const row of byAdapter.values()){
 ...
 if(!source||!combobox||!option||combobox.children.length!==64||option.children.length!==8)throw new Error("COMBOBOX-V4-PROBE-VARIANTS:"+adapterIdentity);
 ...
 for(const component of combobox.children){const target=axes(component),updates={};for(const name of axisNames){const key=propertyKey(instance,name);if(!key)throw new Error("COMBOBOX-V4-PROBE-AXIS:"+name);updates[key]=target[name];}instance.setProperties(updates);const main=await instance.getMainComponentAsync();if(main)visited.add(main.id);}
 ...
 for(const component of option.children){const target=axes(component),updates={};for(const name of ["Size","Option state"]){const key=propertyKey(optionInstance,name);if(!key)throw new Error("COMBOBOX-V4-PROBE-OPTION-AXIS:"+name);updates[key]=target[name];}optionInstance.setProperties(updates);const main=await optionInstance.getMainComponentAsync();if(main)optionVisited.add(main.id);}
...
export function validateComboboxLiveV4ProbePayload(
  value: unknown,
  writer: ComboboxLiveV4WriterOwnership,
): ComboboxLiveV4ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV4ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V4_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V4_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateComboboxLiveV4CaptureManifest(
  cells: readonly ComboboxLiveV4CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V4_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V4_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V4_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V4_SOURCE_IDS.includes(cell.source) ||
...
export const comboboxLiveV4CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV4CaptureCell[],
): string => {
  validateComboboxLiveV4CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV4CaptureProgram(
  writer: ComboboxLiveV4WriterOwnership,
  cell: ComboboxLiveV4CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V4_CAPTURE_COUNT
  )
...
if(!page||page.type!=="PAGE"||page.id==="115:295378"||!set||set.type!=="COMPONENT_SET")throw new Error("COMBOBOX-V4-CAPTURE-TARGET");
const axes=node=>Object.fromEntries(node.name.split(",").map(part=>part.trim()).filter(part=>part.includes("=")).map(part=>{const at=part.indexOf("=");return[part.slice(0,at),part.slice(at+1)];}));
...
if(!component)throw new Error("COMBOBOX-V4-CAPTURE-CELL:"+cell.cellKey);
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V4_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V4-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV4CapturePayload(
  value: unknown,
  cell: ComboboxLiveV4CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV4CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV4CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V4_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V4_CAPTURE_MAX_PNG_BYTES ||
...
export function assertComboboxLiveV4CaptureResponses(
  manifest: readonly ComboboxLiveV4CaptureCell[],
  responses: readonly ComboboxLiveV4CapturePayload[],
): void {
  validateComboboxLiveV4CaptureManifest(manifest);
  if (
...
export interface ComboboxLiveV4ObjectiveReport {
  artifactVersion: "combobox-live-v4-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV4Objective(
  manifest: readonly ComboboxLiveV4CaptureCell[],
  responses: readonly ComboboxLiveV4CapturePayload[],
): ComboboxLiveV4ObjectiveReport {
  assertComboboxLiveV4CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v4-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV4CleanupProgram(
  writer: ComboboxLiveV4WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV4CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V4_NAMESPACE,
  ...
  const guardedResult = `if(JSON.stringify(requestedNodeIds)!==JSON.stringify([expectedPageId])||JSON.stringify([...requestedCollectionIds].sort())!==JSON.stringify([...expectedCollectionIds].sort()))throw new Error("COMBOBOX-V4-CLEANUP-EXACT-OWNERSHIP");${resultStatement}`;
...
export function validateComboboxLiveV4CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV4WriterOwnership,
): ComboboxLiveV4CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV4CleanupPayload;
  if (
    value.complete !== true ||
...
export const COMBOBOX_LIVE_V4_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV4WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
