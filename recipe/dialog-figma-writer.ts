 ...
 *
 * Dialog is paper chrome plus title and body. One named default cell per source.
 * Astryx compiles Dialog. AntD compiles Modal. Full-bleed scrim size is receipted.
 *
...
import { DIALOG_DEFAULT } from "./recipes/dialog.js";
import {
...
export const DIALOG_FIGMA_RUN_SUFFIX = "dialog-v1";
...
export const FORBIDDEN_MENU_NAMESPACE = "ds.contracts.menu.recipe.v1";
export const FORBIDDEN_MENU_PAGE_ID = "183:76259";

export const DIALOG_FIGMA_VARIANTS_PER_SOURCE = DIALOG_DEFAULT.length;

export interface DialogVariablePlan {
  identity: string;
  name: string;
  type: "COLOR" | "FLOAT";
  value: string | number;
...
export interface DialogFigmaWriterInput {
...
export interface DialogFigmaSourcePlan {
  adapterIdentity: string;
  displayName: string;
  recipeHash: string;
  envelopeHash: string;
...
const variableType = (
  ...
  if (binding.type === "COLOR") {
    if (typeof value !== "string")
      throw new TypeError(
        `dialog live writer: ${binding.field} is not a colour fallback`,
      );
    return "COLOR";
  }
  if (binding.type === "FLOAT") {
    if (typeof value !== "number" || !Number.isFinite(value))
      throw new TypeError(
        `dialog live writer: ${binding.field} is not a numeric fallback`,
      );
    return "FLOAT";
  }
  throw new TypeError(
    `dialog live writer: unsupported binding type ${binding.type}`,
  );
};
...
const requireDefault = (root: IRNode): ComponentNode => {
  if (root.kind === "component" && root.role === "dialog/variant/default")
    return root;
  throw new TypeError(
    "dialog live writer: required one named default component, not a Color/Size/Closable set",
  );
};
...
const planSource = (
  input: DialogFigmaWriterInput,
): DialogFigmaSourcePlan => {
  if (
    input.envelope.recipe.id !== "dialog" ||
    input.envelope.recipe.version !== 1
  )
    throw new TypeError("dialog live writer requires dialog@1");
  if (input.envelope.archetype !== "modal / dialog")
    throw new TypeError("dialog live writer requires the modal / dialog archetype");
  ...
  if (chip.variantProperties.Default !== "true")
    throw new TypeError("dialog live writer: default cell must be Default=true");

  const registry = new Map<string, DialogVariablePlan>();
  walk(chip, (node) => {
    if (node.kind === "instance")
      throw new TypeError(
        `dialog live writer: unexpected instance ${node.componentRef}`,
      );
    for (const binding of node.bindings ?? []) {
      ...
      if (previous && canonicalJson(previous.value) !== canonicalJson(value))
        throw new TypeError(
          `dialog live writer: conflicting fallback for ${binding.variable}`,
        );
      registry.set(key, {
  ...
  if (registry.size === 0)
    throw new TypeError("dialog live writer: zero planned variables");
...
export function validateDialogFigmaSourcePlans(
  plans: readonly DialogFigmaSourcePlan[],
): string[] {
  const failures: string[] = [];
  if (plans.length === 0) failures.push("dialog live writer: no sources");
  const identities = new Set(plans.map((plan) => plan.adapterIdentity));
  if (identities.size !== plans.length)
    failures.push("dialog live writer: duplicate adapter identity");
  for (const plan of plans) {
    ...
    if (plan.chip.kind !== "component" || plan.chip.role !== "dialog/variant/default")
      failures.push(
        `${plan.adapterIdentity}: expected one named default dialog component`,
      );
  }
  return failures;
}
...
for(const source of PLAN.sources){
  ...
  const mintComponent=async(ir)=>{
    ...
    for(const [childIndex,child] of hugKids)await render(child,component,"dialog/children/"+childIndex);
    applySizing(component,ir);
    for(const [childIndex,child] of fillKids)await render(child,component,"dialog/children/"+childIndex);
...
export function emitDialogFigmaWriter(
  ...
  const runIdentity =
    ...
    sourcePlans.map((source) => source.recipeHash.slice(0, 8)).join("-") +
      `-${DIALOG_FIGMA_RUN_SUFFIX}`;
  const namespace: string = DIALOG_FIGMA_NAMESPACE;
  ...
  if (
    ...
    namespace === FORBIDDEN_MENU_NAMESPACE ||
    identity === FORBIDDEN_INPUT_RUN_IDENTITY
  )
    throw new TypeError(
      "dialog writer must not reuse Input, Combobox, Table, Checkbox, Radio, Switch, Textarea, Alert, Chip, Badge, Avatar, Link, Tooltip, Tabs, or Menu identity",
    );

  const pageName = `Recipe Pivot / Dialog / ${runIdentity}`;
  const plan = {
    pageName,
    runIdentity,
  ...
  if (
    ...
    throw new TypeError(
      "dialog writer must carry the compile label into the component name",
    );
  if (
    runtime.includes(
      'component.name=ir.role+" :: "+(ir.label||source.sourceName)',
    ...
    throw new TypeError(
      "dialog writer component name must prefer the compile label",
    );
  if (
    runtime.includes(
      "DIALOG-WRITER-HUG-TEXT-INTRINSIC-BEFORE-PARENT-COLLAPSE",
    ) === false
  )
    throw new TypeError(
      "dialog writer must keep hug-text intrinsic size before a 0-width parent can collapse it",
    );
  if (runtime.includes("DIALOG-FONT-PROVENANCE-TAMPER") === false)
    throw new TypeError(
      "dialog writer must refuse font provenance tampering",
    );
  if (
    runtime.includes("DIALOG-MUST-NOT-WRITE-INPUT-PAGE") === false ||
    runtime.includes("DIALOG-MUST-NOT-WRITE-COMBOBOX-PAGE") === false ||
    runtime.includes("DIALOG-MUST-NOT-WRITE-COMBOBOX-V42-PAGE") === false ||
    runtime.includes("DIALOG-MUST-NOT-WRITE-BUTTON-PAGE") === false ||
    runtime.includes("DIALOG-MUST-NOT-WRITE-TABLE-PAGE") === false ||
    runtime.includes("DIALOG-MUST-NOT-WRITE-CALENDAR-PAGE") === false ||
    runtime.includes("DIALOG-MUST-NOT-WRITE-CHECKBOX-PAGE") === false ||
    runtime.includes("DIALOG-MUST-NOT-WRITE-RADIO-PAGE") === false ||
    runtime.includes("DIALOG-MUST-NOT-WRITE-SWITCH-PAGE") === false ||
    runtime.includes("DIALOG-MUST-NOT-WRITE-TEXTAREA-PAGE") === false ||
    runtime.includes("DIALOG-MUST-NOT-WRITE-ALERT-PAGE") === false ||
    runtime.includes("DIALOG-MUST-NOT-WRITE-TOOLTIP-PAGE") === false ||
    runtime.includes("DIALOG-MUST-NOT-WRITE-TABS-PAGE") === false ||
    runtime.includes("DIALOG-MUST-NOT-WRITE-MENU-PAGE") === false
  )
    throw new TypeError(
