...
import {
  assertCalendarLiveV8RootProofs,
  CALENDAR_LIVE_V8_CAPTURE_COUNT,
  CALENDAR_LIVE_V8_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  CALENDAR_LIVE_V8_RESPONSE_CONTRACTS,
  type CalendarLiveV8RootProof,
} from "./calendar-live-v8-contract.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V8_BROKER_VERSION = "calendar-live-v8-broker-v1";
export const CALENDAR_LIVE_V8_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const CALENDAR_LIVE_V8_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const CALENDAR_LIVE_V8_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const CALENDAR_LIVE_V8_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const CALENDAR_LIVE_V8_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const CALENDAR_LIVE_V8_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type CalendarLiveV8RemotePhase =
  (typeof CALENDAR_LIVE_V8_REMOTE_PHASES)[number];
export type CalendarLiveV8AuthorizationMode = "live" | "simulated";

export interface CalendarLiveV8CommitPins {
...
export interface CalendarLiveV8TransactionAuthorization extends CalendarLiveV8CommitPins {
  mode: CalendarLiveV8AuthorizationMode;
...
export interface CalendarLiveV8Manifest {
  artifactVersion: "calendar-live-v8-broker-manifest-v2";
  brokerVersion: typeof CALENDAR_LIVE_V8_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof CALENDAR_LIVE_V8_DYNAMIC_TOOL;
  target: typeof CALENDAR_LIVE_V8_TARGET;
  authorization: CalendarLiveV8TransactionAuthorization;
...
export interface CalendarLiveV8ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface CalendarLiveV8RequestBody {
  artifactVersion: "calendar-live-v8-broker-request-v2";
  brokerVersion: typeof CALENDAR_LIVE_V8_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: CalendarLiveV8RemotePhase;
  sequence: number;
  ...
  commitPins: CalendarLiveV8CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof CALENDAR_LIVE_V8_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: CalendarLiveV8ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface CalendarLiveV8Request extends CalendarLiveV8RequestBody {
...
export interface CalendarLiveV8Receipt {
  artifactVersion: "calendar-live-v8-broker-receipt-v2";
  brokerVersion: typeof CALENDAR_LIVE_V8_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: CalendarLiveV8RemotePhase;
...
export interface CalendarLiveV8AcceptedResponse<T = unknown> {
  request: CalendarLiveV8Request;
  receipt: CalendarLiveV8Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface CalendarLiveV8IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: CalendarLiveV8ResponseContract;
...
const equalTarget = (value: unknown): value is typeof CALENDAR_LIVE_V8_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === CALENDAR_LIVE_V8_TARGET.fileKey &&
  (value as BrokerTarget).fileName === CALENDAR_LIVE_V8_TARGET.fileName &&
  (value as BrokerTarget).editorType === CALENDAR_LIVE_V8_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== CALENDAR_LIVE_V8_TARGET.fileKey ||
    context.fileName !== CALENDAR_LIVE_V8_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === CALENDAR_LIVE_V8_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Calendar live v8 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= CALENDAR_LIVE_V8_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v8 capture index must be 0..7");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Calendar live v8 authorization pins are malformed");
};

const readManifest = (directory: string): CalendarLiveV8Manifest => {
  const manifest = json<CalendarLiveV8Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Calendar live v8 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "calendar-live-v8-broker-manifest-v2" ||
    manifest.brokerVersion !== CALENDAR_LIVE_V8_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== CALENDAR_LIVE_V8_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(CALENDAR_LIVE_V8_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Calendar live v8 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: CalendarLiveV8Request,
): CalendarLiveV8RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: CalendarLiveV8RemotePhase,
  captureIndex?: number,
): CalendarLiveV8Request | undefined => {
  const file = calendarLiveV1RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV8Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: CalendarLiveV8RemotePhase,
  captureIndex?: number,
): CalendarLiveV8Receipt | undefined => {
  const file = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV8Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: CalendarLiveV8Request | undefined;
  receipt: CalendarLiveV8Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: CalendarLiveV8RemotePhase,
): CalendarLiveV8ResponseContract => ({
  ...CALENDAR_LIVE_V8_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: CalendarLiveV8Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: CALENDAR_LIVE_V8_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "calendar-live-v8-plugin-response-v2",
  brokerVersion: CALENDAR_LIVE_V8_BROKER_VERSION,
  target: CALENDAR_LIVE_V8_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*CALENDAR_LIVE_V8_PROGRAM_START*/${program}/*CALENDAR_LIVE_V8_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*CALENDAR_LIVE_V8_PROGRAM_START*/";
  const endMarker = "/*CALENDAR_LIVE_V8_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Calendar live v8 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createCalendarLiveV8Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Calendar live v8 private signing key does not match authorized public-key identity",
    );
  const manifest: CalendarLiveV8Manifest = {
    artifactVersion: "calendar-live-v8-broker-manifest-v2",
    brokerVersion: CALENDAR_LIVE_V8_BROKER_VERSION,
    ...
    captureRequests: CALENDAR_LIVE_V8_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateCalendarLiveV8Request(
  directory: string,
  request: CalendarLiveV8Request,
): void {
  ...
  if (
    request.artifactVersion !== "calendar-live-v8-broker-request-v2" ||
    request.brokerVersion !== CALENDAR_LIVE_V8_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== CALENDAR_LIVE_V8_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Calendar live v8 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueCalendarLiveV8Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: CalendarLiveV8RemotePhase,
  program: string,
  options: CalendarLiveV8IssueOptions = {},
): CalendarLiveV8Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<CalendarLiveV8Request>(targetPath);
    validateCalendarLiveV8Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Calendar live v8 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(calendarLiveV1ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Calendar live v8 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Calendar live v8 empty code envelope refused");
  if (phase === "capture") validateCalendarLiveV8TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Calendar live v8 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: CALENDAR_LIVE_V8_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? CALENDAR_LIVE_V8_SIGNED_WRITER_TIMEOUT_MS
        : CALENDAR_LIVE_V8_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: CalendarLiveV8RequestBody = {
    artifactVersion: "calendar-live-v8-broker-request-v2",
    brokerVersion: CALENDAR_LIVE_V8_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: CalendarLiveV8Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateCalendarLiveV8Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistCalendarLiveV8RawResponse(
  directory: string,
  phase: CalendarLiveV8RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Calendar live v8 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<CalendarLiveV8Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Calendar live v8 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = calendarLiveV1ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Calendar live v8 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): CalendarLiveV8Receipt => {
  const body: Omit<CalendarLiveV8Receipt, "receiptSha256"> = {
    artifactVersion: "calendar-live-v8-broker-receipt-v2",
    brokerVersion: CALENDAR_LIVE_V8_BROKER_VERSION,
...
export function acceptCalendarLiveV8Response<T = unknown>(
  directory: string,
  phase: CalendarLiveV8RemotePhase,
  options: {
    captureIndex?: number;
...
): CalendarLiveV8AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Calendar live v8 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<CalendarLiveV8Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Calendar live v8 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "calendar-live-v8-plugin-response-v2" ||
    plugin?.brokerVersion !== CALENDAR_LIVE_V8_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Calendar live v8 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistCalendarLiveV8TechnicalGates(
  directory: string,
  rootProofs: readonly CalendarLiveV8RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertCalendarLiveV8RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Calendar live v8 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Calendar live v8 probe payload hash malformed");
  const gates = Object.fromEntries(
    CALENDAR_LIVE_V8_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "calendar-live-v8-host-technical-gates-v2",
    brokerVersion: CALENDAR_LIVE_V8_BROKER_VERSION,
...
export function validateCalendarLiveV8TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "calendar-live-v8-host-technical-gates-v2" ||
    ...
    CALENDAR_LIVE_V8_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Calendar live v8 technical gates artifact");
}

export function validateCalendarLiveV8Receipt(
  directory: string,
  phase: CalendarLiveV8RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateCalendarLiveV8Request(directory, request);
  const receipt = json<CalendarLiveV8Receipt>(
  ...
  if (
    receipt.artifactVersion !== "calendar-live-v8-broker-receipt-v2" ||
    receipt.brokerVersion !== CALENDAR_LIVE_V8_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Calendar live v8 accepted receipt: ${request.requestId}`,
    );
}

export function readCalendarLiveV8AcceptedRawPayload(
  directory: string,
  phase: CalendarLiveV8RemotePhase,
  captureIndex?: number,
): unknown {
  validateCalendarLiveV8Receipt(directory, phase, captureIndex);
...
export function createCalendarLiveV8FakeDynamicResponse(
  request: CalendarLiveV8Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? CALENDAR_LIVE_V8_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? CALENDAR_LIVE_V8_TARGET,
    ...overrides.extra,
  };
}

export function listCalendarLiveV8PersistedRequests(
  directory: string,
): CalendarLiveV8Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<CalendarLiveV8Request>(file)] : [];
    });
}
