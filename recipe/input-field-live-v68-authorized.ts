...
import { INPUT_LIVE_V68_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v68.js";
import { verifyInputLiveV68Authorization } from "./input-field-live-v68-authorization.js";
import { runInputLiveV68Preflight } from "./input-field-live-v68-preflight.js";
import { InputLiveV68Orchestrator } from "./run-input-field-live-v68.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V68_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v68-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV68Authorization(root);
    runInputLiveV68Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV68Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
