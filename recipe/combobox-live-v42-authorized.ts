...
import { COMBOBOX_LIVE_V42_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v42.js";
import { verifyComboboxLiveV42Authorization } from "./combobox-live-v42-authorization.js";
import { runComboboxLiveV42Preflight } from "./combobox-live-v42-preflight.js";
import { ComboboxLiveV42Orchestrator } from "./run-combobox-live-v42.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v42 requires exact authorization artifact ${COMBOBOX_LIVE_V42_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v42-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV42Authorization(root);
    runComboboxLiveV42Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV42Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV42Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
