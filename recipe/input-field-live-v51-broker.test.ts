...
import {
  ...
  createInputLiveV51FakeDynamicResponse,
  ...
  listInputLiveV51PersistedRequests,
  persistInputLiveV51RawResponse,
  persistInputLiveV51TechnicalGates,
  validateInputLiveV51Receipt,
  validateInputLiveV51Request,
  type InputLiveV51RemotePhase,
...
import {
  ...
  type InputLiveV51ExtractPayload,
  type InputLiveV51ProbePayload,
  type InputLiveV51RestorePayload,
  type InputLiveV51RootProof,
  type InputLiveV51WriterOwnership,
} from "./input-field-live-v51-contract.js";
import {
  InputLiveV51Orchestrator,
  simulatedInputLiveV51Authorization,
} from "./run-input-field-live-v51.js";
...
const captures: InputLiveV51CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV51Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v51-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV51Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV51CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV51FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV51FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V51_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V51_TARGET);
  persistInputLiveV51RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV51Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV51Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV51WriterOwnership,
): InputLiveV51RestorePayload => ({
...
const rootProofs = (): InputLiveV51RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV51CaptureCell,
): InputLiveV51CapturePayload => {
  const bytes = Buffer.from(`input-live-v51-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV51Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV51Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV51RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV51RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV51ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV51ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV51CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV51CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV51CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV51Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV51CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV51PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV51ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV51Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV51Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV51Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV51Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V51_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV51RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V51_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV51Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV51Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV51ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV51Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV51WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV51RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV51Request;
    validateInputLiveV51Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV51Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV51CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v51-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV51Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV51Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV51Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V51_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV51RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V51_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV51Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV51Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV51RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V51_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV51Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV51Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV51Request;
