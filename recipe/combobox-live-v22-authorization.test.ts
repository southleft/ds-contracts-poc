...
import { COMBOBOX_LIVE_V22_INDEX_PATH } from "./build-combobox-live-proof-v22.js";
import {
  COMBOBOX_LIVE_V22_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V22_TARGET,
} from "./combobox-live-v22-broker.js";
import {
  buildComboboxLiveV22AuthorizationArtifact,
  validateComboboxLiveV22AntecedentIndex,
  validateComboboxLiveV22AuthorizationArtifact,
  validateComboboxLiveV22History,
  validateComboboxLiveV22UnitTestSource,
  type ComboboxLiveV22AntecedentIndex,
  type ComboboxLiveV22AuthorizationArtifact,
  type ComboboxLiveV22AuthorizationProof,
  type ComboboxLiveV22HistoryState,
} from "./combobox-live-v22-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV22ControlFlowSource,
  validateComboboxLiveV22SecurityAttestation,
  type ComboboxLiveV22SecurityAttestation,
} from "./combobox-live-v22-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v22-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V22_INDEX_PATH);
const index = (): ComboboxLiveV22AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV22AntecedentIndex;

const artifact = (): ComboboxLiveV22AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV22AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV22HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV22HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV22AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v22/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV22AuthorizationArtifact,
): ComboboxLiveV22SecurityAttestation => {
  const body: Omit<ComboboxLiveV22SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V22_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV22History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV22History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV22History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV22History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV22History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV22UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV22UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV22AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV22AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV22AuthorizationArtifact(
        planted as ComboboxLiveV22AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV22SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV22SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV22SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV22ControlFlowSource(source), []);
});
