...
import { CALENDAR_LIVE_V3_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v3.js";
import { verifyCalendarLiveV3Authorization } from "./calendar-live-v3-authorization.js";
import { runCalendarLiveV3Preflight } from "./calendar-live-v3-preflight.js";
import { CalendarLiveV3Orchestrator } from "./run-calendar-live-v3.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v3 requires exact authorization artifact ${CALENDAR_LIVE_V3_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v3-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV3Authorization(root);
    runCalendarLiveV3Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV3Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV3Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
