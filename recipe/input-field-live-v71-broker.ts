...
import {
  assertInputLiveV71RootProofs,
  INPUT_LIVE_V71_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V71_RESPONSE_CONTRACTS,
  type InputLiveV71RootProof,
} from "./input-field-live-v71-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V71_BROKER_VERSION = "input-live-v71-broker-v2";
export const INPUT_LIVE_V71_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V71_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V71_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V71_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V71_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V71_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV71RemotePhase =
  (typeof INPUT_LIVE_V71_REMOTE_PHASES)[number];
export type InputLiveV71AuthorizationMode = "live" | "simulated";

export interface InputLiveV71CommitPins {
...
export interface InputLiveV71TransactionAuthorization extends InputLiveV71CommitPins {
  mode: InputLiveV71AuthorizationMode;
...
export interface InputLiveV71Manifest {
  artifactVersion: "input-live-v71-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V71_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V71_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V71_TARGET;
  authorization: InputLiveV71TransactionAuthorization;
...
export interface InputLiveV71ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV71RequestBody {
  artifactVersion: "input-live-v71-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V71_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV71RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV71CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V71_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV71ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV71Request extends InputLiveV71RequestBody {
...
export interface InputLiveV71Receipt {
  artifactVersion: "input-live-v71-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V71_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV71RemotePhase;
...
export interface InputLiveV71AcceptedResponse<T = unknown> {
  request: InputLiveV71Request;
  receipt: InputLiveV71Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV71IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV71ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V71_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V71_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V71_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V71_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V71_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V71_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V71_TARGET.editorType;
};
...
export const inputLiveV71RequestSequence = (
  phase: InputLiveV71RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV71RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV71RequestPath = (
  directory: string,
  phase: InputLiveV71RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV71ResponsePath = (
  directory: string,
  phase: InputLiveV71RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV71ReceiptPath = (
  directory: string,
  phase: InputLiveV71RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV71Manifest => {
  const manifest = json<InputLiveV71Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v71-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V71_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V71_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV71RemotePhase,
  captureIndex?: number,
): InputLiveV71Request | undefined => {
  const file = inputLiveV71RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV71Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV71RemotePhase,
  captureIndex?: number,
): InputLiveV71Receipt | undefined => {
  const file = inputLiveV71ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV71Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV71Request | undefined;
  receipt: InputLiveV71Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV71RemotePhase,
): InputLiveV71ResponseContract => ({
  ...INPUT_LIVE_V71_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V71_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V71_DYNAMIC_TOOL,
      target: INPUT_LIVE_V71_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v71-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V71_BROKER_VERSION,
  target: INPUT_LIVE_V71_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V71_PROGRAM_START*/${program}/*INPUT_LIVE_V71_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V71_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V71_PROGRAM_END*/";
...
export function createInputLiveV71Transaction(
  directory: string,
  ...
  const manifest: InputLiveV71Manifest = {
    artifactVersion: "input-live-v71-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V71_BROKER_VERSION,
...
export function validateInputLiveV71Request(
  directory: string,
  request: InputLiveV71Request,
): void {
  ...
  const expectedSequence = inputLiveV71RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v71-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V71_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V71_TARGET.fileKey ||
...
export function issueInputLiveV71Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV71RemotePhase,
  program: string,
  options: InputLiveV71IssueOptions = {},
): InputLiveV71Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV71RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV71Request>(targetPath);
    validateInputLiveV71Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV71ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV71TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV71RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V71_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V71_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V71_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV71RequestBody = {
    artifactVersion: "input-live-v71-broker-request-v2",
    brokerVersion: INPUT_LIVE_V71_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV71Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV71Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV71RawResponse(
  directory: string,
  phase: InputLiveV71RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV71RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV71Request>(requestFile);
  if (
  ...
  const target = inputLiveV71ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV71Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v71-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V71_BROKER_VERSION,
...
export function acceptInputLiveV71Response<T = unknown>(
  directory: string,
  phase: InputLiveV71RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV71AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV71ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV71Request>(
    inputLiveV71RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV71Request(directory, request);
  if (phase === "capture") validateInputLiveV71TechnicalGates(directory);
  const rawFile = inputLiveV71ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v71-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V71_BROKER_VERSION ||
...
export function persistInputLiveV71TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV71RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV71RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V71_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v71-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V71_BROKER_VERSION,
...
export function validateInputLiveV71TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v71-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V71_TECHNICAL_GATES.some(
...
export function validateInputLiveV71Receipt(
  directory: string,
  phase: InputLiveV71RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV71Request(directory, request);
  const receipt = json<InputLiveV71Receipt>(
    inputLiveV71ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV71ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v71-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V71_BROKER_VERSION ||
...
export function readInputLiveV71AcceptedRawPayload(
  directory: string,
  phase: InputLiveV71RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV71Receipt(directory, phase, captureIndex);
...
export function createInputLiveV71FakeDynamicResponse(
  request: InputLiveV71Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V71_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V71_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV71PersistedRequests(
  directory: string,
): InputLiveV71Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV71Request>(file)] : [];
    });
}

export const inputLiveV71Sha256 = sha256;
