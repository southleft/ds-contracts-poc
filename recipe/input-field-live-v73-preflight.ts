...
import {
  ...
  verifyInputLiveV73Authorization,
  type InputLiveV73AntecedentIndex,
  type InputLiveV73AuthorizationProof,
} from "./input-field-live-v73-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V73_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v73/operator-security-attestation-template.json";
export const INPUT_LIVE_V73_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v73-security-attestation.json";

export interface InputLiveV73SecurityAttestation {
...
export interface InputLiveV73PreflightReport {
  artifactVersion: "input-live-v73-preflight-v1";
...
export const inputLiveV73AttestationSha256 = (
  value: Omit<InputLiveV73SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV73SecurityAttestation(
  value: unknown,
  proof: InputLiveV73AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v73-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V73_TARGET) ||
...
export function validateInputLiveV73ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV73Authorization");
  const preflight = source.indexOf("runInputLiveV73Preflight");
  const initialize = source.indexOf("InputLiveV73Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV73Preflight(
  root: string,
  proof: InputLiveV73AuthorizationProof,
  attestationPath = INPUT_LIVE_V73_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV73PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V73_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V73_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V73_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV73AntecedentIndex;
  failures.push(...validateInputLiveV73AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V73_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V73_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v73-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV73ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV73SecurityAttestation;
  return {
    artifactVersion: "input-live-v73-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V73_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV73Preflight(
    process.cwd(),
    verifyInputLiveV73Authorization(),
    attestationPath,
