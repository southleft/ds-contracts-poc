...
import { INPUT_LIVE_V82_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v82.js";
import { verifyInputLiveV82Authorization } from "./input-field-live-v82-authorization.js";
import { runInputLiveV82Preflight } from "./input-field-live-v82-preflight.js";
import { InputLiveV82Orchestrator } from "./run-input-field-live-v82.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V82_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v82-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV82Authorization(root);
    runInputLiveV82Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV82Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
