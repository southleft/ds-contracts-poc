...
import { calendarLiveV1Sha256 } from "./calendar-live-v45-broker.js";

export interface CalendarLiveV45LifecycleSimulationReport {
  artifactVersion: "calendar-live-v45-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV45AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV45Lifecycle(): Promise<CalendarLiveV45LifecycleSimulationReport> {
  await buildCalendarLiveV45Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V45_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV45AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v45-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V45_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V45_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV45History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV45History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV45AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V45_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V45_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV45History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV45History(authorized, "pending");
    ...
    await buildCalendarLiveV45Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V45_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV45AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV45Lifecycle(), null, 2)}\n`,
  );
