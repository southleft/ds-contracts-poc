...
import {
  ...
  createInputLiveV49FakeDynamicResponse,
  ...
  listInputLiveV49PersistedRequests,
  persistInputLiveV49RawResponse,
  persistInputLiveV49TechnicalGates,
  validateInputLiveV49Receipt,
  validateInputLiveV49Request,
  type InputLiveV49RemotePhase,
...
import {
  ...
  type InputLiveV49ExtractPayload,
  type InputLiveV49ProbePayload,
  type InputLiveV49RestorePayload,
  type InputLiveV49RootProof,
  type InputLiveV49WriterOwnership,
} from "./input-field-live-v49-contract.js";
import {
  InputLiveV49Orchestrator,
  simulatedInputLiveV49Authorization,
} from "./run-input-field-live-v49.js";
...
const captures: InputLiveV49CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV49Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v49-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV49Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV49CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV49FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV49FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V49_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V49_TARGET);
  persistInputLiveV49RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV49Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV49Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV49WriterOwnership,
): InputLiveV49RestorePayload => ({
...
const rootProofs = (): InputLiveV49RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV49CaptureCell,
): InputLiveV49CapturePayload => {
  const bytes = Buffer.from(`input-live-v49-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV49Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV49Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV49RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV49RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV49ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV49ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV49CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV49CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV49CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV49Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV49CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV49PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV49ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV49Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV49Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV49Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV49Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V49_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV49RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V49_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV49Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV49Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV49ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV49Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV49WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV49RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV49Request;
    validateInputLiveV49Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV49Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV49CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v49-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV49Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV49Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV49Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V49_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV49RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V49_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV49Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV49Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV49RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V49_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV49Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV49Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV49Request;
