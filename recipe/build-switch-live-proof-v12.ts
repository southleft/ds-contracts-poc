...
import { muiSwitchAdapterConfig as muiSwitchProposedConfig, muiSwitchSource as muiSwitchProposedSource } from "./fixtures/generated/switch.mui.js";
import { shadcnSwitchAdapterConfig, shadcnSwitchSource } from "./fixtures/generated/switch.shadcn.js";
import { chakraSwitchAdapterConfig, chakraSwitchSource } from "./fixtures/generated/switch.chakra.js";
import { compileSwitchRecipe, switchRecipe } from "./recipes/switch.js";

const EVIDENCE = "recipe/evidence/switch-live-pivot-v12";
...
const sources = [
  ...
  {
    // PROPOSED by recipe/fixture-reader/propose-switch.ts from MUI's own
    // capture — the bare mount (no label part): the label-less cell.
    adapterIdentity: "mui-switch-proposed-v1",
    displayName: "MUI (proposed)",
    source: muiSwitchProposedSource,
    config: muiSwitchProposedConfig,
  },
  {
    ...
    adapterIdentity: "shadcn-switch-proposed-v1",
    displayName: "shadcn",
    source: shadcnSwitchSource,
    config: shadcnSwitchAdapterConfig,
  },
  {
    // PROPOSED from a capture made TODAY: Chakra's Switch had never been
    // captured; a person authored the config entry and seed contract (the
    // one input the reader cannot draft from a capture, because it makes
    // the capture), and the command read 33 leaves with no --set.
    adapterIdentity: "chakra-switch-proposed-v1",
    displayName: "Chakra",
    source: chakraSwitchSource,
    config: chakraSwitchAdapterConfig,
  },
].map((entry) => {
...
const receiptOwned = {
      artifactVersion: "switch-live-pivot-v12-prepare",
      teaching:
...
if (process.argv.includes("--check")) {
  ...
  if (writer.sourcePlans.length !== 6)
    throw new Error("switch prepare requires six library sources (three reviewed + the proposed MUI, shadcn and Chakra)");
  console.log(
    JSON.stringify({
      check: "ok",
      runIdentity: writer.runIdentity,
