...
import { buildComboboxLiveV38CleanupRuntime } from "./combobox-live-v38-cleanup.js";
import {
...
import {
  ...
  type ComboboxLiveV38RestorePayload,
} from "./combobox-live-v38-restore.js";
import {
  buildComboboxLiveV38RawPropertyRuntime,
  normalizeComboboxLiveV38Scene,
  type ComboboxLiveV38RawNode,
} from "./combobox-live-v38-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v38.js";
import {
...
export const COMBOBOX_LIVE_V38_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V38_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V38_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V38_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V38_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V38_SET_COUNT = 4;
export const COMBOBOX_LIVE_V38_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V38_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V38_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V38_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V38_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV38SourceId = (typeof COMBOBOX_LIVE_V38_SOURCE_IDS)[number];

export interface ComboboxLiveV38SourceIdentity {
  source: ComboboxLiveV38SourceId;
...
export interface ComboboxLiveV38WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV38ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV38SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV38RawNode;
    optionScene: ComboboxLiveV38RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV38RootProof {
  source: ComboboxLiveV38SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV38ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV38SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV38CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV38SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV38CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV38SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV38CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v38 ${label} must be an object`);
...
export function validateComboboxLiveV38WriterPayload(
  value: unknown,
): ComboboxLiveV38WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V38_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v38 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v38 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v38 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV38SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV38ExtractProgram(
  writer: ComboboxLiveV38WriterOwnership,
  sources: readonly ComboboxLiveV38SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v38 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V38_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V38_NAMESPACE)}
${buildComboboxLiveV38RawPropertyRuntime()}
...
export function validateComboboxLiveV38ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV38WriterOwnership,
): ComboboxLiveV38ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v38 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V38_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v38 extract schema/two-root mismatch");
  return payload as ComboboxLiveV38ExtractPayload;
}

export function proveComboboxLiveV38Roots<Instance>(
  extract: ComboboxLiveV38ExtractPayload,
  sources: readonly (ComboboxLiveV38SourceIdentity & {
...
): ComboboxLiveV38RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v38 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v38 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV38Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV38Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV38FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v38 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v38 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v38 compile lost owned sets");
...
export function assertComboboxLiveV38RootProofs(
  proofs: readonly ComboboxLiveV38RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v38 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v38 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV38ProbeProgram(
  writer: ComboboxLiveV38WriterOwnership,
  sources: readonly ComboboxLiveV38SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V38_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV38ProbePayload(
  value: unknown,
  writer: ComboboxLiveV38WriterOwnership,
): ComboboxLiveV38ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV38ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V38_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V38_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v38 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV38CaptureManifest(
  cells: readonly ComboboxLiveV38CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V38_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V38_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V38_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V38_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v38 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV38CaptureCell[],
): string => {
  validateComboboxLiveV38CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV38CaptureProgram(
  writer: ComboboxLiveV38WriterOwnership,
  cell: ComboboxLiveV38CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V38_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v38 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v38 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V38_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV38CapturePayload(
  value: unknown,
  cell: ComboboxLiveV38CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV38CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV38CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V38_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V38_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v38 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV38CaptureResponses(
  manifest: readonly ComboboxLiveV38CaptureCell[],
  responses: readonly ComboboxLiveV38CapturePayload[],
): void {
  validateComboboxLiveV38CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v38 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV38ObjectiveReport {
  artifactVersion: "combobox-live-v38-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV38Objective(
  manifest: readonly ComboboxLiveV38CaptureCell[],
  responses: readonly ComboboxLiveV38CapturePayload[],
): ComboboxLiveV38ObjectiveReport {
  assertComboboxLiveV38CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v38-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV38CleanupProgram(
  writer: ComboboxLiveV38WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV38CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V38_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v38 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV38CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV38WriterOwnership,
): ComboboxLiveV38CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV38CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v38 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V38_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV38WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
