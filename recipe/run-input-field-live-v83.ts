...
import {
  ...
  readInputLiveV83AcceptedRawPayload,
...
import {
  ...
  buildInputLiveV83CleanupProgram,
...
import {
  ...
  INPUT_LIVE_V83_PLAN_PATH,
  type InputLiveV83ProofPlan,
} from "./build-input-field-live-proof-v83.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-v83.js";

export const INPUT_LIVE_V83_RUNNER_VERSION = "input-live-v83-runner-v1";

interface InputLiveV83JournalEntry {
  index: number;
  event: string;
...
interface InputLiveV83OrchestratorState {
  artifactVersion: "input-live-v83-orchestrator-state-v1";
  ...
  journal: InputLiveV83JournalEntry[];
}

interface LoadedProofInputs {
  plan: InputLiveV83ProofPlan;
  planSha256: string;
  captures: InputLiveV83CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    InputLiveV83SourceIdentity & {
...
export interface InputLiveV83NextAction {
...
const writeState = (
  directory: string,
  state: InputLiveV83OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, INPUT_LIVE_V83_PLAN_PATH);
  const capturePath = path.join(root, INPUT_LIVE_V83_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as InputLiveV83ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as InputLiveV83CaptureCell[];
  validateInputLiveV83CaptureManifest(captures);
  if (
    plan.captureManifest.sha256 !== inputLiveV83Sha256(captureBytes) ||
    captureArtifact.cellsSha256 !== inputLiveV83CaptureManifestSha256(captures)
  )
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (
      inputLiveV83Sha256(expectedBytes) !== source.expectedScenePlan.sha256 ||
      inputLiveV83Sha256(uncompressed) !==
  ...
  return {
    plan,
    planSha256: inputLiveV83Sha256(planBytes),
    captures,
    captureManifestSha256: inputLiveV83Sha256(captureBytes),
    sources,
...
const appendJournal = (
  ...
  const payloadSha256 = inputLiveV83Sha256(canonicalJson(payload));
  const body = {
  ...
  const entry = {
    ...body,
    entrySha256: inputLiveV83Sha256(canonicalJson(body)),
  };
  writeNew(
    path.join(
      directory,
...
const writerOwnership = (directory: string): InputLiveV83WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): InputLiveV83RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): InputLiveV83Request =>
  readJson<InputLiveV83Request>(file);

export class InputLiveV83Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueInputLiveV83Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: InputLiveV83OrchestratorState = {
      artifactVersion: "input-live-v83-orchestrator-state-v1",
      transactionDirectory: options.transactionDirectory,
      activeRequest: inputLiveV83RequestPath(
        options.transactionDirectory,
        "writer",
      ),
      cleanupRequest: null,
    ...
    return new InputLiveV83Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<InputLiveV83OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new InputLiveV83Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): InputLiveV83NextAction {
    const cleanup =
      this.#state.cleanupRequest &&
      !this.#state.cleanupAccepted &&
      !existsSync(inputLiveV83ReceiptPath(this.#directory, "cleanup"))
        ? this.#state.cleanupRequest
        : null;
  ...
  ingestAndAdvance(rawResponse: string): InputLiveV83NextAction {
  ...
  ingestCleanup(rawResponse: string): InputLiveV83NextAction {
    ...
    persistInputLiveV83RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptInputLiveV83Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV83CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      existsSync(inputLiveV83ReceiptPath(this.#directory, "writer")) &&
      !existsSync(path.join(this.#directory, OWNERSHIP_FILE))
    ) {
      const raw = readInputLiveV83AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateInputLiveV83WriterPayload(raw),
      );
    }
    if (existsSync(inputLiveV83RequestPath(this.#directory, "cleanup")))
      this.#state.cleanupRequest = inputLiveV83RequestPath(
        this.#directory,
        "cleanup",
      );
    if (
      existsSync(path.join(this.#directory, OWNERSHIP_FILE)) &&
      !existsSync(inputLiveV83RequestPath(this.#directory, "cleanup"))
    )
      this.#issueCleanupAndRestore(false);
    else if (
      existsSync(inputLiveV83RequestPath(this.#directory, "cleanup")) &&
      !existsSync(inputLiveV83RequestPath(this.#directory, "restore"))
    )
      this.#issueRestore(false);
    if (
      existsSync(inputLiveV83RequestPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV83ReceiptPath(this.#directory, "restore"))
    )
      this.#state.activeRequest = inputLiveV83RequestPath(
        this.#directory,
        "restore",
      );
    if (
      existsSync(inputLiveV83ReceiptPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV83RequestPath(this.#directory, "extract"))
    ) {
      const ownership = writerOwnership(this.#directory);
      const extract = issueInputLiveV83Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildInputLiveV83ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV83RequestPath(
        this.#directory,
        "extract",
    ...
    if (existsSync(inputLiveV83ReceiptPath(this.#directory, "cleanup")))
      this.#state.cleanupAccepted = true;
    this.#persist();
  }

  #acceptActive(request: InputLiveV83Request): void {
    if (request.phase === "writer") {
      const accepted = acceptInputLiveV83Response(this.#directory, "writer", {
    ...
    if (request.phase === "restore") {
      const accepted = acceptInputLiveV83Response(this.#directory, "restore", {
        validate: (payload) =>
          validateInputLiveV83RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptInputLiveV83Response(this.#directory, "extract", {
        validate: (payload) =>
          validateInputLiveV83ExtractPayload(payload, ownership),
      });
      const proofs = proveInputLiveV83Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseInputFieldRecipe,
        compileInputFieldRecipe,
      ...
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "input-live-v83-host-root-proofs-v1",
      ...
      appendJournal(
        ...
        request.requestId,
        { proofsSha256: inputLiveV83Sha256(canonicalJson(proofs)) },
      );
      const next = issueInputLiveV83Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildInputLiveV83ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV83RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptInputLiveV83Response(this.#directory, "probe", {
        validate: (payload) =>
          validateInputLiveV83ProbePayload(payload, ownership),
      });
      persistInputLiveV83TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      const accepted = acceptInputLiveV83Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateInputLiveV83CapturePayload(payload, cell, rawBytes),
      });
      const png = Buffer.from(accepted.payload.pngBase64, "base64");
      const capturePath = path.join(
        this.#directory,
        "captures",
        `${String(cell.index).padStart(3, "0")}-${inputLiveV83Sha256(cell.cellKey).slice(0, 20)}.png`,
      );
      ...
      if (next < INPUT_LIVE_V83_CAPTURE_COUNT) this.#issueCapture(next);
      else {
          ...
          pngBase64: Buffer.from(
            readFileSync(
              path.join(
                this.#directory,
                "captures",
                `${String(response.index).padStart(3, "0")}-${inputLiveV83Sha256(response.cellKey).slice(0, 20)}.png`,
              ),
            ),
          ).toString("base64"),
        }));
        assertInputLiveV83CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateInputLiveV83Objective(
        ...
        appendJournal(
          ...
          request.requestId,
          {
            reportSha256: inputLiveV83Sha256(canonicalJson(objective)),
  ...
  #issueCleanupAndRestore(journal: boolean): void {
    ...
    const cleanup = issueInputLiveV83Request(
      ...
      this.#privateKey,
      "cleanup",
      buildInputLiveV83CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = inputLiveV83RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueInputLiveV83Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildInputLiveV83RestoreProgram(ownership),
    );
    this.#state.activeRequest = inputLiveV83RequestPath(
      this.#directory,
      "restore",
  ...
  #readCaptureResponseIndex(): InputLiveV83CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: InputLiveV83CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedInputLiveV83Authorization = (
  privateKey: KeyObject,
...
export const generateInputLiveV83SimulationKey = (): KeyObject =>
