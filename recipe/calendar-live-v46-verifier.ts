...
import {
  verifyCalendarLiveV46FixedSceneFixedPoint,
  type CalendarLiveV46FixedFixedPoint,
} from "./calendar-live-v46-fixed-point.js";

export const CALENDAR_LIVE_V46_VERIFIER_VERSION = "calendar-live-v46-verifier-v1";

export function buildCalendarLiveV46RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV46RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV46RawNode[];
}

export interface CalendarLiveV46NormalizedScene {
...
export interface CalendarLiveV46SceneProof {
...
export function normalizeCalendarLiveV46Scene(
  root: CalendarLiveV46RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV46NormalizedScene {
  const canonicalBindings: CalendarLiveV46NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV46RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV46Scene<Instance>(
  normalized: CalendarLiveV46NormalizedScene,
...
): CalendarLiveV46SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV46FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV46ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV46PreCaptureGates(payload: unknown): void {
