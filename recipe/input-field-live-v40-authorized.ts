...
import { INPUT_LIVE_V40_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v40.js";
import { verifyInputLiveV40Authorization } from "./input-field-live-v40-authorization.js";
import { runInputLiveV40Preflight } from "./input-field-live-v40-preflight.js";
import { InputLiveV40Orchestrator } from "./run-input-field-live-v40.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V40_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v40-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV40Authorization(root);
    runInputLiveV40Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV40Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
