...
import {
  verifyCalendarLiveV41FixedSceneFixedPoint,
  type CalendarLiveV41FixedFixedPoint,
} from "./calendar-live-v41-fixed-point.js";

export const CALENDAR_LIVE_V41_VERIFIER_VERSION = "calendar-live-v41-verifier-v1";

export function buildCalendarLiveV41RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV41RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV41RawNode[];
}

export interface CalendarLiveV41NormalizedScene {
...
export interface CalendarLiveV41SceneProof {
...
export function normalizeCalendarLiveV41Scene(
  root: CalendarLiveV41RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV41NormalizedScene {
  const canonicalBindings: CalendarLiveV41NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV41RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV41Scene<Instance>(
  normalized: CalendarLiveV41NormalizedScene,
...
): CalendarLiveV41SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV41FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV41ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV41PreCaptureGates(payload: unknown): void {
