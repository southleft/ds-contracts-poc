...
import {
  verifyTableLiveV25FixedSceneFixedPoint,
  type TableLiveV25FixedFixedPoint,
} from "./table-live-v25-fixed-point.js";

export const TABLE_LIVE_V25_VERIFIER_VERSION = "table-live-v25-verifier-v1";

export function buildTableLiveV25RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV25RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV25RawNode[];
}

export interface TableLiveV25NormalizedScene {
...
export interface TableLiveV25SceneProof {
...
export function normalizeTableLiveV25Scene(
  root: TableLiveV25RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV25NormalizedScene {
  const canonicalBindings: TableLiveV25NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV25RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV25Scene<Instance>(
  normalized: TableLiveV25NormalizedScene,
...
): TableLiveV25SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV25FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV25ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV25PreCaptureGates(payload: unknown): void {
