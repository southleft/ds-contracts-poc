...
import {
  verifyComboboxLiveV3FixedSceneFixedPoint,
  type ComboboxLiveV3FixedFixedPoint,
} from "./combobox-live-v3-fixed-point.js";

export const COMBOBOX_LIVE_V3_VERIFIER_VERSION = "combobox-live-v3-verifier-v1";

export function buildComboboxLiveV3RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV3RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV3RawNode[];
}

export interface ComboboxLiveV3NormalizedScene {
...
export interface ComboboxLiveV3SceneProof {
...
export function normalizeComboboxLiveV3Scene(
  root: ComboboxLiveV3RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV3NormalizedScene {
  const canonicalBindings: ComboboxLiveV3NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV3RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV3Scene<Instance>(
  normalized: ComboboxLiveV3NormalizedScene,
...
): ComboboxLiveV3SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV3FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV3ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV3PreCaptureGates(payload: unknown): void {
