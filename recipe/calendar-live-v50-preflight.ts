...
import {
  validateCalendarLiveV50AntecedentIndex,
  verifyCalendarLiveV50Authorization,
  type CalendarLiveV50AntecedentIndex,
  type CalendarLiveV50AuthorizationProof,
} from "./calendar-live-v50-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V50_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v50/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V50_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v50-security-attestation.json";

export interface CalendarLiveV50SecurityAttestation {
  artifactVersion: "calendar-live-v50-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV50PreflightReport {
  artifactVersion: "calendar-live-v50-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV50SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV50SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV50AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v50-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V50_TARGET) ||
...
export function validateCalendarLiveV50ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV50Authorization");
  const preflight = source.indexOf("runCalendarLiveV50Preflight");
  const initialize = source.indexOf("CalendarLiveV50Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV50Preflight(
  root: string,
  proof: CalendarLiveV50AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V50_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV50PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V50_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V50_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV50AntecedentIndex;
  failures.push(...validateCalendarLiveV50AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V50_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V50_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV50SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v50-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV50ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v50 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV50SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v50-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V50_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV50Preflight(
    process.cwd(),
    verifyCalendarLiveV50Authorization(),
    attestationPath,
