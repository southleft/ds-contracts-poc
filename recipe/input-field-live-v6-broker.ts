...
import {
  ...
  INPUT_LIVE_V6_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V6_RESPONSE_CONTRACTS,
  type InputLiveV6RootProof,
} from "./input-field-live-v6-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V6_BROKER_VERSION = "input-live-v6-broker-v2";
export const INPUT_LIVE_V6_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V6_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V6_REMOTE_PHASES = [
  "writer",
  "extract",
  "probe",
  "capture",
...
export const INPUT_LIVE_V6_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV6RemotePhase =
  (typeof INPUT_LIVE_V6_REMOTE_PHASES)[number];
export type InputLiveV6AuthorizationMode = "live" | "simulated";

export interface InputLiveV6CommitPins {
...
export interface InputLiveV6TransactionAuthorization extends InputLiveV6CommitPins {
  mode: InputLiveV6AuthorizationMode;
...
export interface InputLiveV6Manifest {
  artifactVersion: "input-live-v6-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V6_BROKER_VERSION;
...
export interface InputLiveV6ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV6RequestBody {
  ...
  commitPins: InputLiveV6CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V6_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV6ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV6Request extends InputLiveV6RequestBody {
...
export interface InputLiveV6Receipt {
  artifactVersion: "input-live-v6-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V6_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV6RemotePhase;
...
export interface InputLiveV6AcceptedResponse<T = unknown> {
  request: InputLiveV6Request;
  receipt: InputLiveV6Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV6IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV6ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V6_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V6_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V6_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V6_TARGET.editorType;
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Input live v6 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    throw new TypeError("Input live v6 capture index must be 0..127");
  return `capture-${String(captureIndex).padStart(3, "0")}`;
};

export const inputLiveV6RequestSequence = (
  phase: InputLiveV6RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV6RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV6RequestPath = (
  directory: string,
  phase: InputLiveV6RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV6ResponsePath = (
  directory: string,
  phase: InputLiveV6RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV6ReceiptPath = (
  directory: string,
  phase: InputLiveV6RemotePhase,
  captureIndex?: number,
): string =>
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Input live v6 authorization pins are malformed");
};

const readManifest = (directory: string): InputLiveV6Manifest => {
  const manifest = json<InputLiveV6Manifest>(
...
const unsignedBody = (request: InputLiveV6Request): InputLiveV6RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV6RemotePhase,
  captureIndex?: number,
): InputLiveV6Request | undefined => {
  const file = inputLiveV6RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV6Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV6RemotePhase,
  captureIndex?: number,
): InputLiveV6Receipt | undefined => {
  const file = inputLiveV6ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV6Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV6Request | undefined;
  receipt: InputLiveV6Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV6RemotePhase,
): InputLiveV6ResponseContract => ({
  ...INPUT_LIVE_V6_RESPONSE_CONTRACTS[phase],
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v6-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V6_BROKER_VERSION,
  target: INPUT_LIVE_V6_TARGET,
  ...request,
})};
if(figma.fileKey!==__broker.target.fileKey||figma.root.name!==__broker.target.fileName||figma.editorType!==__broker.target.editorType)throw new Error("INPUT-LIVE-V6-WRONG-TARGET");
const payload=await (async()=>{/*INPUT_LIVE_V6_PROGRAM_START*/${program}/*INPUT_LIVE_V6_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V6_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V6_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Input live v6 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createInputLiveV6Transaction(
  directory: string,
  ...
  if (existsSync(path.join(directory, MANIFEST_FILE)))
    throw new Error("Input live v6 transaction already exists");
  validateAuthorization(options.authorization);
  if (
    !SHA256.test(options.proofPlanSha256) ||
    !SHA256.test(options.captureManifestSha256)
  )
    throw new TypeError("Input live v6 transaction plan hashes malformed");
...
export function validateInputLiveV6Request(
  directory: string,
  request: InputLiveV6Request,
): void {
  ...
  const expectedSequence = inputLiveV6RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v6-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V6_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V6_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Input live v6 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueInputLiveV6Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV6RemotePhase,
  program: string,
  options: InputLiveV6IssueOptions = {},
): InputLiveV6Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV6RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV6Request>(targetPath);
    validateInputLiveV6Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v6 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV6ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Input live v6 replay refused: ${id} already accepted`);
  if (phase === "capture") validateInputLiveV6TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Input live v6 out-of-order phase: ${id}`);
  const expectedResponse =
    options.expectedResponse ?? responseContractFor(phase);
  const programSha256 = sha256(program);
  const sequence = inputLiveV6RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V6_TARGET.fileKey,
  ...
  const body: InputLiveV6RequestBody = {
    artifactVersion: "input-live-v6-broker-request-v2",
    brokerVersion: INPUT_LIVE_V6_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV6Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV6Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV6RawResponse(
  directory: string,
  phase: InputLiveV6RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV6RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v6 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV6Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Input live v6 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = inputLiveV6ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Input live v6 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV6Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v6-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V6_BROKER_VERSION,
...
export function acceptInputLiveV6Response<T = unknown>(
  directory: string,
  phase: InputLiveV6RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV6AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV6ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v6 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV6Request>(
    inputLiveV6RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV6Request(directory, request);
  if (phase === "capture") validateInputLiveV6TechnicalGates(directory);
  const rawFile = inputLiveV6ResponsePath(directory, phase, captureIndex);
  if (!existsSync(rawFile))
    throw new Error(`Input live v6 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v6-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V6_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Input live v6 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistInputLiveV6TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV6RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV6RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Input live v6 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Input live v6 probe payload hash malformed");
  const gates = Object.fromEntries(
    INPUT_LIVE_V6_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v6-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V6_BROKER_VERSION,
...
export function validateInputLiveV6TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v6-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V6_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Input live v6 technical gates artifact");
}

export function validateInputLiveV6Receipt(
  directory: string,
  phase: InputLiveV6RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV6Request(directory, request);
  const receipt = json<InputLiveV6Receipt>(
    inputLiveV6ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV6ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v6-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V6_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Input live v6 accepted receipt: ${request.requestId}`,
    );
}

export function readInputLiveV6AcceptedRawPayload(
  directory: string,
  phase: InputLiveV6RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV6Receipt(directory, phase, captureIndex);
...
export function createInputLiveV6FakeDynamicResponse(
  request: InputLiveV6Request,
  payload: unknown,
  overrides: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V6_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V6_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV6PersistedRequests(
  directory: string,
): InputLiveV6Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV6Request>(file)] : [];
    });
}

export const inputLiveV6Sha256 = sha256;
