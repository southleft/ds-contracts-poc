...
import {
  ...
  INPUT_LIVE_V53_INDEX_PATH,
} from "./build-input-field-live-proof-v53.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV53HistoryExpectation = "pending" | "authorized";

export interface InputLiveV53AuthorizationArtifact {
  artifactVersion: "input-live-v53-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V53_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV53AntecedentIndex {
...
export interface InputLiveV53HistoryState {
  index?: InputLiveV53AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV53AuthorizationArtifact;
...
export interface InputLiveV53AuthorizationProof extends InputLiveV53TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V53_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V53_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V53_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V53_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v53-authorization") ||
  artifactPath.includes("input-field-live-v53-preflight") ||
  artifactPath.includes("input-field-live-v53-authorized") ||
  artifactPath.includes("input-field-live-pivot-v53-status") ||
...
export function validateInputLiveV53AntecedentIndex(
  index: InputLiveV53AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v53-antecedent-index-v1" ||
...
export function validateInputLiveV53UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV53History\(\s*["']/.test(source) ||
    /readInputLiveV53HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV53AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV53AuthorizationArtifact {
  ...
  ) as InputLiveV53AntecedentIndex;
  const indexFailures = validateInputLiveV53AntecedentIndex(index);
...
export function validateInputLiveV53AuthorizationArtifact(
  artifact: InputLiveV53AuthorizationArtifact | undefined,
  index: InputLiveV53AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v53-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v53" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V53_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V53_TARGET) ||
    !equalJson(
...
export function validateInputLiveV53History(
  state: InputLiveV53HistoryState,
  expected: InputLiveV53HistoryExpectation,
): string[] {
  const failures = validateInputLiveV53AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV53AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV53HistoryState(
  root = process.cwd(),
): InputLiveV53HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V53_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV53AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V53_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V53_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V53_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV53History(
  expected: InputLiveV53HistoryExpectation,
  root = process.cwd(),
): InputLiveV53HistoryState {
  const state = readInputLiveV53HistoryState(root);
  const failures = validateInputLiveV53History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV53Authorization(
  root = process.cwd(),
): InputLiveV53AuthorizationProof {
  const state = verifyInputLiveV53History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V53_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v53/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v53.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V53_TARGET,
    expectedDynamicTool: INPUT_LIVE_V53_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V53_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV53HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV53History(expected);
