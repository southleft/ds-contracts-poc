...
import {
  ...
  verifyInputLiveV37Authorization,
  type InputLiveV37AntecedentIndex,
  type InputLiveV37AuthorizationProof,
} from "./input-field-live-v37-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V37_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v37/operator-security-attestation-template.json";
export const INPUT_LIVE_V37_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v37-security-attestation.json";

export interface InputLiveV37SecurityAttestation {
...
export interface InputLiveV37PreflightReport {
  artifactVersion: "input-live-v37-preflight-v1";
...
export const inputLiveV37AttestationSha256 = (
  value: Omit<InputLiveV37SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV37SecurityAttestation(
  value: unknown,
  proof: InputLiveV37AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v37-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V37_TARGET) ||
...
export function validateInputLiveV37ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV37Authorization");
  const preflight = source.indexOf("runInputLiveV37Preflight");
  const initialize = source.indexOf("InputLiveV37Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV37Preflight(
  root: string,
  proof: InputLiveV37AuthorizationProof,
  attestationPath = INPUT_LIVE_V37_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV37PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V37_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V37_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V37_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV37AntecedentIndex;
  failures.push(...validateInputLiveV37AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V37_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V37_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v37-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV37ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV37SecurityAttestation;
  return {
    artifactVersion: "input-live-v37-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V37_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV37Preflight(
    process.cwd(),
    verifyInputLiveV37Authorization(),
    attestationPath,
