...
import {
  ...
  INPUT_LIVE_V58_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V58_RESPONSE_CONTRACTS,
  type InputLiveV58RootProof,
} from "./input-field-live-v58-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V58_BROKER_VERSION = "input-live-v58-broker-v2";
export const INPUT_LIVE_V58_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V58_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V58_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V58_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V58_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V58_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV58RemotePhase =
  (typeof INPUT_LIVE_V58_REMOTE_PHASES)[number];
export type InputLiveV58AuthorizationMode = "live" | "simulated";

export interface InputLiveV58CommitPins {
...
export interface InputLiveV58TransactionAuthorization extends InputLiveV58CommitPins {
  mode: InputLiveV58AuthorizationMode;
...
export interface InputLiveV58Manifest {
  artifactVersion: "input-live-v58-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V58_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V58_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V58_TARGET;
...
export interface InputLiveV58ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV58RequestBody {
  artifactVersion: "input-live-v58-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V58_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV58RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV58CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V58_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV58ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV58Request extends InputLiveV58RequestBody {
...
export interface InputLiveV58Receipt {
  artifactVersion: "input-live-v58-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V58_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV58RemotePhase;
...
export interface InputLiveV58AcceptedResponse<T = unknown> {
  request: InputLiveV58Request;
  receipt: InputLiveV58Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV58IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV58ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V58_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V58_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V58_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V58_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V58_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V58_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V58_TARGET.editorType;
};
...
export const inputLiveV58RequestSequence = (
  phase: InputLiveV58RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV58RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV58RequestPath = (
  directory: string,
  phase: InputLiveV58RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV58ResponsePath = (
  directory: string,
  phase: InputLiveV58RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV58ReceiptPath = (
  directory: string,
  phase: InputLiveV58RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV58Manifest => {
  const manifest = json<InputLiveV58Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v58-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V58_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V58_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV58RemotePhase,
  captureIndex?: number,
): InputLiveV58Request | undefined => {
  const file = inputLiveV58RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV58Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV58RemotePhase,
  captureIndex?: number,
): InputLiveV58Receipt | undefined => {
  const file = inputLiveV58ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV58Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV58Request | undefined;
  receipt: InputLiveV58Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV58RemotePhase,
): InputLiveV58ResponseContract => ({
  ...INPUT_LIVE_V58_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V58_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V58_DYNAMIC_TOOL,
      target: INPUT_LIVE_V58_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v58-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V58_BROKER_VERSION,
  target: INPUT_LIVE_V58_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V58_PROGRAM_START*/${program}/*INPUT_LIVE_V58_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V58_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V58_PROGRAM_END*/";
...
export function createInputLiveV58Transaction(
  directory: string,
  ...
  const manifest: InputLiveV58Manifest = {
    artifactVersion: "input-live-v58-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V58_BROKER_VERSION,
...
export function validateInputLiveV58Request(
  directory: string,
  request: InputLiveV58Request,
): void {
  ...
  const expectedSequence = inputLiveV58RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v58-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V58_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V58_TARGET.fileKey ||
...
export function issueInputLiveV58Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV58RemotePhase,
  program: string,
  options: InputLiveV58IssueOptions = {},
): InputLiveV58Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV58RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV58Request>(targetPath);
    validateInputLiveV58Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV58ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV58TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV58RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V58_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V58_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V58_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV58RequestBody = {
    artifactVersion: "input-live-v58-broker-request-v2",
    brokerVersion: INPUT_LIVE_V58_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV58Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV58Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV58RawResponse(
  directory: string,
  phase: InputLiveV58RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV58RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV58Request>(requestFile);
  if (
  ...
  const target = inputLiveV58ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV58Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v58-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V58_BROKER_VERSION,
...
export function acceptInputLiveV58Response<T = unknown>(
  directory: string,
  phase: InputLiveV58RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV58AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV58ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV58Request>(
    inputLiveV58RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV58Request(directory, request);
  if (phase === "capture") validateInputLiveV58TechnicalGates(directory);
  const rawFile = inputLiveV58ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v58-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V58_BROKER_VERSION ||
...
export function persistInputLiveV58TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV58RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV58RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V58_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v58-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V58_BROKER_VERSION,
...
export function validateInputLiveV58TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v58-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V58_TECHNICAL_GATES.some(
...
export function validateInputLiveV58Receipt(
  directory: string,
  phase: InputLiveV58RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV58Request(directory, request);
  const receipt = json<InputLiveV58Receipt>(
    inputLiveV58ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV58ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v58-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V58_BROKER_VERSION ||
...
export function readInputLiveV58AcceptedRawPayload(
  directory: string,
  phase: InputLiveV58RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV58Receipt(directory, phase, captureIndex);
...
export function createInputLiveV58FakeDynamicResponse(
  request: InputLiveV58Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V58_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V58_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV58PersistedRequests(
  directory: string,
): InputLiveV58Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV58Request>(file)] : [];
    });
}

export const inputLiveV58Sha256 = sha256;
