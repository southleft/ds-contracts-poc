...
import {
  validateComboboxLiveV11AntecedentIndex,
  verifyComboboxLiveV11Authorization,
  type ComboboxLiveV11AntecedentIndex,
  type ComboboxLiveV11AuthorizationProof,
} from "./combobox-live-v11-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V11_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v11/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V11_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v11-security-attestation.json";

export interface ComboboxLiveV11SecurityAttestation {
  artifactVersion: "combobox-live-v11-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV11PreflightReport {
  artifactVersion: "combobox-live-v11-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV11SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV11SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV11AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v11-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V11_TARGET) ||
...
export function validateComboboxLiveV11ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV11Authorization");
  const preflight = source.indexOf("runComboboxLiveV11Preflight");
  const initialize = source.indexOf("ComboboxLiveV11Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV11Preflight(
  root: string,
  proof: ComboboxLiveV11AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V11_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV11PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V11_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V11_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V11_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV11AntecedentIndex;
  failures.push(...validateComboboxLiveV11AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V11_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V11_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV11SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v11-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV11ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v11 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV11SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v11-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V11_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV11Preflight(
    process.cwd(),
    verifyComboboxLiveV11Authorization(),
    attestationPath,
