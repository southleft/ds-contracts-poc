...
import { COMBOBOX_LIVE_V5_TARGET } from "./combobox-live-v5-broker.js";
import {
  COMBOBOX_LIVE_V5_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV5Authorization();
...
const attestation: ComboboxLiveV5SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV5SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
...
const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V5_SECURITY_ATTESTATION_DEFAULT_PATH,
);
