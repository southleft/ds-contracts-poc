...
export type CalendarLiveV44HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV44AuthorizationArtifact {
  artifactVersion: "calendar-live-v44-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V44_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV44AntecedentIndex {
...
export interface CalendarLiveV44HistoryState {
  index?: CalendarLiveV44AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV44AuthorizationArtifact;
...
export interface CalendarLiveV44AuthorizationProof
  extends CalendarLiveV44TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V44_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V44_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V44_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V44_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v44-authorization") ||
  artifactPath.includes("calendar-live-v44-preflight") ||
  artifactPath.includes("calendar-live-v44-authorized") ||
  artifactPath.includes("create-calendar-live-v44-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v44-status") ||
...
export function validateCalendarLiveV44AntecedentIndex(
  index: CalendarLiveV44AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v44-antecedent-index-v1" ||
...
export function validateCalendarLiveV44UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV44History\(\s*["']/.test(source) ||
    /readCalendarLiveV44HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV44AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV44AuthorizationArtifact {
  ...
  ) as CalendarLiveV44AntecedentIndex;
  const indexFailures = validateCalendarLiveV44AntecedentIndex(index);
...
export function validateCalendarLiveV44AuthorizationArtifact(
  artifact: CalendarLiveV44AuthorizationArtifact | undefined,
  index: CalendarLiveV44AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v44-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v44" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V44_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V44_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V44_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V44_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V44_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V44_SOURCE_ROOTS ||
  ...
  if (
    ...
    artifact?.execution?.comboboxLiveV44AuthorizationReusable !== false ||
...
export function validateCalendarLiveV44History(
  state: CalendarLiveV44HistoryState,
  expected: CalendarLiveV44HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV44AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV44AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV44HistoryState(
  root = process.cwd(),
): CalendarLiveV44HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V44_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV44AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V44_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V44_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V44_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV44History(
  expected: CalendarLiveV44HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV44HistoryState {
  const state = readCalendarLiveV44HistoryState(root);
  const failures = validateCalendarLiveV44History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v44 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV44Authorization(
  root = process.cwd(),
): CalendarLiveV44AuthorizationProof {
  const state = verifyCalendarLiveV44History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V44_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v44/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v44.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V44_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V44_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V44_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV44HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV44History(expected);
  process.stdout.write(
    `Calendar live v44 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
