...
import { verifyTableLiveV17Authorization } from "./table-live-v17-authorization.js";
import { TABLE_LIVE_V17_TARGET } from "./table-live-v17-broker.js";
import {
  TABLE_LIVE_V17_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV17Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v17-session";
...
const attestation: TableLiveV17SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV17SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v17 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V17_SECURITY_ATTESTATION_DEFAULT_PATH,
);
