...
import { buildTableLiveV30CleanupRuntime } from "./table-live-v30-cleanup.js";
import { type TableLiveV30FixedFixedPoint } from "./table-live-v30-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV30RestoreProgram,
  validateTableLiveV30RestorePayload,
  type TableLiveV30RestorePayload,
} from "./table-live-v30-restore.js";
import {
  buildTableLiveV30RawPropertyRuntime,
  normalizeTableLiveV30Scene,
  type TableLiveV30RawNode,
} from "./table-live-v30-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v30.js";
import {
...
export const TABLE_LIVE_V30_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V30_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V30_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V30_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V30_VARIANT_COUNT = 20;
export const TABLE_LIVE_V30_SET_COUNT = 6;
export const TABLE_LIVE_V30_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V30_HOST_PHASES = 3;
export const TABLE_LIVE_V30_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V30_RESTORE_COUNT = 8;
export const TABLE_LIVE_V30_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V30_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV30SourceId = (typeof TABLE_LIVE_V30_SOURCE_IDS)[number];
export type TableLiveV30Kind = "table" | "row" | "cell";

export interface TableLiveV30SourceIdentity {
  source: TableLiveV30SourceId;
...
export interface TableLiveV30WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV30ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV30SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV30RawNode;
    rowScene: TableLiveV30RawNode;
    cellScene: TableLiveV30RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV30RootProof {
  source: TableLiveV30SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV30FixedFixedPoint;
}

export interface TableLiveV30ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV30SourceId;
    adapterIdentity: string;
...
export interface TableLiveV30CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV30SourceId;
  adapterIdentity: string;
...
export interface TableLiveV30CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV30SourceId;
  frameWidth: number;
...
export interface TableLiveV30CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v30 ${label} must be an object`);
...
export function validateTableLiveV30WriterPayload(
  value: unknown,
): TableLiveV30WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V30_NAMESPACE ||
    ...
    throw new TypeError("Table live v30 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Table live v30 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v30 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV30SourceIdentity,
  kind: TableLiveV30Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV30ExtractProgram(
  writer: TableLiveV30WriterOwnership,
  sources: readonly TableLiveV30SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
      "Table live v30 extract requires two source identities",
    );
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V30_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V30_NAMESPACE)}
${buildTableLiveV30RawPropertyRuntime()}
...
export function validateTableLiveV30ExtractPayload(
  value: unknown,
  writer: TableLiveV30WriterOwnership,
): TableLiveV30ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v30 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V30_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v30 extract schema/two-root mismatch");
  return payload as TableLiveV30ExtractPayload;
}

export function proveTableLiveV30Roots<Instance>(
  extract: TableLiveV30ExtractPayload,
  sources: readonly (TableLiveV30SourceIdentity & {
...
): TableLiveV30RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v30 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v30 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV30Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV30Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV30Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV30FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Table live v30 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v30 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v30 compile lost owned sets");
...
export function assertTableLiveV30RootProofs(
  proofs: readonly TableLiveV30RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v30 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v30 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV30ProbeProgram(
  writer: TableLiveV30WriterOwnership,
  sources: readonly TableLiveV30SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V30_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV30ProbePayload(
  value: unknown,
  writer: TableLiveV30WriterOwnership,
): TableLiveV30ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV30ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V30_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v30 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV30CaptureManifest(
  cells: readonly TableLiveV30CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V30_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V30_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V30_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V30_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v30 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV30CaptureCell[],
): string => {
  validateTableLiveV30CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV30CaptureProgram(
  writer: TableLiveV30WriterOwnership,
  cell: TableLiveV30CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V30_CAPTURE_COUNT
  )
    throw new TypeError("Table live v30 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Table live v30 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V30_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV30CapturePayload(
  value: unknown,
  cell: TableLiveV30CaptureCell,
  rawResponseBytes: number,
): TableLiveV30CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV30CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V30_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V30_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v30 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV30CaptureResponses(
  manifest: readonly TableLiveV30CaptureCell[],
  responses: readonly TableLiveV30CapturePayload[],
): void {
  validateTableLiveV30CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v30 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV30ObjectiveReport {
  artifactVersion: "table-live-v30-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV30Objective(
  manifest: readonly TableLiveV30CaptureCell[],
  responses: readonly TableLiveV30CapturePayload[],
): TableLiveV30ObjectiveReport {
  assertTableLiveV30CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v30-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV30CleanupProgram(
  writer: TableLiveV30WriterOwnership,
): string {
  const baseProgram = buildTableLiveV30CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V30_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v30 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV30CleanupPayload(
  payload: unknown,
  writer: TableLiveV30WriterOwnership,
): TableLiveV30CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as TableLiveV30CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v30 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V30_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV30WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
