...
import { tableLiveV2Sha256 } from "./table-live-v25-broker.js";

export interface TableLiveV25LifecycleSimulationReport {
  artifactVersion: "table-live-v25-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV25AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV25Lifecycle(): Promise<TableLiveV25LifecycleSimulationReport> {
  await buildTableLiveV25Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V25_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV25AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v25-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V25_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V25_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV25History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV25History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV25AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V25_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V25_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV25History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV25History(authorized, "pending");
    ...
    await buildTableLiveV25Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V25_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV25AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV25Lifecycle(), null, 2)}\n`,
  );
