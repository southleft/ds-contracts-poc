...
import {
  ...
  createInputLiveV36FakeDynamicResponse,
  ...
  listInputLiveV36PersistedRequests,
  persistInputLiveV36RawResponse,
  persistInputLiveV36TechnicalGates,
  validateInputLiveV36Receipt,
  validateInputLiveV36Request,
  type InputLiveV36RemotePhase,
...
import {
  ...
  type InputLiveV36ExtractPayload,
  type InputLiveV36ProbePayload,
  type InputLiveV36RestorePayload,
  type InputLiveV36RootProof,
  type InputLiveV36WriterOwnership,
} from "./input-field-live-v36-contract.js";
import {
  InputLiveV36Orchestrator,
  simulatedInputLiveV36Authorization,
} from "./run-input-field-live-v36.js";
...
const captures: InputLiveV36CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV36Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v36-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV36Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV36CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV36FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV36FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V36_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V36_TARGET);
  persistInputLiveV36RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV36Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV36Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV36WriterOwnership,
): InputLiveV36RestorePayload => ({
...
const rootProofs = (): InputLiveV36RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV36CaptureCell,
): InputLiveV36CapturePayload => {
  const bytes = Buffer.from(`input-live-v36-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV36Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV36Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV36RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV36RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV36ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV36ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV36CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV36CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV36CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV36Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV36CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV36PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV36ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV36Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV36Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV36Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV36Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V36_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV36RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V36_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV36Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV36Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV36ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV36Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV36WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV36RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV36Request;
    validateInputLiveV36Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV36Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV36CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v36-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV36Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV36Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV36Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V36_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV36RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V36_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV36Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV36Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV36RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V36_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV36Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV36Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV36Request;
