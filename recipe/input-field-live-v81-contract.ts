...
} from "./input-field-live-v3-verifier-v81.js";
import {
  buildInputLiveV81RawPropertyRuntime,
  normalizeInputLiveV81Scene,
  type InputLiveV81RawNode,
} from "./input-field-live-v81-verifier.js";
...
export {
  ...
  type InputLiveV81RestorePayload,
} from "./input-field-live-v81-restore.js";

export const INPUT_LIVE_V81_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V81_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V81_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V81_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V81_VARIANT_COUNT = 256;
export const INPUT_LIVE_V81_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V81_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV81SourceId = (typeof INPUT_LIVE_V81_SOURCE_IDS)[number];

export interface InputLiveV81SourceIdentity {
  source: InputLiveV81SourceId;
...
export interface InputLiveV81WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV81ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV81SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV81RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV81RootProof {
  source: InputLiveV81SourceId;
  adapterIdentity: string;
...
export interface InputLiveV81ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV81SourceId;
    adapterIdentity: string;
...
export interface InputLiveV81CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV81SourceId;
  adapterIdentity: string;
...
export interface InputLiveV81CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV81SourceId;
  strata: InputLiveV81CaptureCell["strata"];
...
export interface InputLiveV81CleanupPayload {
...
export function validateInputLiveV81WriterPayload(
  value: unknown,
): InputLiveV81WriterOwnership {
...
const identityPlan = (source: InputLiveV81SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV81ExtractProgram(
  writer: InputLiveV81WriterOwnership,
  sources: readonly InputLiveV81SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V81_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V81_NAMESPACE)}
${buildInputLiveV81RawPropertyRuntime()}
...
export function validateInputLiveV81ExtractPayload(
  value: unknown,
  writer: InputLiveV81WriterOwnership,
): InputLiveV81ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V81_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV81ExtractPayload;
}

export function proveInputLiveV81Roots<Instance>(
  extract: InputLiveV81ExtractPayload,
  sources: readonly (InputLiveV81SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV81Scene(
      root.scene,
...
export function assertInputLiveV81RootProofs(
  proofs: readonly InputLiveV81RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV81ProbeProgram(
  writer: InputLiveV81WriterOwnership,
  sources: readonly InputLiveV81SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V81_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV81ProbePayload(
  value: unknown,
  writer: InputLiveV81WriterOwnership,
): InputLiveV81ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV81ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V81_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V81_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV81CaptureManifest(
  cells: readonly InputLiveV81CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V81_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V81_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V81_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V81_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V81_SOURCE_IDS.some(
...
export const inputLiveV81CaptureManifestSha256 = (
  cells: readonly InputLiveV81CaptureCell[],
): string => {
  validateInputLiveV81CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV81CaptureProgram(
  writer: InputLiveV81WriterOwnership,
  cell: InputLiveV81CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V81_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V81_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V81_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV81CapturePayload(
  value: unknown,
  cell: InputLiveV81CaptureCell,
  rawResponseBytes: number,
): InputLiveV81CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV81CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V81_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V81_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV81CaptureResponses(
  manifest: readonly InputLiveV81CaptureCell[],
  responses: readonly InputLiveV81CapturePayload[],
): void {
  validateInputLiveV81CaptureManifest(manifest);
  if (
...
export interface InputLiveV81ObjectiveReport {
  artifactVersion: "input-live-v81-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV81Objective(
  manifest: readonly InputLiveV81CaptureCell[],
  responses: readonly InputLiveV81CapturePayload[],
  readReference: (cell: InputLiveV81CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV81CaptureCell) => Buffer,
): InputLiveV81ObjectiveReport {
  assertInputLiveV81CaptureResponses(manifest, responses);
...
export function buildInputLiveV81CleanupProgram(
  writer: InputLiveV81WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V81_NAMESPACE,
...
export function validateInputLiveV81CleanupPayload(
  value: unknown,
  writer: InputLiveV81WriterOwnership,
): InputLiveV81CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV81CleanupPayload;
...
export const INPUT_LIVE_V81_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV81WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
