...
import {
  validateCalendarLiveV35AntecedentIndex,
  verifyCalendarLiveV35Authorization,
  type CalendarLiveV35AntecedentIndex,
  type CalendarLiveV35AuthorizationProof,
} from "./calendar-live-v35-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V35_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v35/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V35_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v35-security-attestation.json";

export interface CalendarLiveV35SecurityAttestation {
  artifactVersion: "calendar-live-v35-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV35PreflightReport {
  artifactVersion: "calendar-live-v35-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV35SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV35SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV35AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v35-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V35_TARGET) ||
...
export function validateCalendarLiveV35ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV35Authorization");
  const preflight = source.indexOf("runCalendarLiveV35Preflight");
  const initialize = source.indexOf("CalendarLiveV35Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV35Preflight(
  root: string,
  proof: CalendarLiveV35AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V35_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV35PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V35_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V35_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV35AntecedentIndex;
  failures.push(...validateCalendarLiveV35AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V35_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V35_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV35SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v35-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV35ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v35 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV35SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v35-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V35_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV35Preflight(
    process.cwd(),
    verifyCalendarLiveV35Authorization(),
    attestationPath,
