...
import { INPUT_LIVE_V85_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v85.js";
import { verifyInputLiveV85Authorization } from "./input-field-live-v85-authorization.js";
import { runInputLiveV85Preflight } from "./input-field-live-v85-preflight.js";
import { InputLiveV85Orchestrator } from "./run-input-field-live-v85.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V85_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v85-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV85Authorization(root);
    runInputLiveV85Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV85Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
