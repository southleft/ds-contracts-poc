...
import {
  ...
  createInputLiveV23FakeDynamicResponse,
  ...
  listInputLiveV23PersistedRequests,
  persistInputLiveV23RawResponse,
  persistInputLiveV23TechnicalGates,
  validateInputLiveV23Receipt,
  validateInputLiveV23Request,
  type InputLiveV23RemotePhase,
...
import {
  ...
  type InputLiveV23ExtractPayload,
  type InputLiveV23ProbePayload,
  type InputLiveV23RestorePayload,
  type InputLiveV23RootProof,
  type InputLiveV23WriterOwnership,
} from "./input-field-live-v23-contract.js";
import {
  InputLiveV23Orchestrator,
  simulatedInputLiveV23Authorization,
} from "./run-input-field-live-v23.js";
...
const captures: InputLiveV23CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV23Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v23-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV23Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV23CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV23FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV23FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V23_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V23_TARGET);
  persistInputLiveV23RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV23Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV23Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV23WriterOwnership,
): InputLiveV23RestorePayload => ({
...
const rootProofs = (): InputLiveV23RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV23CaptureCell,
): InputLiveV23CapturePayload => {
  const bytes = Buffer.from(`input-live-v23-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV23Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV23Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV23RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV23RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV23ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV23ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV23CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV23CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV23CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV23Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV23CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV23PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV23ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV23Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV23Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV23Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV23Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V23_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV23RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V23_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV23Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV23Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV23ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV23Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV23WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV23RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV23Request;
    validateInputLiveV23Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV23Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV23CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v23-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV23Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV23Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV23Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V23_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV23RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V23_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV23Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV23Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV23RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V23_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV23Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV23Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV23Request;
