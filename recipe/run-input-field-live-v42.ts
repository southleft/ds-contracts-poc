...
import {
  acceptInputLiveV42Response,
  createInputLiveV42Transaction,
  inputLiveV42ReceiptPath,
  inputLiveV42RequestPath,
  inputLiveV42ResponsePath,
  inputLiveV42Sha256,
  issueInputLiveV42Request,
  persistInputLiveV42RawResponse,
  persistInputLiveV42TechnicalGates,
  readInputLiveV42AcceptedRawPayload,
  type InputLiveV42Request,
  type InputLiveV42TransactionAuthorization,
} from "./input-field-live-v42-broker.js";
import {
  INPUT_LIVE_V42_CAPTURE_COUNT,
  assertInputLiveV42CaptureResponses,
  assertInputLiveV42RootProofs,
  buildInputLiveV42CaptureProgram,
  buildInputLiveV42CleanupProgram,
  buildInputLiveV42ExtractProgram,
  buildInputLiveV42ProbeProgram,
  buildInputLiveV42RestoreProgram,
  validateInputLiveV42RestorePayload,
  evaluateInputLiveV42Objective,
  inputLiveV42CaptureManifestSha256,
  proveInputLiveV42Roots,
  validateInputLiveV42CaptureManifest,
  validateInputLiveV42CapturePayload,
  validateInputLiveV42CleanupPayload,
  validateInputLiveV42ExtractPayload,
  validateInputLiveV42ProbePayload,
  validateInputLiveV42WriterPayload,
  type InputLiveV42CaptureCell,
  type InputLiveV42CapturePayload,
  type InputLiveV42RootProof,
  type InputLiveV42SourceIdentity,
  type InputLiveV42WriterOwnership,
} from "./input-field-live-v42-contract.js";
import {
  INPUT_LIVE_V42_CAPTURE_MANIFEST_PATH,
  INPUT_LIVE_V42_PLAN_PATH,
  type InputLiveV42ProofPlan,
...
import type { ExpectedScenePlan } from "./scene-readback-v42.js";

export const INPUT_LIVE_V42_RUNNER_VERSION = "input-live-v42-runner-v1";

interface InputLiveV42JournalEntry {
  index: number;
  event: string;
...
interface InputLiveV42OrchestratorState {
  artifactVersion: "input-live-v42-orchestrator-state-v1";
  ...
  journal: InputLiveV42JournalEntry[];
}

interface LoadedProofInputs {
  plan: InputLiveV42ProofPlan;
  planSha256: string;
  captures: InputLiveV42CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    InputLiveV42SourceIdentity & {
...
export interface InputLiveV42NextAction {
...
const writeState = (
  directory: string,
  state: InputLiveV42OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, INPUT_LIVE_V42_PLAN_PATH);
  const capturePath = path.join(root, INPUT_LIVE_V42_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as InputLiveV42ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as InputLiveV42CaptureCell[];
  validateInputLiveV42CaptureManifest(captures);
  if (
    plan.captureManifest.sha256 !== inputLiveV42Sha256(captureBytes) ||
    captureArtifact.cellsSha256 !== inputLiveV42CaptureManifestSha256(captures)
  )
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (
      inputLiveV42Sha256(expectedBytes) !== source.expectedScenePlan.sha256 ||
      inputLiveV42Sha256(uncompressed) !==
  ...
  return {
    plan,
    planSha256: inputLiveV42Sha256(planBytes),
    captures,
    captureManifestSha256: inputLiveV42Sha256(captureBytes),
    sources,
...
const appendJournal = (
  ...
  const payloadSha256 = inputLiveV42Sha256(canonicalJson(payload));
  const body = {
  ...
  const entry = {
    ...body,
    entrySha256: inputLiveV42Sha256(canonicalJson(body)),
  };
  writeNew(
    path.join(
      directory,
...
const writerOwnership = (directory: string): InputLiveV42WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): InputLiveV42RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): InputLiveV42Request =>
  readJson<InputLiveV42Request>(file);

export class InputLiveV42Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueInputLiveV42Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: InputLiveV42OrchestratorState = {
      artifactVersion: "input-live-v42-orchestrator-state-v1",
      transactionDirectory: options.transactionDirectory,
      activeRequest: inputLiveV42RequestPath(
        options.transactionDirectory,
        "writer",
      ),
      cleanupRequest: null,
    ...
    return new InputLiveV42Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<InputLiveV42OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new InputLiveV42Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): InputLiveV42NextAction {
    const cleanup =
      this.#state.cleanupRequest &&
      !this.#state.cleanupAccepted &&
      !existsSync(inputLiveV42ReceiptPath(this.#directory, "cleanup"))
        ? this.#state.cleanupRequest
        : null;
  ...
  ingestAndAdvance(rawResponse: string): InputLiveV42NextAction {
  ...
  ingestCleanup(rawResponse: string): InputLiveV42NextAction {
    ...
    persistInputLiveV42RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptInputLiveV42Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV42CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      existsSync(inputLiveV42ReceiptPath(this.#directory, "writer")) &&
      !existsSync(path.join(this.#directory, OWNERSHIP_FILE))
    ) {
      const raw = readInputLiveV42AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateInputLiveV42WriterPayload(raw),
      );
    }
    if (existsSync(inputLiveV42RequestPath(this.#directory, "cleanup")))
      this.#state.cleanupRequest = inputLiveV42RequestPath(
        this.#directory,
        "cleanup",
      );
    if (
      existsSync(path.join(this.#directory, OWNERSHIP_FILE)) &&
      !existsSync(inputLiveV42RequestPath(this.#directory, "cleanup"))
    )
      this.#issueCleanupAndRestore(false);
    else if (
      existsSync(inputLiveV42RequestPath(this.#directory, "cleanup")) &&
      !existsSync(inputLiveV42RequestPath(this.#directory, "restore"))
    )
      this.#issueRestore(false);
    if (
      existsSync(inputLiveV42RequestPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV42ReceiptPath(this.#directory, "restore"))
    )
      this.#state.activeRequest = inputLiveV42RequestPath(
        this.#directory,
        "restore",
      );
    if (
      existsSync(inputLiveV42ReceiptPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV42RequestPath(this.#directory, "extract"))
    ) {
      const ownership = writerOwnership(this.#directory);
      const extract = issueInputLiveV42Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildInputLiveV42ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV42RequestPath(
        this.#directory,
        "extract",
    ...
    if (existsSync(inputLiveV42ReceiptPath(this.#directory, "cleanup")))
      this.#state.cleanupAccepted = true;
    this.#persist();
  }

  #acceptActive(request: InputLiveV42Request): void {
    if (request.phase === "writer") {
      const accepted = acceptInputLiveV42Response(this.#directory, "writer", {
        validate: validateInputLiveV42WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptInputLiveV42Response(this.#directory, "restore", {
        validate: (payload) =>
          validateInputLiveV42RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptInputLiveV42Response(this.#directory, "extract", {
        validate: (payload) =>
          validateInputLiveV42ExtractPayload(payload, ownership),
      });
      const proofs = proveInputLiveV42Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseInputFieldRecipe,
        compileInputFieldRecipe,
      );
      assertInputLiveV42RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "input-live-v42-host-root-proofs-v1",
      ...
      appendJournal(
        ...
        request.requestId,
        { proofsSha256: inputLiveV42Sha256(canonicalJson(proofs)) },
      );
      const next = issueInputLiveV42Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildInputLiveV42ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV42RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptInputLiveV42Response(this.#directory, "probe", {
        validate: (payload) =>
          validateInputLiveV42ProbePayload(payload, ownership),
      });
      persistInputLiveV42TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      const accepted = acceptInputLiveV42Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateInputLiveV42CapturePayload(payload, cell, rawBytes),
      });
      const png = Buffer.from(accepted.payload.pngBase64, "base64");
      const capturePath = path.join(
        this.#directory,
        "captures",
        `${String(cell.index).padStart(3, "0")}-${inputLiveV42Sha256(cell.cellKey).slice(0, 20)}.png`,
      );
      ...
      if (next < INPUT_LIVE_V42_CAPTURE_COUNT) this.#issueCapture(next);
      else {
          ...
          pngBase64: Buffer.from(
            readFileSync(
              path.join(
                this.#directory,
                "captures",
                `${String(response.index).padStart(3, "0")}-${inputLiveV42Sha256(response.cellKey).slice(0, 20)}.png`,
              ),
            ),
          ).toString("base64"),
        }));
        assertInputLiveV42CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateInputLiveV42Objective(
        ...
        appendJournal(
          ...
          request.requestId,
          {
            reportSha256: inputLiveV42Sha256(canonicalJson(objective)),
  ...
  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueInputLiveV42Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildInputLiveV42CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = inputLiveV42RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueInputLiveV42Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildInputLiveV42RestoreProgram(ownership),
    );
    this.#state.activeRequest = inputLiveV42RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueInputLiveV42Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildInputLiveV42CaptureProgram(
  ...
  #readCaptureResponseIndex(): InputLiveV42CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: InputLiveV42CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedInputLiveV42Authorization = (
  privateKey: KeyObject,
): InputLiveV42TransactionAuthorization => ({
  mode: "simulated",
  ...
  signingPublicKeySha256: inputLiveV42Sha256(
...
export const generateInputLiveV42SimulationKey = (): KeyObject =>
