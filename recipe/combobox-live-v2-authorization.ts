...
export type ComboboxLiveV2HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV2AuthorizationArtifact {
  artifactVersion: "combobox-live-v2-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V2_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV2AntecedentIndex {
...
export interface ComboboxLiveV2HistoryState {
  index?: ComboboxLiveV2AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV2AuthorizationArtifact;
...
export interface ComboboxLiveV2AuthorizationProof extends ComboboxLiveV2TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V2_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V2_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V2_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V2_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v2-authorization") ||
  artifactPath.includes("combobox-live-v2-preflight") ||
  artifactPath.includes("combobox-live-v2-authorized") ||
  artifactPath.includes("create-combobox-live-v2-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v2-status") ||
...
export function validateComboboxLiveV2AntecedentIndex(
  index: ComboboxLiveV2AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v2-antecedent-index-v1" ||
...
export function validateComboboxLiveV2UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV2History\(\s*["']/.test(source) ||
    /readComboboxLiveV2HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV2AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV2AuthorizationArtifact {
  ...
  ) as ComboboxLiveV2AntecedentIndex;
  const indexFailures = validateComboboxLiveV2AntecedentIndex(index);
...
export function validateComboboxLiveV2AuthorizationArtifact(
  artifact: ComboboxLiveV2AuthorizationArtifact | undefined,
  index: ComboboxLiveV2AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v2-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v2" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V2_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V2_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V2_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V2_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V2_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V2_SOURCE_ROOTS ||
...
export function validateComboboxLiveV2History(
  state: ComboboxLiveV2HistoryState,
  expected: ComboboxLiveV2HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV2AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV2AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV2HistoryState(
  root = process.cwd(),
): ComboboxLiveV2HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V2_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV2AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V2_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V2_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V2_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV2History(
  expected: ComboboxLiveV2HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV2HistoryState {
  const state = readComboboxLiveV2HistoryState(root);
  const failures = validateComboboxLiveV2History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v1 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV2Authorization(
  root = process.cwd(),
): ComboboxLiveV2AuthorizationProof {
  const state = verifyComboboxLiveV2History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V2_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v2/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v2.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V2_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V2_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V2_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV2HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV2History(expected);
  process.stdout.write(
    `Combobox live v1 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
