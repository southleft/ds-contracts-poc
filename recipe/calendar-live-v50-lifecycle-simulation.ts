...
import { calendarLiveV1Sha256 } from "./calendar-live-v50-broker.js";

export interface CalendarLiveV50LifecycleSimulationReport {
  artifactVersion: "calendar-live-v50-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV50AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV50Lifecycle(): Promise<CalendarLiveV50LifecycleSimulationReport> {
  await buildCalendarLiveV50Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V50_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV50AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v50-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V50_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V50_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV50History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV50History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV50AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V50_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V50_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV50History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV50History(authorized, "pending");
    ...
    await buildCalendarLiveV50Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V50_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV50AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV50Lifecycle(), null, 2)}\n`,
  );
