...
import {
  ...
  verifyInputLiveV41Authorization,
  type InputLiveV41AntecedentIndex,
  type InputLiveV41AuthorizationProof,
} from "./input-field-live-v41-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V41_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v41/operator-security-attestation-template.json";
export const INPUT_LIVE_V41_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v41-security-attestation.json";

export interface InputLiveV41SecurityAttestation {
...
export interface InputLiveV41PreflightReport {
  artifactVersion: "input-live-v41-preflight-v1";
...
export const inputLiveV41AttestationSha256 = (
  value: Omit<InputLiveV41SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV41SecurityAttestation(
  value: unknown,
  proof: InputLiveV41AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v41-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V41_TARGET) ||
...
export function validateInputLiveV41ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV41Authorization");
  const preflight = source.indexOf("runInputLiveV41Preflight");
  const initialize = source.indexOf("InputLiveV41Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV41Preflight(
  root: string,
  proof: InputLiveV41AuthorizationProof,
  attestationPath = INPUT_LIVE_V41_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV41PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V41_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V41_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V41_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV41AntecedentIndex;
  failures.push(...validateInputLiveV41AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V41_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V41_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v41-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV41ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV41SecurityAttestation;
  return {
    artifactVersion: "input-live-v41-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V41_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV41Preflight(
    process.cwd(),
    verifyInputLiveV41Authorization(),
    attestationPath,
